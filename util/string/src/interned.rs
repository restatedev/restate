    ...
    S: Internable,
{
    ...
    #[inline(always)]
    unsafe fn from_unchecked(s: ReString) -> Self {
        unsafe { Self(S::from_unchecked(s)) }
    }
...
impl Interned<ReString> {
    ...
    pub fn new(s: &str) -> Self {
        ThreadInterner::try_get_or_intern::<ReString>(s).expect("ReString::try_new is infallible")
    }
}
    ...
    pub(crate) fn try_get_or_intern<S: Internable>(s: &str) -> Result<Interned<S>, S::Err> {
        ...
        S::intern_pool().with_borrow_mut(|pool| {
            if let Some(found) = pool.get(s) {
                ...
                Ok(Interned(unsafe { S::from_unchecked(found.clone()) }))
            } else {
...
#[cfg(feature = "serde")]
impl<T: StringLike> serde::Serialize for Interned<T> {
...
#[cfg(feature = "serde")]
impl<'de, S> serde::Deserialize<'de> for Interned<S>
where
    S: Internable,
{
...
mod bilrost_encoding {
    ...
    impl<S: Internable> ForOverwrite<(), Interned<S>> for () {
        ...
        fn for_overwrite() -> Interned<S> {
            ...
            Interned(unsafe { S::from_unchecked(ReString::default()) })
        }
    }
    ...
    impl<S: Internable> EmptyState<(), Interned<S>> for () {
        ...
        fn clear(val: &mut Interned<S>) {
            ...
            *val = Interned(unsafe { S::from_unchecked(ReString::default()) });
        }
    }
