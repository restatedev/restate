...
// Compact strings are typically short enough to use their inline representation.
impl MemCmpTarget for CompactString {
    const STACK_DECODE_LIMIT: usize = size_of::<CompactString>();
}
...
/// A prepared, case-sensitive string-prefix predicate for mem-comparable fields.
///
/// Encodes the prefix once and compares directly with borrowed [`EncodedMemCmpStr`]
/// values. Matching does not allocate or decode strings. The empty prefix matches
/// every string; nullable-field handling belongs to the caller.
#[derive(Debug, Clone)]
pub struct MemCmpPrefix {
    encoded: Box<[u8]>,
    /// End of the prefix's payload, excluding final-group padding and its marker.
    comparison_len: usize,
}

impl MemCmpPrefix {
    /// Prepares a literal UTF-8 prefix; no pattern characters are interpreted.
    pub fn new(prefix: &str) -> Self {
        let mut encoded = Vec::with_capacity(serialized_length(prefix.len()));
        write_bytes(prefix.as_bytes(), &mut encoded);
        let final_group_start = encoded.len() - ENCODED_GROUP_SIZE;
        let tail_len = usize::from(encoded[encoded.len() - 1]);
        Self {
            encoded: encoded.into_boxed_slice(),
            comparison_len: final_group_start + tail_len,
        }
    }

    /// Returns the complete string encoding of the prefix, suitable for an
    /// inclusive lower bound within an enclosing key prefix.
    ///
    /// Its bytewise successor is not an upper bound for string-prefix matching.
    pub fn encoded_lower_bound(&self) -> &[u8] {
        &self.encoded
    }

    /// Appends the exclusive byte boundary after all strings starting with this
    /// prefix. Returns `false` for the empty prefix, without writing anything:
    /// that prefix has no field-local upper bound.
    ///
    /// Preserves continuation markers but excludes terminal padding and the final
    /// marker before incrementing the last payload byte. The result is a seek
    /// boundary, not a complete string encoding; do not append later key fields.
    pub fn encode_upper_bound<B: BufMut>(&self, target: &mut B) -> bool {
        let Some((&last, preceding)) = self.encoded[..self.comparison_len].split_last() else {
            return false;
        };
        // UTF-8 payload bytes never contain 0xff, so this cannot carry into a
        // preceding byte or a continuation marker, even for U+10FFFF.
        target.put_slice(preceding);
        target.put_u8(last + 1);
        true
    }

    /// Returns whether the string represented by `value` starts with this prefix.
    ///
    /// Earlier groups must match completely, including their continuation markers.
    /// In the last compared group, ignore padding but require enough actual payload:
    /// otherwise a prefix containing NUL could match a shorter string's padding.
    /// A continuation marker also satisfies this length check, allowing extensions
    /// of a prefix that ends exactly at a group boundary.
    pub fn matches(&self, value: &EncodedMemCmpStr) -> bool {
        let marker_offset = self.encoded.len() - 1;
        value
            .as_bytes()
            .get(marker_offset)
            .is_some_and(|&marker| marker >= self.encoded[marker_offset])
            && value
                .as_bytes()
                .starts_with(&self.encoded[..self.comparison_len])
    }
}
...
pub fn decode_str_into(input: &[u8], dst: &mut [u8]) -> MemCmpResult<(usize, usize)> {
    ...
    for (index, group) in input.as_chunks::<ENCODED_GROUP_SIZE>().0.iter().enumerate() {
...
fn write_bytes<B: BufMut>(source: &[u8], target: &mut B) {
    ...
    if chunk.len() >= encoded_len {
        let mut pos = 0;
        for group in source[..continuation_len].as_chunks::<GROUP_SIZE>().0 {
...
fn copy_groups<const CHECK_ASCII: bool>(encoded: &[u8], decoded: &mut [u8]) -> bool {
    let mut groups = encoded.as_chunks::<ENCODED_GROUP_SIZE>().0.iter();
    let mut is_ascii = true;
    // full groups copy with a compile-time-known length
    let (full, tail) = decoded.as_chunks_mut::<GROUP_SIZE>();
    for out in full {
...
mod tests {
    ...
    #[test]
    fn prepared_prefix_matches_borrowed_fields_like_str_starts_with() {
        ...
        .into();
        for len in 0..=17 {
            // Exercise padding and terminal/continuation markers around two
            // group boundaries, including prefixes ending in actual NUL bytes.
            samples.push("\0".repeat(len));
            samples.push("a".repeat(len));
            samples.push(format!("{}\0", "a".repeat(len)));
        }
        let keys: Vec<_> = samples
            .iter()
            .map(|value| {
                let mut key = encode_mem_comparable(value).to_vec();
        ...
        for prefix in &samples {
            let prepared = MemCmpPrefix::new(prefix);
            let mut upper = Vec::new();
            let bounded = prepared.encode_upper_bound(&mut upper);
            assert_eq!(bounded, !prefix.is_empty());
            assert_eq!(upper.is_empty(), prefix.is_empty());
            assert_eq!(
                prepared.encoded_lower_bound(),
                encode_mem_comparable(prefix).as_ref()
            );
            for (value, key) in samples.iter().zip(&keys) {
                let (field, remaining) = EncodedMemCmpStr::try_ref_from_prefix(key).unwrap();
                assert_eq!(field.as_bytes().as_ptr(), key.as_ptr());
                assert_eq!(remaining, b"suffix");
                assert_eq!(
                    prepared.matches(field),
                    value.starts_with(prefix),
                    "prefix {prefix:?}, value {value:?}"
                );
                // Bounds must agree with logical prefix matching even with later
                // fields attached, NUL padding, and terminal/continuation changes.
                assert_eq!(
                    key.as_slice() >= prepared.encoded_lower_bound()
                        && (!bounded || key.as_slice() < upper.as_slice()),
                    value.starts_with(prefix),
                    "prefix bounds {prefix:?}, value {value:?}"
                );
            }
        }
        for (prefix, expected) in [
            ("Large", b"Largf".as_slice()),
            ("abcdefgh", b"abcdefgi".as_slice()),
            ("abcdefghi", b"abcdefgh\x09j".as_slice()),
        ] {
            let mut upper = Vec::new();
            assert!(MemCmpPrefix::new(prefix).encode_upper_bound(&mut upper));
