...
use restate_util_random::pseudo_random;
...
impl DurationExt for StdDuration {
    ...
    fn add_jitter(self, multiplier: f32) -> Self {
        ...
        if max_jitter <= MIN_JITTER {
            ...
            let span = max - min;
            // Map the pseudo-random u64 uniformly into 0..span with a wide-multiply
            // reduction (no modulo bias, no float).
            let offset = ((pseudo_random() as u128 * span as u128) >> 64) as u64;
