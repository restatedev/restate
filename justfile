...
docker-local-fedora:
    ...
    staging="$(mktemp -d ./.docker-local-XXXXXX)"
    trap 'rm -rf "$staging"' EXIT
    cp -l target/debug/restate-server "$staging/restate-server" 2>/dev/null || cp target/debug/restate-server "$staging/restate-server"
    # Build the Docker image using the local.Dockerfile
    docker buildx build . --platform linux/{{ _docker_arch }} --file docker/local-fedora.Dockerfile --build-arg RESTATE_BINARY_PATH="$staging/restate-server" --tag={{ docker_image }} --progress='{{ DOCKER_PROGRESS }}' --load
