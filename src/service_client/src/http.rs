...
impl HttpClient {
    ...
    pub fn request(
        ...
        match Self::build_request(uri, version, body, path, headers) {
            Ok(request) => Either::Left(self.client.request(request).map_err(HttpError::Hyper)),
            Err(err) => Either::Right(future::ready(Err(HttpError::Http(err)))),
        }
    }
}
