...
This document describes the testing tasks that should be performed before releasing a new minor version of Restate.
...
**Important**: Make sure that the [`RESTATE_RELEASED_CONTAINER_IMAGE`](https://github.com/restatedev/e2e-verification-runner/blob/main/scripts/run-verification.sh#L8) is pointing to the latest released version of the previous minor release.

### Long-Running Load Tests

<!-- TODO: Document the process for updating and running load tests for new releases -->
...
The following categories require manual testing before each minor release. Create sub-issues for each category and track progress in an umbrella issue.
...
- Resume from a cloud control plane backup
  <!-- TODO: Add link to scripts in restatedev/restate-cloud repo -->
