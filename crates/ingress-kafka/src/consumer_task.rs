...
use restate_types::net::ingest::{DedupSequenceNrQueryRequest, ResponseStatus};
                ...
                let response = self
                    ...
                    .call_rpc(
                        ...
                        DedupSequenceNrQueryRequest {
                            producer_id: legacy_producer_id.clone(),
                        },
        ...
        loop {
            tokio::select! {
                ...
                Some(committed) = Self::head_committed(&mut inflight) => {
                    ...
                    let offset = committed.map_err(|_| Error::IngressClosed)?;

                    ingress_request_counter.increment(1);
                    trace!(
                        topic=%self.topic_partition.0, kafka_partition=%self.topic_partition.1,
                        offset=%offset, consumer_group=%self.consumer_group_id,
                        "store kafka offset",
                    );
                ...
                Some(received) = consumer_stream.next() => {
                    ...
                    if let Some(dedup_offset) = legacy_dedup_offset && offset as u64 <= dedup_offset {
                        // we skip the messages instead of seeking to avoid seeking backward,
                        // plus the gap should be small anyway.
                        debug!(
                            topic=%self.topic_partition.0, kafka_partition=%self.topic_partition.1,
                            offset=%offset, consumer_group=%self.consumer_group_id,
                    ...
                    // while trying to send the message, we also make sure to process committed messages to release permits as soon as
                    // possible, which gives better chances for sending to
                    // not block
                    let mut ingest = self.ingestion.ingest(envelope.partition_key(), envelope);
                    ...
                    let commit_token = loop {
                        tokio::select!{
                            ...
                            Some(committed) = Self::head_committed(&mut inflight) => {
                                ...
                                ingress_request_counter.increment(1);
                                trace!(
                                    topic=%self.topic_partition.0, kafka_partition=%self.topic_partition.1, offset=%offset,
                                    "store kafka offset",
                                );
                                self.consumer.store_offset(&self.topic_partition.0, self.topic_partition.1, offset)?;
                            }
                    ...
                    inflight.push_back(commit_token.map(|_| offset));
                }
            }
        }
    }
...
fn dedup_producer_id(
    ...
    write!(
        hasher,
        "{subscription}:{consumer_group}:{topic}:{partition}"
    )
    .unwrap();
