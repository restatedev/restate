...
async fn topic_partition_queue_consumption_loop<T: TransportConnect>(
    ...
    debug!(
        topic=%topic_partition.0, kafka_partition=%topic_partition.1,
    ...
    let err = loop {
        ...
        if dedup_offset.is_some_and(|dedup_offset| offset as u64 <= dedup_offset) {
            trace!(
                topic=%topic_partition.0, kafka_partition=%topic_partition.1,
                offset=%offset, consumer_group=%consumer_group_id,
...
async fn query_partition_dedup_offset<T: TransportConnect>(
    ...
    let result = async {
        ...
        match req.status {
            ResponseStatus::Ack => Ok(req.sequence_number),
            status => anyhow::bail!(
                "failed to query latest dedup sequence number for partition \
            {partition_id} (node: {node_id}: {status:?}"
            ),
        }
    }
    .await;
