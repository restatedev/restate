...
struct Bin {
    pool: MemoryPool,
    updater: Box<dyn Fn(&MemoryPool) + Send + Sync>,
    used_gauge: Gauge,
...
impl MemoryController {
    pub fn create_pool(
        &self,
        name: &'static str,
        capacity: impl Into<NonZeroByteCount>,
        updater: impl Fn(&MemoryPool) + Send + Sync + 'static,
    ) -> MemoryPool {
        ...
        let pool = MemoryPool::with_capacity(capacity.into());
