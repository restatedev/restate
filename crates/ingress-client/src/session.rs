    ...
    T: TransportConnect,
{
    ...
    async fn connected(&mut self, ctx: &CancellationToken, connection: Connection) -> SessionState {
        ...
        let chunked = if self.opts.batch_timeout != Duration::ZERO {
            ChunksTimeout::new(&mut self.rx, self.opts.batch_size, self.opts.batch_timeout)
                .left_stream()
        } else {
            (&mut self.rx)
                .ready_chunks(self.opts.batch_size)
                .right_stream()
        };
        ...
        // don't lose the buffered batch in case of ChunksTimeout
        if let Either::Left(chunked) = chunked.as_pin_mut() {
