...
fn get_invocation_status_from<S: StorageAccess>(
    storage: &mut S,
    partition_key: PartitionKey,
    ...
    if invocation_statuses.is_empty() {
        Ok(None)
    } else {
...
fn delete_invocation_status<S: StorageAccess>(storage: &mut S, service_id: &ServiceId) {
