    ...
    fn iterator_from<K: TableKey>(
        ...
        match scan {
            PhysicalScan::Prefix(table, key_kind, prefix) => {
                ...
                let mut it = self.prefix_iterator(table, key_kind, prefix.clone());
                it.seek(prefix);
                it
            }
            PhysicalScan::RangeExclusive(table, key_kind, scan_mode, start, end) => {
                ...
                let mut it =
                    self.range_iterator(table, key_kind, scan_mode, start.as_ref()..end.as_ref());
                it.seek(start);
                it
            }
        }
...
impl<'a> StorageAccess for RocksDBTransaction<'a> {
    ...
    fn iterator_from<K: TableKey>(
        ...
        match scan {
            ...
            PhysicalScan::RangeExclusive(table, key_kind, scan_mode, start, end) => {
                let mut it = self.range_iterator(table, key_kind, scan_mode, start.clone()..end);
                it.seek(start);
                it
            }
        }
