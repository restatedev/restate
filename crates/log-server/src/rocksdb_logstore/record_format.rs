...
impl DataRecordEncoder<'_> {
    ...
    pub fn encode_to_disk_format(
        ...
        let encoder = match format_version {
            RecordFormat::V1 => v1::DataRecordEncoder::from(self.0),
        };

        let header_size = size_of::<RecordFormat>() + encoder.header_size();
        scratch.reserve(header_size);
        // Write the format version
        scratch.put_u8(format_version as u8);
        // Delegate to the sub-encoder
