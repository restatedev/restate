...
use super::memory::LogStoreMemoryConfig;
...
impl restate_rocksdb::configuration::DbConfigurator for RocksDbConfigurator {
    ...
    fn get_db_options(
        ...
        let mem_config = LogStoreMemoryConfig::calculate(log_server_config);
        ...
        // dedicated environment for log-server
        db_options.set_env(&self.env);
        // todo: consider a dedicated rate limiter for log-server
        ...
        if !log_server_config.rocksdb_disable_wal() {
            ...
            db_options.set_recycle_log_file_num(mem_config.max_write_buffer_number() as usize);
        }
        ...
        db_options.set_max_total_wal_size(mem_config.max_wal_total_size() as u64);
    ...
    fn note_config_update(&self, db: &restate_rocksdb::RocksAccess) {
        ...
        let mem_config = LogStoreMemoryConfig::calculate(config);

        update_data_cf_budget(db, &mem_config);
        ...
        update_metadata_cf_write_buffer_size(db, mem_config.max_bytes_for_level_base());
        ...
        let will_flush = total_data_usage > mem_config.memory_budget().as_usize();
        info!(
            "Updating log-server data CF memory budget. usage:{}/{}, will_flush: {}",
            ByteCount::from(total_data_usage),
            mem_config.memory_budget(),
            will_flush
        );
        if will_flush {
            db.flush_memtables(
...
fn update_data_cf_budget(db: &restate_rocksdb::RocksAccess, mem_config: &LogStoreMemoryConfig) {
    let max_bytes_for_level_base_str = mem_config.max_bytes_for_level_base().to_restring();
    let write_buffer_size_str = mem_config.write_buffer_size().to_restring();
    let max_write_buffer_number_str = mem_config.max_write_buffer_number().to_restring();
...
fn update_metadata_cf_write_buffer_size(db: &restate_rocksdb::RocksAccess, l0_budget: usize) {
    let write_buffer_size = l0_budget.to_string();
...
fn cf_data_options(
    ...
    opts.set_min_write_buffer_number_to_merge(mem_config.min_write_buffer_number_to_merge() as i32);

    opts.set_write_buffer_size(mem_config.write_buffer_size());
    opts.set_max_write_buffer_number(mem_config.max_write_buffer_number() as i32);
    opts.set_target_file_size_base(mem_config.target_file_size_base() as u64);
    opts.set_max_compaction_bytes(mem_config.max_compaction_bytes() as u64);
    opts.set_max_bytes_for_level_base(mem_config.max_bytes_for_level_base() as u64);
...
fn cf_metadata_options(
    ...
    opts.set_max_write_buffer_number(12);
