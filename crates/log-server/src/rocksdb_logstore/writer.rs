...
impl LogStoreWriter {
    ...
    async fn handle_commands(
        ...
        let mut all_memory: MemoryReservation = MemoryReservation::unlinked();
    ...
    async fn commit(&mut self, opts: &LogServerOptions, reservation: MemoryReservation) {
        // Take the batch out without allocating a replacement (Option::take leaves None).
        // Note that the write batch is rapped in `Option` so we can move it out without
