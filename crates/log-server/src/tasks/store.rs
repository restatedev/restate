...
impl StoreStorageTask {
    pub fn new(
        loglet_id: LogletId,
        memory: MemoryLease,
        reply_to: Reciprocal<Oneshot<Stored>>,
    ) -> Self {
        Self {
            loglet_id,
    ...
    pub fn set_sequencer(&mut self, sequencer: GenerationalNodeId) {
...
impl OnComplete for StoreStorageTask {
    fn on_complete(
        ...
        if let Some(reply_to) = self.reply_to.take() {
            let memory = self.memory.release();
            trace!(
                loglet_id = %self.loglet_id,
                "Store request completed [{status}]. size={}, local_tail: {local_tail}, global_tail: {global_tail}, total={}, enqueue_duration: {}, commit_duration: {}.",
                ByteCount::from(memory),
