    ...
    async fn provision_node(
        ...
        if let Some(marker) = &maybe_marker {
            if *marker.id() != my_node_id.as_plain() {
                return Err(anyhow::anyhow!(
                    "log-server data marker was found but it was created by another node. Found {} while this node is {}",
                    marker.id(),
                    my_node_id.as_plain()
                ));
            } else {
                debug!(
                    "Found log-server data marker, written at '{}'",
        ...
        match my_storage_state {
            StorageState::Provisioning => {
                ...
                debug!(
                    "Written log-server marker for my node {} at {}",
                    new_marker.id(),
