...
impl RequestPump {
    pub fn new(
        ...
        let data_pool = MemoryPool::with_capacity(config.log_server.data_service_memory_limit);
        // 2MiB for all metadata requests of all loglets
        let meta_pool = MemoryPool::with_capacity(NonZeroUsize::new(2 * 1024 * 1024).unwrap());
