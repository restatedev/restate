...
impl<S: LogStore> LogletWorker<S> {
    ...
    async fn run(
        ...
        loop {
            tokio::select! {
                ...
                Some(current_tail) = local_tail_stream.next(), if self.pending.has_waiting_for_writes() => {
                    self.on_local_tail_change(current_tail);
                }
                _ = tokio::time::sleep(Duration::from_secs(20)), if self.pending.has_waiting_for_writes() => {
    ...
    fn process_store(&mut self, msg: Incoming<Rpc<Store>>) {
        ...
        debug!("Processing store request: {:?}", body.header);
...
impl PendingWaiters {
    ...
    pub fn on_local_tail_change(
        ...
        self.stores
            .drain_up_to(current_tail.offset().prev(), |reciprocal| {
                trace!("Responding to store request for {}", current_tail);
    ...
    pub fn drain_waiting_seal(
        ...
        for reciprocal in self.seals.drain(..) {
            reciprocal.send(Sealed::new(current_tail, global_tail).with_status(status));
        }
    }
