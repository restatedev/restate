...
use restate_types::net::log_server::{Release, Status, Store, StoreFlags, Stored};
...
impl<S: LogStore> LogletWorker<S> {
    pub fn start(
        ...
        let tc_handle = task_center.spawn_unmanaged(
            ...
            "loglet-worker",
            None,
            writer.run(store_rx, release_rx),
        )?;
        ...
        loop {
            tokio::select! {
                ...
                Some(msg) = store_rx.recv() => {
                    if msg.known_global_tail > known_global_tail {
                    ...
                    let peer = msg.peer();
                    let (status, maybe_store_token) = self.process_store(peer, msg.into_body(), &mut staging_local_tail, next_ok_offset).await;
