...
/// down — entries with `inflight() == 0` are dropped, which triggers
/// graceful close via [`conn::Connection`]'s `Drop`.
///
...
async fn eviction_task<C: Clone>(
    ...
    let mut draining: Vec<conn::Connection<C>> = Vec::new();
...
mod test {
    ...
    /// Evicted connections with in-flight streams are kept in the drain
