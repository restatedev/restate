...
struct AuthorityPoolInner<C> {
    epoch: usize,
    connections: VecDeque<Connection<C>>,
}
...
enum AuthorityPoolState<C> {
    ExpandCandidates {
        polled: VecDeque<Connection<C>>,
        unpolled: VecDeque<Connection<C>>,
    },
    PollCandidates {
        candidates: VecDeque<Connection<C>>,
    },
    Ready(Connection<C>),
}
    ...
    C::Error: Into<Error>,
{
    ...
    ///      with `unpolled ∪ polled` and loop back to poll them. The union
    ///      favors newly added connections to the pool.
        ...
        loop {
            let state = self
                ...
                .unwrap_or_else(|| AuthorityPoolState::ExpandCandidates {
                    polled: VecDeque::default(),
                    unpolled: VecDeque::default(),
                });
            ...
            match state {
                ...
                AuthorityPoolState::PollCandidates { mut candidates } => {
                    ...
                    while i < candidates.len() {
                        ...
                        match candidate.poll_ready(cx) {
                            Poll::Ready(Ok(_)) => {
                                self.state = Some(AuthorityPoolState::Ready(
                                    candidates.swap_remove_back(i).unwrap(),
                                ));
                                return Poll::Ready(Ok(()));
                            }
                            Poll::Ready(Err(err)) => {
                ...
                AuthorityPoolState::ExpandCandidates {
                    ...
                    for candidate in &inner.connections {
                        ...
                        unpolled.push_back(candidate.clone());
                    }
                    ...
                    // Note: Avoid dividing by zero by checking that total_max_concurrent_streams == 0
                            ...
                            Some(mut candidate) => {
                                ...
                                match candidate.poll_ready(cx) {
                                    ...
                                    Poll::Pending => {
                                        ...
                                        polled.push_front(candidate);
                                    }
                                    Poll::Ready(Err(err))
                                        if polled.is_empty() && unpolled.is_empty() =>
                                    {
                    ...
                    if !unpolled.is_empty() {
                        // favor new connections
                        unpolled.extend(polled);

                        self.state = Some(AuthorityPoolState::PollCandidates {
                            candidates: unpolled,
                        });
                    } else {
                        return Poll::Pending;
...
mod test {
    ...
    use super::{AuthorityPool, AuthorityPoolState, PoolConfig};
    ...
    /// When several connections all have available streams, `poll_ready`
    /// must select the most recently created one. The pool keeps newest at
    /// the front of `inner.connections` (via `push_front` in
    /// `try_expand_pool`) and the candidate-merge ordering is supposed to
    /// preserve that.
    #[tokio::test]
    async fn favors_newer_connections() {
        let mut pool = make_pool(10);

        // Default `connection_saturation_threshold` is 0.7, so expansion
        // fires when `available_ratio < 0.3`. Holding 8 of 10 streams on the
        // first connection (ratio 0.2) forces the 9th request to open a
        // second connection.
        ...
        let (newer_id, older_id) = {
            let inner = pool.shared.inner.read();
            assert_eq!(inner.connections.len(), 2);
            (
                inner.connections.front().unwrap().id(),
                inner.connections.back().unwrap().id(),
            )
        };

        // Free every stream so both connections fully advertise availability
        // — the choice between them is now driven purely by candidate order.
        drop(bodies);
        ...
        let selected = match &pool.state {
            Some(AuthorityPoolState::Ready(conn)) => conn.id(),
            _ => panic!("expected Ready state after poll_ready"),
        };
        assert_eq!(
            selected, newer_id,
            "expected newer connection (id={newer_id}) to be selected, got id={selected} (older={older_id})",
        );
    }
