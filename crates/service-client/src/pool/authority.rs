    ...
    C::Error: Into<Error>,
{
    ...
    ///        and re-evaluates `should_expand`.
        ...
        loop {
            ...
            match state {
                ...
                AuthorityPoolState::ExpandCandidates {
                    ...
                    let should_expand = total_available_streams == 0
                    ...
                    // At this point we:
                    // Have some candidates either from the shared inner pool or
                    // via the pool expansion mechanism. We think there are enough
                    // streams in the pool for acquisition
                    // so we have 2 options:
                    // - try to poll again if we have added more connections to the pool
                    // - or return pending (if we added none) since the waker has been registered
                    //   on all candidates already
                    assert!(polled.len() + unpolled.len() > 0);
