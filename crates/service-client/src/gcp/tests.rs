...
fn permanently_failing_source() -> IDTokenCredentials {
    MockSource::credentials(|_| MockOutcome::Error(permanent_error("impersonation misconfigured")))
}
    ...
    for (name, failure, retained) in [
        ...
        let source = match failure {
            ...
            MintFailure::Permanent => {
                MockSource::credentials(|_| MockOutcome::Error(permanent_error("misconfigured")))
            }
        };
