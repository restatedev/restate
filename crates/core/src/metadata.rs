                ...
                std::cmp::Ordering::Greater => {
                    ...
                    if let Some(peer) = remote_peer {
                        ...
                        if let PeerAddress::ServerNode(node_id) = peer.peer() {
                            let NodeId::Generational(node_id) = *node_id else {
                                // in practice we should only see generational node ids, but for future
                                // proofing, we can simply ignore nodes that are not fully recognized as
                                // generational yet.
                                return false;
                            };
