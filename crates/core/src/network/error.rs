...
impl From<ConnectError> for NetworkError {
    fn from(value: ConnectError) -> Self {
        match value {
            ...
            ConnectError::Throttled(_, _) => NetworkError::ConnectionFailed(value.to_string()),
            ConnectError::Transport(e) => NetworkError::ConnectionFailed(e),
            ConnectError::Discovery(e) => NetworkError::DiscoveryError(e),
...
pub enum ConnectError {
    ...
    #[error("Rejected connecting to {0}; will remain in connect throttle window error for {1:?}")]
    Throttled(Destination, Duration),
