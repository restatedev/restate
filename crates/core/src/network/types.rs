...
use restate_types::net::{AdvertisedAddress, ProtocolVersion, RpcResponse, SubscriptionResponse};
...
///
/// It's life is bound to the user holding on to the ReplyRx. This is the _dual_ of ReplyRx.
pub(super) type SubscriptionReplyTx = mpsc::Sender<RawSubscriptionReply>;

pub struct SubscriptionRx<O: SubscriptionResponse> {
    protocol_version: ProtocolVersion,
    inner: mpsc::Receiver<RawSubscriptionReply>,
    _marker: PhantomData<O>,
}

impl<O: SubscriptionResponse> SubscriptionRx<O> {
    pub fn new(protocol_version: ProtocolVersion, capacity: usize) -> (SubscriptionReplyTx, Self) {
        let (reply_sender, inner) = mpsc::channel(capacity);
        (
            reply_sender,
            Self {
                protocol_version,
...
pub enum RawSubscriptionReply {
    Message(Bytes),
    Error(rpc_reply::Status),
}
