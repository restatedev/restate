...
use restate_types::net::{RpcRequest, Service, ServiceTag};

use super::incoming::{Incoming, RawRpc, RawSubscribe, RawUnary};
use super::metric_definitions::NETWORK_MESSAGE_PROCESSING_DURATION;
use super::{RawSvcRpc, RawSvcSubscribe, RawSvcUnary, RouterError, Verdict};
...
pub trait Handler: Send + 'static {
    ...
    fn on_subscribe(
        &mut self,
        message: Incoming<RawSvcSubscribe<Self::Service>>,
    ) -> impl Future<Output = ()> + Send {
        std::future::ready(())
    }
}
...
impl MessageRouter {
    ...
    pub async fn call_rpc(
        ...
        permit.send(ServiceOp::CallRpc { msg_type, message });
    ...
    pub async fn call_subscribe(
        ...
        target: ServiceTag,
        msg_type: String,
        message: Incoming<RawSubscribe>,
    ) -> Result<(), RouterError> {
        Ok(self
            .get_sender(target)?
            .reserve()
            .await?
            .send(ServiceOp::Subscribe { msg_type, message }))
    }
    ...
    pub async fn call_unary(
        ...
        Ok(self
            ...
            .reserve()
            .await?
            .send(ServiceOp::Unary { msg_type, message }))
    }
}
...
impl<S: Service> Stream for ServiceStream<S> {
    ...
    fn poll_next(
        ...
        match ready!(self.as_mut().inner.as_mut().poll_recv(cx)) {
            Some(op) => match op {
                ServiceOp::CallRpc { msg_type, message } => {
                    let message = Incoming::from_raw_rpc(msg_type, message);
                    Poll::Ready(Some(ServiceMessage::Rpc(message)))
                }
                ServiceOp::Subscribe { msg_type, message } => {
                    let message = Incoming::from_raw_subscribe(msg_type, message);
                    Poll::Ready(Some(ServiceMessage::Subscribe(message)))
                }
                ServiceOp::Unary { msg_type, message } => {
                    let message = Incoming::from_raw_unary(msg_type, message);
...
pub enum ServiceMessage<S> {
    ...
    Subscribe(Incoming<RawSvcSubscribe<S>>),
...
impl<S: Service> ServiceMessage<S> {
        ...
        M: RpcRequest<Service = S>,
    {
        ...
        (
            ServiceMessage::Rpc(Incoming::from_raw_rpc(M::TYPE.to_owned(), raw_incoming)),
            reply_token,
        )
    }
    ...
    pub fn follow_from_sender(&mut self) {
        match self {
            ...
            Self::Subscribe(i) => i.follow_from_sender(),
...
impl<S: Service> Buffered<S> {
    fn new(rx: ServiceReceiver<S>) -> Self {
        Self { rx }
    }
...
{
    fn new(inner: H, rx: ServiceReceiver<H::Service>) -> Self {
        Self { inner, rx }
    }
            ...
            tokio::select! {
                ...
                Some(msg) = rx.next() => {
                    match msg {
                        ...
                        ServiceMessage::Subscribe(mut message) => {
                            message.follow_from_sender();
                            self.inner.on_subscribe(message).await;
                        }
