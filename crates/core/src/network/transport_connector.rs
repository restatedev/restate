...
pub mod test_util {
    ...
    use crate::network::{
        Connection, ConnectionManager, FakeIncomingConnection, HandshakeError, Incoming,
        MessageRouter, PeerMetadataVersion, Swimlane,
    };
    ...
    pub struct MockConnector<F> {
        ...
        new_connection_sender: mpsc::UnboundedSender<FakeIncomingConnection>,
    }
    ...
    {
        pub fn new(router_factory: F) -> (Self, mpsc::UnboundedReceiver<FakeIncomingConnection>) {
        ...
        F: FnMut(GenerationalNodeId, &mut MessageRouterBuilder) + Send + Sync + 'static,
    {
        async fn connect(
            ...
            let connection = FakeIncomingConnection::new(
                peer_node_id,
