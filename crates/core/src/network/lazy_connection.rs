...
/// A connection to a generational peer with connects in the background
///
        ...
        'connect: loop {
            ...
            let connection = match network_sender.get_connection(peer, swimlane).await {
                ...
                Err(err) if !auto_reconnect => {
                    debug!(%swimlane, %peer, "Connection failed and we were not asked to reconnect: {}", err);
                ...
                Err(err @ ConnectError::Transport(_)) => {
                    debug!(%swimlane, %peer, "Cannot connect, will retry: {}", err);
                    continue 'connect;
                }
                // on failure, we should see throttled in the subsequent attempt
                Err(err @ ConnectError::Throttled(dur)) => {
                    debug!(%swimlane, %peer, "Cannot connect, will retry later: {}", err);
                ...
                Err(
                    ...
                    debug!(%swimlane, %peer, "Cannot to connect, will retry: {}", err);
                    continue 'connect;
                }
                Err(
                ...
                Err(err @ ConnectError::Handshake(HandshakeError::UnsupportedVersion(_))) => {
                    ...
                    debug!(%swimlane, %peer, "Cannot connect, giving up: {}", err);
                    return;
                }
            ...
            loop {
                ...
                // Note that we only try to secure the a send permit after we have an actual message to
                // send. If we don't do this, we risk holding a permit indefinitely (if not messages
                ...
                let Some(permit) = connection.reserve().await else {
                    ...
                    if auto_reconnect {
                        debug!(%swimlane, %peer, "Connection lost to {}, will reconnect", peer);
                        // keep the message so we can send it after we reconnect
                        inflight_msg = Some(msg);
                        continue 'connect;
                    } else {
                        debug!(%swimlane, %peer, "Connection lost to {}", peer);
                        return;
                    }
                };
