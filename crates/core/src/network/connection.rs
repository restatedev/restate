...
impl Connection {
    ...
    pub fn new_fake(
        ...
        protocol_version: ProtocolVersion,
        capacity: usize,
    ) -> (Self, super::io::EgressStream, super::io::Shared) {
        use super::io::EgressStream;

        let (sender, egress, shared) = EgressStream::create(capacity);
        (Self::new(peer, protocol_version, sender), egress, shared)
    }
    ...
    pub(super) async fn connect_inner_with_node_id(
        ...
        let my_node_id = my_node_id.or(metadata.my_node_id_opt());
...
pub mod test_util {
    ...
    // For testing
    //
    // Used to simulate incoming connection.
    #[derive(derive_more::Debug)]
    pub struct FakeIncomingConnection {
    ...
    //     ) -> anyhow::Result<Self> {
    //         let (sender, incoming, shared) = EgressStream::create(message_buffer);
    //
    //         let hello = Hello::new(
    //             Some(from_node_id),
    //             my_cluster_name,
    //             ConnectionDirection::Bidirectional,
    //         );
    //         shared.unbounded_send(EgressMessage::Message(Header::default(), hello.into(), None))?;
    //
    ...
    //     pub fn process_with_message_handler<H: Handler + Send + Sync + 'static>(
    //         self,
    //         handler: H,
