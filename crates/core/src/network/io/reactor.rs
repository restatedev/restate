...
use restate_types::{SharedString, Version, Versioned};
...
impl ConnectionReactor {
    ...
    async fn handle_message(
        ...
        let msg = match msg {
            Some(Ok(msg)) => msg,
            Some(Err(status)) => {
                // stream has terminated. This indicates an unrecoverable error
                // in the underlying stream. It's very likely that we can't
                // send or receive anything. We'll choose to drop the queued
            ...
            None => {
                // we are draining.
                // peer terminated the connection stream has terminated cleanly.
