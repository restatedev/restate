        ...
        match body {
            ...
            Body::Datagram(Datagram {
                ...
                trace!(
                    ...
                    rpc_call.id, rpc_call.message_type
                );
            ...
            Body::Datagram(Datagram {
                datagram: Some(datagram::Datagram::Subscribe(sub)),
            }) => {
                // subscription request
                todo!()
            }
            Body::Datagram(Datagram {
                datagram: Some(datagram::Datagram::SubMessage(msg)),
            }) => {
...
fn send_rpc_error(tx: &super::UnboundedEgressSender, err: RouterError, id: u64) {
    ...
    header.in_response_to = Some(id);

    let _ = tx.unbounded_send(EgressMessage::Message(header, datagram.into(), None));
}
                    ...
                    match reply {
                        Ok(envelope) => {
                            ...
                            let payload = match envelope.body {
                                    ...
                                    // Options:
                                    // 1. ignore v2-only errors
                                    // 2. convert it to v1 message in known cases (PP rpc responses)
                                    // we still want to make sure we ship service/target routing errors
                                    rpc_reply::Body::Status(_status) => unimplemented!(),
