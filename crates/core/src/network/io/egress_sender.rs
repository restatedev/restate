...
impl EgressSender {
    ...
    pub fn try_reserve_owned(self) -> Result<mpsc::OwnedPermit<EgressMessage>, NetworkError> {
        match self.inner.try_reserve_owned() {
            Ok(permit) => Ok(permit),
            Err(mpsc::error::TrySendError::Full(_)) => return Err(NetworkError::Full),
    ...
    /// terminating but no new messages will be accepted. Returns Err if the connection is draining
...
/// senders is how fast the rpc-service's allows us to enqueue requests. RPC services should
