...
pub use restate_memory::MemoryReservation;
...
impl Incoming<RawRpc> {
    ...
    pub(crate) fn swap_reservation(
        &mut self,
        new_reservation: MemoryReservation,
    ) -> MemoryReservation {
...
impl<M: RpcRequest + WireDecode> Incoming<Rpc<M>> {
    ...
    ///
    /// **Panics** if message decoding failed
    pub fn split_with_reservation(
        self,
    ) -> (Reciprocal<Oneshot<M::Response>>, M, MemoryReservation) {
        let body = M::decode(self.inner.payload, self.protocol_version);
        (
            Reciprocal {
                protocol_version: self.protocol_version,
