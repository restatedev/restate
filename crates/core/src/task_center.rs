...
impl TaskCenter {
        ...
        F: Future<Output = anyhow::Result<()>> + 'static,
    {
        ...
        let fut = wrapper(tc, context, future);
    ...
    fn spawn_local_inner<F>(tc: &TaskCenter, context: TaskContext, future: F, localset: &LocalSet)
    where
        F: Future<Output = anyhow::Result<()>> + 'static,
    {
        ...
        F: Future<Output = anyhow::Result<()>> + 'static,
    {
        ...
        // todo: configure the runtime according to kind perhaps?
        ...
        let _ = thread_builder
            .spawn(move || {
                ...
                debug!("Runtime {} exited", runtime_name);
