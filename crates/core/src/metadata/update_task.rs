...
pub struct GlobalMetadataUpdateTask<T> {
    ...
    peers_attempted_for_this_version: HashSet<NodeId>,
...
impl<T: GlobalMetadata> GlobalMetadataUpdateTask<T> {
    pub fn start(
        ...
        let mut next_fetch_interval = tokio::time::interval(Duration::from_millis(100));
    ...
    fn tick(
        ...
        let metadata_store_idle_dur: Duration = config.common.metadata_update_interval.into();
        ...
        let fallback_to_metadata_dur: Duration =
            config.common.metadata_fetch_from_peer_timeout.into();
        ...
        if is_new_version_anticipated {
            ...
            for connection in latest_observed.peers.values() {
                let PeerAddress::ServerNode(node_id) = connection.peer() else {
                    // we should only be dealing with server nodes here actually, but no reason to
                    // panic if otherwise.
                    continue;
                };
                // very rudimentary approach, we only keep a single connection and it's possible
                // (although very rare) that an old generation overrides a new generation, or a
                // connection that's about to be closed takes over a new one.
                //
                // todo: only store the generational node-id with message fabric v2
        ...
        if is_first_run || is_idle || new_version_waiting_for_too_long || is_new_version_no_peers {
            let reasons = [
                is_first_run.then(|| "first start"),
                is_idle.then(|| "staleness check"),
                new_version_waiting_for_too_long.then(|| "new version peer wait timeout"),
                is_new_version_no_peers.then(|| "new version with no peers"),
            ];
