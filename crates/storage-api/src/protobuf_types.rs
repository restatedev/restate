...
pub mod v1 {
    ...
    pub mod lazy {
        ...
        /// Status filter mode: include only matching statuses, or exclude certain statuses.
        #[derive(Clone, Copy, Debug)]
        pub struct StatusFilter {
            set: EnumSet<InvocationStatusDiscriminants>,
            negate: bool,
        }
        ...
        impl StatusFilter {
            ...
            pub fn exclude(status: InvocationStatusDiscriminants) -> Self {
                Self {
                    set: enum_set!(status),
                    negate: true,
                }
            ...
            /// Intersect two status filters (AND), producing the tighter constraint.
            pub fn intersect(self, other: Self) -> Self {
                match (self.negate, other.negate) {
                    (false, false) => Self {
            ...
            /// Union two status filters (OR), producing the looser constraint.
        ...
        pub struct InvocationStatusLazyFilter {
            /// Status filter. If None, all statuses pass.
        ...
        impl InvocationStatusLazyFilter {
            ...
            pub fn completed() -> Self {
                Self {
                    statuses: Some(StatusFilter::include(
            ...
            #[inline]
            fn check_status(&self, status: InvocationStatusDiscriminants) -> bool {
                self.statuses.is_none_or(|filter| filter.check(status))
            }

            #[inline]
            fn check_creation_time(&self, creation_time: u64) -> bool {
                match self.created_after {
                    None => true,
                    Some(ref threshold) => creation_time >= threshold.load(Ordering::Relaxed),
                }
            }

            #[inline]
            fn check_modification_time(&self, modification_time: u64) -> bool {
                match self.modified_after {
                    None => true,
                    Some(ref threshold) => modification_time >= threshold.load(Ordering::Relaxed),
                }
            }
        }
        ...
        impl<'a> InvocationStatusV2Lazy<'a> {
            ...
            pub fn merge_with_filter(
                ...
                while !buf.is_empty() {
                    ...
                    match tag {
                        1 => {
                            // We let through any unregognised status
                            if let Ok(status) = self.status()
                                && !filter.check_status(status)
                            {
                                return Ok(false);
                            }
                        }
                        5 => {
                            if !filter.check_creation_time(self.inner.creation_time) {
                                return Ok(false);
                            }
                        }
                        6 => {
                            if !filter.check_modification_time(self.inner.modification_time) {
                                return Ok(false);
                            }
                        }
                        _ => {}
