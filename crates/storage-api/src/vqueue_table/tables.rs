...
use super::{
    AsEntryState, AsEntryStateHeader, EntryCard, EntryId, EntryKind, EntryStateKind, InboxEntry,
};
use crate::Result;
...
pub trait WriteVQueueTable {
    ...
    /// Removes and returns an entry from an inbox stage
    fn pop_inbox_entry(
        &mut self,
        qid: &VQueueId,
        stage: Stage,
...
pub trait ReadVQueueTable {
    ...
    /// Get the InboxEntry from an inbox stage
    fn get_inbox_entry(
        &mut self,
        qid: &VQueueId,
        stage: Stage,
