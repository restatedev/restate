    ...
    fn apply_update(&mut self, update: &Update) -> anyhow::Result<()> {
        ...
        match update.action {
            ...
            Action::StartAttempt {
                ...
                // only measure queue latency if enqueued_at is set. For entries that were emplaced
                // after resumption, enqueued_at will be None to avoid skewing the wait time.
                if let Some(enqueued_at) = enqueued_at {
                    let latency = now.to_unix_millis().saturating_sub_ms(enqueued_at);
                    self.stats.update_avg_queue_duration(latency);
                }
            }
            Action::Park {
...
pub enum Action {
    ...
    StartAttempt {
        enqueued_at: Option<MillisSinceEpoch>,
        priority: EffectivePriority,
    },
    #[bilrost(tag(4), message)]
    Park {
        release_concurrency_token: bool,
        was_running: bool,
    },
