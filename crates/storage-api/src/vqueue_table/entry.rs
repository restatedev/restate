...
use restate_types::vqueue::{
    EffectivePriority, UserPriority, VQueueId, VQueueInstance, VQueueParent,
};
...
pub struct EntryCard {
    ...
    /// The unique timestamp of the initial enqueue (creation) of the entry.
    pub enqueued_at: UniqueTimestamp,
    pub kind: EntryKind,
    pub id: EntryId,
}
...
impl EntryCard {
    ...
    pub fn encode<B: BufMut>(&self, target: &mut B) {
        ...
        target.put_u64(self.enqueued_at.as_u64());
    ...
    pub fn decode<B: Buf>(source: &mut B) -> crate::Result<Self> {
        ...
        let enqueued_at = UniqueTimestamp::try_from(source.get_u64())
