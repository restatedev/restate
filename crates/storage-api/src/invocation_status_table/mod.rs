...
impl CompletionRangeEpochMap {
    ...
    pub fn into_trim_points(self) -> Vec<(CompletionId, InvocationEpoch)> {
        let mut ranges = self.0.into_iter().collect::<Vec<_>>();
        debug_assert!(
            !ranges.is_empty(),
...
pub struct InFlightInvocationMetadata {
    ...
    /// For more details on the algorithm, see the OnJournalEntryCommand in the PartitionProcessor state machine.
...
mod tests {
    ...
    mod completion_range_epoch_map {
        ...
        fn default() {
            ...
            assert_eq!(map.clone().into_trim_points(), expected_trim_points);
            assert_eq!(
