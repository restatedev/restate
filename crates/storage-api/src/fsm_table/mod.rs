...
pub trait ReadFsmTable {
    ...
    fn get_partition_config_state(
        &mut self,
    ) -> impl Future<Output = Result<Option<StoredPartitionConfigState>>> + Send + '_;
}
...
pub trait WriteFsmTable {
    ...
    fn put_partition_config_state(&mut self, state: &StoredPartitionConfigState) -> Result<()>;
}
...
impl From<PartitionConfiguration> for StoredReplicaSetState {
...
/// *Since v1.6*
#[derive(Debug, Clone, bilrost::Message)]
pub struct StoredPartitionConfigState {
...
impl StorageEncode for StoredPartitionConfigState {
    fn default_codec(&self) -> StorageCodecKind {
        StorageCodecKind::Bilrost
    }

    fn encode(&self, buf: &mut BytesMut) -> Result<(), StorageEncodeError> {
        buf.extend_from_slice(&encode::encode_bilrost(self));
        Ok(())
    }
}

impl StorageDecode for StoredPartitionConfigState {
