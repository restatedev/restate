...
pub enum MoveResult {
    /// Perhaps is not parked?
    NotRunning,
    Success,
}

pub enum WakeUpResult {
    NotParked,
    Success,
}
...
impl<Item> InboxEvent<Item> {
    pub fn new(qid: VQueueId, at: UniqueTimestamp, details: EventDetails<Item>) -> Self {
        Self {
            qid,
            ts: at,
            details,
...
// VQueue
//
// source of truth. Something to see all entriess in the vqueue (modulo completed/failed?)
// ordered by their run_at.
// --> needs more thought.
// vq | PKEY | QID | RUN_AT | LSN | TYPE_TAG | ENTRY_ID. Just the list of ids in the queue.
// PRIORITY = 0 is the highest priority.

// Entries
// entries | PKEY | TTAG | ENTRY_ID -> {status, accumulator}

// VQueue Indexes
//   i_inbox | PKEY | QID | VISIBLE_AT | PRIORITY | LSN | TTAG | ENTRY_ID
//    -> shipped to invoker, after acquiring capacity slot.
//   i_running | PKEY | QID | RUN_AT | PRIORITY | LSN | TTAG | ENTRY_ID
//    -> paused or suspended. on resumption, we put back on inbox, use run_at that's immediate.
//   i_parked | PKEY | QID | PRIORITY | LSN | TTAG | ENTRY_ID -> {status}
//   // see completed by queue source ordered by recently completed first.
//   i_completed | PKEY | Q_SRC_ID | COMPLETED_AT_INVERTED | COMPLETION_LSN | Q_INSTANCE | TTAG | ENTRY_ID
//
//   tokens | PKEY | QID | TTAG | ENTRY_ID -> <status>
//   --> VQueues that may become eligible.
//   active_vqueue PKEY | QID
//
//   park empty vqueue_meta in there for garbage collection either immediately or after some time.
//   empty_vqueue | PKEY | QID
//
// OPTIONAL (item index by status)
// entries | PKEY | status | TTAG | ENTRY_ID -> {status, accumulator}
// OPTIONAL (empty vqueues)
//   park empty vqueue_meta in there for garbage collection either immediately or after some time.
    ...
    S: WriteVQueueTable + ReadVQueueTable,
{
    ...
    pub async fn wake_up(
        ...
        at: UniqueTimestamp,
        card: &EntryCard,
    ) -> Result<WakeUpResult, StorageError> {
        self.storage
    ...
    ) -> Result<ParkResult, StorageError> {
        // Remove from inbox and move to ready
        self.storage
