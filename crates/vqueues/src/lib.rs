...
use restate_storage_api::vqueue_table::{
    ...
    ReadVQueueTable, Stage, WaitStats, WriteVQueueTable, metadata,
};
    ...
    S: WriteVQueueTable + ReadVQueueTable + WriteLockTable,
{
    ...
    {
        // find the entry
        let header = storage.get_entry_state_header(partition_key, id).await?;
        ...
        inbox.end(at, &entry_state, updated_state);
        Ok(true)
    }
        ...
        (): bilrost::encoding::EmptyState<(), T>,
    {
        ...
        let value = EntryValue {
            ...
            first_runnable_at: stats.first_runnable_at.into(),
            metadata: metadata.clone(),
        };

        self.storage
            .put_vqueue_entry(meta.vqueue_id(), Stage::Inbox, &key, &value);
        ...
        (): bilrost::encoding::EmptyState<(), T>,
    {
        ...
        self.storage
            .delete_vqueue_entry(vqueue_id, Stage::Inbox, header.entry_key());
        ...
        self.storage.put_vqueue_entry(
            vqueue_id,
            Stage::Running,
            &modified_key,
            &EntryValue {
                first_started_at: stats.first_attempt_at,
                first_runnable_at: stats.first_runnable_at.into(),
        ...
        (): bilrost::encoding::EmptyState<(), T>,
    {
        ...
        self.storage
            .delete_vqueue_entry(vqueue_id, header.stage(), header.entry_key());
        ...
        self.storage
            .put_vqueue_entry(vqueue_id, Stage::Inbox, &modified_key, &value);
    ...
    pub fn pause_entry<T>(
        ...
        // add new state
    {
        self.park_entry(at, header, Stage::Paused, updated_state)
    }

    /// Suspend an entry
    pub fn suspend_entry<T>(
        &mut self,
        at: UniqueTimestamp,
        header: &impl EntryStateHeader,
        updated_state: &T,
        ...
        (): bilrost::encoding::EmptyState<(), T>,
    {
        self.park_entry(at, header, Stage::Suspended, updated_state)
    }
        ...
        (): bilrost::encoding::EmptyState<(), T>,
    {
        ...
        self.storage
            .put_vqueue_entry(vqueue_id, Stage::Finished, &modified_key, &value);
        ...
        self.storage
            .delete_vqueue_entry_state(vqueue_id.partition_key(), header.entry_id());
        // delete the entry's input
        self.storage
            .delete_item(vqueue_id, header.stats().created_at, header.entry_id());
        // delete the inbox entry
        self.storage
            .delete_vqueue_entry(vqueue_id, Stage::Finished, &modified_key);
        // update cache
