...
impl EligibilityTracker {
        ...
        S::Item: std::fmt::Debug,
    {
        ...
        let eligibility = vqueue.check_eligibility(self.datum.now_ts(), meta, config);
                    ...
                    State::Scheduled {
                        wake_up: WakeUp {
                            ...
                            timer_key: self.delayed_eligibility.insert_at(
                                vqueue.handle,
                                self.datum.ts_to_future_instant(eligible_at),
                            ),
                        },
                    },
                );
            ...
            (Some(State::Scheduled { wake_up }), Eligibility::EligibleAt(eligible_at_ts)) => {
                if eligible_at_ts.cmp_physical(&wake_up.ts) != Ordering::Equal {
                    ...
                    self.delayed_eligibility.reset_at(
                        &wake_up.timer_key,
                        self.datum.ts_to_future_instant(eligible_at_ts),
                    );
                    self.states.insert(
                        vqueue.handle,
                        State::Scheduled {
    ...
    pub fn front_throttled(&mut self, delay_ms: u64) {
        if let Some(handle) = self.ready_ring.front() {
            let now = self.datum.now_ts();
            let wake_up_ts = now.add_millis(delay_ms).unwrap_or(now);
            let wake_up_instant = self.datum.ts_to_future_instant(wake_up_ts);
            ...
            let old = self.states.insert(
                *handle,
                State::Throttled {
                    wake_up: WakeUp {
                        ts: wake_up_ts,
                        timer_key: self.delayed_eligibility.insert_at(*handle, wake_up_instant),
                    },
                },
            );
            // cancel the previous timer
            match old {
                Some(State::Throttled { wake_up } | State::Scheduled { wake_up }) => {
                    self.delayed_eligibility.remove(&wake_up.timer_key);
                }
                _ => {}
            }
