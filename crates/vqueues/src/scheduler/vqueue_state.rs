...
impl<S: VQueueStore> VQueueState<S> {
        ...
        match resources.poll_acquire_permit(
            ...
            AcquireOutcome::Acquired(resources) => {
                ...
                let action = RunAction {
                    key: *inbox_head_key,
                    acquire_lock: resources.has_lock(),
    ...
    pub fn check_eligibility(&self) -> DetailedEligibility {
        ...
        let run_at = inbox_head_key.run_at();
        if !run_at.can_run(SchedulerClock.now_millis()) {
            return DetailedEligibility::Scheduled(run_at.as_millis());
        }
