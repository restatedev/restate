...
pub enum UserPermitKind {
    ...
    LimitKeyConcurrency(LimitKey<ReString>),
}
...
impl PermitBuilder {
    ...
    pub(crate) fn revert(&mut self, resource_manager: &mut ResourceManager) {
        if let Some(user_permit) = self.user_permit.take()
            && let Some(lock) = user_permit.lock
        {
            resource_manager
                .locks
                .release_lock(&lock.scope, &lock.lock_name);
        }

        self.system_permit.take();
    }

    pub(crate) fn take(&mut self, resource_manager: &ResourceManager) -> ReservedResources {
        ReservedResources {
            user_permit: self.user_permit.take().unwrap(),
...
impl ReservedResources {
    ...
    pub fn is_empty(&self) -> bool {
        self.user_permit.resources.is_empty()
    }
}
    ...
    fn drop(&mut self) {
            ...
            && !self.is_empty()
        {
            let _ = manager_tx.send(ResourceManagerUpdate::PermitReleased(
                self.user_permit.resources.drain(..).collect(),
            ));
        }
    }
}
