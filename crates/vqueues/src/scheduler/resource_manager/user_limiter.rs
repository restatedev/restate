...
//!
//! Invariant: a node with zero usage must have an empty waiter list — any waiter implies the
//! counter was at its limit when the vqueue was checked, and every decrement wakes a waiter.
//! This is enforced via `debug_assert` in `is_unused()`.
...
impl UserLimiter {
    pub async fn create() -> Result<Self, StorageError> {
        Ok(Self {
            rules: Rules::default(),
    ...
    ///
    /// Returns a fully-owned [`CapacityResult`] that the caller can inspect, log, or store.
    ...
    /// Removes a vqueue from the waiter list using routing info from `ResourceKind`.
...
impl ScopeNode {
    ...
    fn is_unused(&self) -> bool {
        debug_assert!(
            ...
            "invariant violation: scope node has zero usage but {} waiters",
            self.waiters.len(),
        );
...
impl L1Node {
    ...
    fn is_unused(&self) -> bool {
        debug_assert!(
            !self.value.is_zero() || self.waiters.is_empty(),
            "invariant violation: L1 node has zero usage but {} waiters",
            self.waiters.len(),
        );
...
impl L2Leaf {
    fn is_unused(&self) -> bool {
        debug_assert!(
            !self.value.is_zero() || self.waiters.is_empty(),
            "invariant violation: L2 leaf has zero usage but {} waiters",
            self.waiters.len(),
        );
        self.value.is_zero() && self.waiters.is_empty()
    }
...
// -- Pattern-guided trie walk helpers --
...
fn for_each_matching_scope(
    ...
    match pat {
        Pattern::Exact(val) => {
            // Scope is keyed by Scope type; look up by str
            for (key, node) in scopes.iter_mut() {
                if key.as_str() == val.as_str() {
                    f(node);
                    return;
                }
            }
...
// -- Capacity result types (fully owned, no borrows) --

/// Result of a capacity check across all applicable hierarchy levels.
/// Fully owned — can be returned from methods and stored by the caller.
#[derive(Clone, Debug)]
...
/// Status of a single level's capacity. Fully owned.
#[derive(Clone, Debug)]
