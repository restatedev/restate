    ...
    pub fn on_inbox_event(&mut self, event: VQueueEvent) {
        ...
        for update in event.updates {
            match update {
                ...
                EventDetails::DecisionConfirmed { ref key } => {
...
mod tests {
    ...
    use restate_storage_api::vqueue_table::scheduler::{RunAction, SchedulerAction};
    use restate_storage_api::vqueue_table::{
        EntryId, EntryKey, InvocationEntryMetadata, RunAt, WaitStats,
    };
    ...
    async fn enqueue_entry(
        ...
        let run_at = RunAt::new(MillisSinceEpoch::new(BASE_RUN_AT_MS + run_at_ms));
    ...
    async fn test_run_at_below_now_preempts_within_inbox() {
        ...
        let future_key = enqueue_entry_with_run_at(
            ...
            &qid,
            1,
            RunAt::new(MillisSinceEpoch::new(now.saturating_add(60_000))),
            None,
        )
        .await;
        let overdue_key = enqueue_entry_with_run_at(
            ...
            &qid,
            2,
            RunAt::new(MillisSinceEpoch::new(now.saturating_sub(1_000))),
            None,
        )
        .await;
        txn.commit().await.expect("commit should succeed");
    ...
    async fn test_round_robin_fairness() {
        ...
        for (qid, actions) in &decision.qids {
            ...
            assert_eq!(runs[0].key.seq(), qid.partition_key());
        }
    }
    ...
    async fn test_higher_priority_preempts_between_polls() {
        ...
        assert_eq!(next_keys[0].seq(), 125);
        assert_eq!(next_keys[1].seq(), 3);
        ...
        assert_eq!(keys[0].seq(), 4);
        assert_eq!(keys[1].seq(), 5);
    }
