...
/// **Regression guard.** Reproduces the overlay-at-capacity scenario where
/// the back entry is a tombstone and a new add with `pos < overlay.len()`
/// would historically `pop_back` and silently drop that tombstone — letting
/// the (already-deleted) row leak into the cache.
///
/// The bug is now fixed by the `horizon` mechanism in
/// `RefillTask::prepare_overlay_insert`: on overflow we shrink the horizon
/// (an exclusive upper bound) to the back's key instead of evicting, so any
/// merge row at or above the horizon is dropped and rediscovered on the
/// next refill via `seek_after`. Pre-fix, this test would expose the leak;
/// it is kept to guard against regressions in the overflow path.
///
...
/// ```
///
/// The trigger add (seq=400) sorts at `pos = CAPACITY-1`. With the horizon
/// fix, the overflow path sets `horizon = 500` (the back's key) instead of
/// evicting the tombstone. The merge then drops `Left(seq500)` because
/// `seq500 >= horizon`, so the deleted row never enters the cache.
#[restate_core::test]
async fn tombstone_evicted_on_overlay_overflow_leaks_deleted_row() {
    ...
    // the overlay at cap, the overflow path sets the horizon to the
    // back's key (seq=500) — sealing r_target from the merge instead of
    // evicting its tombstone (which was the original bug).
    ...
    // Trigger the overflow path. With the fix, this sets the horizon
    // and Tombstone(r_target) is preserved (the back becomes the new
    // horizon, sealing r_target from the merge).
    ...
    // Regression: r_target must NOT appear in the cache. The horizon
    // mechanism seals it from the merge so the deleted row stays out
    // even when the overlay would otherwise overflow.
    assert!(
        !drained.contains(&r_target.0),
...
/// `yield_entry` emits `RemovedFromInbox(k)` immediately followed by
/// `EnqueuedToInbox { key: k, .. }` when the modified key equals the
/// original (i.e. `set_run_at(None)` is a no-op). If a refill is in
/// flight and `k` is in the overlay's coverage zone, the overlay must
/// upgrade the tombstone to an `Add` rather than panicking — and the
/// merge result must include the re-enqueued row with its (possibly
/// updated) value.
#[restate_core::test]
async fn overlay_tombstone_then_add_resurrects_row() {
    let target = entry_at_seq(100);

    let storage = GatedStore::new(vec![target.clone()]);
    let qid = test_qid(4);
    let mut queue: Queue<GatedStore> = Queue::new(0, &storage, &qid);
    let skip = UnconfirmedAssignments::new();
    ...
    // Simulate the yield_entry sequence: tombstone followed by an
    // enqueue of the same key with a new value.
    queue.remove(&target.0);
    let mut new_value = target.1.clone();
    new_value.status = Status::Scheduled;
    queue.enqueue(&target.0, &new_value);
