...
use restate_types::logs::metadata::{ProviderKind, SegmentRef};
...
impl BifrostInner {
    ...
    async fn get_trim_point(&self, log_id: LogId) -> Result<Lsn, Error> {
        ...
        let logs_ref = self.metadata.logs();

        let log_chain = logs_ref
            .as_ref()
            .and_then(|logs| logs.logs.get(&log_id))
    ...
    async fn get_loglet(&self, segment: SegmentRef<'_>) -> Result<LogletWrapper, Error> {
