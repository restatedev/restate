...
impl Bifrost {
    ...
    /// buffers.
    pub fn create_appender(&self, log_id: LogId) -> Appender {
        Appender::new(log_id, self.inner.clone())
    }

    /// Like [`Self::create_appender()`] except that it uses the supplied buffer space for
    /// serialization.
    pub fn create_appender_with_serde_buffer(&self, log_id: LogId, buf: BytesMut) -> Appender {
        Appender::with_serde_buffer(log_id, self.inner.clone(), buf)
    }
...
impl BifrostInner {
    ...
    pub(crate) async fn wait_next_unsealed_loglet(
        &self,
        log_id: LogId,
...
mod tests {
    ...
    async fn test_append_smoke() -> googletest::Result<()> {
        ...
        tc.run_in_scope("test", None, async {
            ...
            let mut appender_0 = bifrost.create_appender(LogId::from(0));
            let mut appender_3 = bifrost.create_appender(LogId::from(3));
            ...
            let resp = bifrost.create_appender(invalid_log).append_raw("").await;

            assert_that!(resp, pat!(Err(pat!(Error::UnknownLogId(eq(invalid_log))))));

            // use a cloned bifrost.
            let cloned_bifrost = bifrost.clone();
            let mut second_appender_0 = cloned_bifrost.create_appender(LogId::from(0));
            for _ in 1..=5 {
                // Append a record to memory
