...
pub struct LogReadStream {
    ...
    read_pointer: Lsn,
    /// Used to back-off until a time point in the future regardless of the state machine's state
    //
    // NOTE: Sleep is Pinned box and not #[pin] because Sleep is not `Unpin`. Using #[pin] makes
    // LogReadStream also !Unpin which is something we are trying to avoid.
    maybe_backoff: Option<Pin<Box<tokio::time::Sleep>>>,
    #[pin]
    state: State,
...
impl Stream for LogReadStream {
    ...
    fn poll_next(
        ...
        loop {
            ...
            // Are we backing off?
            if let Some(backoff) = this.maybe_backoff {
                ready!(backoff.as_mut().poll(cx));
                this.maybe_backoff.take();
            }
            ...
            match state {
                ...
                StateProj::Reading {
                    ...
                    // This depends on whether we know its tail (if sealed), or if we the value of
                    // `safe_known_tail` higher than the `read_pointer` of the substream.
