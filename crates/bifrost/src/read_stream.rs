        ...
        loop {
            ...
            match state {
                ...
                StateProj::SealingChain { seal_chain_fut } => {
                    ...
                    // moving to this state. The goal is to finish the seal. We do this by waiting
                    // for the task that'd tell us that our segment is sealed.
                    ready!(seal_chain_fut.poll(cx));
                    // We are done (or failed), we go back to awaiting reconfiguration.
                    // Note that if the seal operation failed, awaiting-reconfiguration will bring
                    // us back here again after the grace period.
                    // awaiting reconfiguration will come back to sealing
                    this.state.set(State::awaiting_reconfiguration());
                    continue;
                }
...
async fn seal_chain(bifrost: &BifrostInner, log_id: LogId, segment_index: SegmentIndex) {
    ...
    {
        Ok(lsn) => {
            info!(%log_id, %segment_index, "Reader sealed the chain at lsn={lsn}, will resume reading");
        }
