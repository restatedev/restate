...
use super::log_store_writer::{RocksDbLogWriter, WriterOptions};
use super::Options;
...
impl RocksDbLogStore {
    pub fn new(storage_path: &Path, options: &Options) -> anyhow::Result<Self> {
        ...
        let cfs = [
            rocksdb::ColumnFamilyDescriptor::new(DATA_CF, cf_options(options, cache.clone())),
            rocksdb::ColumnFamilyDescriptor::new(METADATA_CF, cf_options(options, cache.clone())),
        ];
    ...
    pub fn create_writer(&self, opts: WriterOptions) -> RocksDbLogWriter {
        RocksDbLogWriter::new(self.db.clone(), opts)
    }
...
fn cf_options(opts: &Options, cache: Option<Cache>) -> rocksdb::Options {
    ...
    cf_options.set_min_write_buffer_number_to_merge(10);
