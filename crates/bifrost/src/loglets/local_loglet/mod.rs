...
impl LocalLoglet {
    pub async fn create(
        ...
        let next_write_offset = AtomicU64::new(log_state.release_pointer + 1);
        ...
        debug!(
            ...
            next_offset = loglet.next_write_offset.load(Ordering::Relaxed),
...
impl LogletBase for LocalLoglet {
    ...
    async fn append(&self, payload: Payload) -> Result<LogletOffset, Error> {
        let offset = LogletOffset::from(self.next_write_offset.fetch_add(1, Ordering::Relaxed));
        let res = self
            .log_writer
            .put_record(
                self.log_id,
