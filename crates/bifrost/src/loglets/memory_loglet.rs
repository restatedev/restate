...
impl PartialOrd for OffsetWatcher {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        // reversed comparator. The watcher with the lowest offset is ranks
...
pub struct MemoryLoglet {
    ...
    watchers: Mutex<BinaryHeap<OffsetWatcher>>,
}
...
impl MemoryLoglet {
    ...
    pub fn notify_watchers(&self) {
        // it's safe to not lock the logs mutext because commit offset increases monotonically.
        ...
        while let Some(watcher) = watchers.peek() {
            if watcher.offset <= committed {
                let watcher = watchers.pop().expect("watcher is present");
...
impl LogletBase for MemoryLoglet {
    ...
    /// Find the head (oldest) record in the loglet. If the loglet is empty or have been trimmed, the loglet should
    /// return `None`.
    ...
    async fn read_next_single(
        ...
        loop {
            ...
            if trim_point > after {
                return Ok(LogRecord::new_trim_gap(after.next(), trim_point));
            }
            ...
            if from_offset > commit_offset {
                ...
                let receiver = self.watch_for_offset(from_offset);
                drop(guard);
                receiver.await.unwrap();
                continue;
            } else {
                let index = self.saturating_offset_to_index(from_offset);
                break Ok(LogRecord::new_data(
                    from_offset,
...
mod tests {
    ...
    async fn test_memory_loglet() -> Result<()> {
        ...
        let LogRecord { offset, record } = loglet.read_next_single(LogletOffset::INVALID).await?;
