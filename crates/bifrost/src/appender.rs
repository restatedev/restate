...
use crate::{ErasedInputRecord, Error, InputRecord, Result};
...
impl Appender {
    ...
    pub async fn append<T>(&mut self, body: impl Into<InputRecord<T>>) -> Result<Lsn> {
    ...
    pub async fn append_batch<T>(&mut self, batch: Vec<impl Into<InputRecord<T>>>) -> Result<Lsn> {
