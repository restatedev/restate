...
impl<T: StorageEncode> LogSender<T> {
    ...
    /// draining or drained.
    pub fn enqueue<A>(&mut self, record: A) -> Result<(), EnqueueError<Record>>
    where
    ...
    pub fn enqueue_unchecked<A>(&mut self, record: A) -> Result<(), EnqueueError<Record>>
    where
        A: Into<InputRecord<T>>,
    {
        let record = self.admit(record.into().into_record())?;
    ...
    pub fn enqueue_many_unchecked<I, A>(&mut self, records: I) -> Result<(), EnqueueError<()>>
    where
        I: Iterator<Item = A> + ExactSizeIterator,
        A: Into<InputRecord<T>>,
    {
    ...
    pub fn enqueue_with_notification<A>(
        &mut self,
        record: A,
    ) -> Result<CommitToken, EnqueueError<Record>>
    where
        A: Into<InputRecord<T>>,
    {
        let record = self.admit(record.into().into_record())?;
