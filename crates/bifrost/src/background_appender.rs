...
/// A future that resolves when a record is committed by the appender.
...
/// A handle to a background appender.
///
/// Dropping this handle will request a graceful drain of the background task with no guarantee
...
/// Additionally it batches records whenever possible to reduce round-trips to the loglet.
#[pin_project]
pub struct BackgroundAppender<T> {
    appender: Appender,
    /// The maximum number of records that can be buffered in the queue while the current batch is
    /// being written to the loglet.
    queue_capacity: usize,
    /// The maximum number of records that get batched together per loglet append
    max_batch_size: usize,
    /// A reusable allocation for buffering recv() operations
    recv_buffer: Vec<AppendOperation<T>>,
    /// A reusable buffer for callbacks of enqueue_with_notification calls
    notif_buffer: Vec<oneshot::Sender<()>>,
    /// A reusable buffer for serialized payloads
    ...
    T: HasRecordKeys + StorageEncode + 'static,
{
    ...
    /// Starts the background appender. The appender starts as a spawned background task
    /// and will immediately start processing records.
    pub fn start(
        self,
