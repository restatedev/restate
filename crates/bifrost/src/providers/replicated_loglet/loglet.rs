    ...
    pub async fn find_tail_inner(
        ...
        match self.sequencer {
            ...
            SequencerAccess::Remote { .. } => {
                ...
                let tail_status = task.run(opts).await;
                match tail_status {
...
impl<T: TransportConnect> Loglet for ReplicatedLoglet<T> {
    #[instrument(
        ...
        fields(
            ...
            otel.name = "replicated_loglet: create_read_stream",
        )
    )]
    ...
    #[instrument(
        ...
        fields(
            loglet_id = %self.my_params.loglet_id,
            otel.name = "replicated_loglet: find_tail",
        )
    )]
    ...
    async fn seal(&self) -> Result<(), OperationError> {
        ...
        //   not be up-to-date at the time we sealed.
