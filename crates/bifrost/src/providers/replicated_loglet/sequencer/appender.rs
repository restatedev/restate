    ...
    pub async fn run(mut self) {
        ...
        let final_state = loop {
            state = match state {
                ...
                State::Backoff => {
                    ...
                    // todo(asoli): upgrade to error after a few waves.
                    info!(
        ...
        match final_state {
            State::Done => {
                ...
                trace!("SequencerAppender task completed");
            }
            State::Cancelled => {
                trace!("SequencerAppender task cancelled");
                if let Some(commit_resolver) = self.commit_resolver.take() {
                    commit_resolver.error(AppendError::Shutdown(ShutdownError));
                }
            }
            State::Sealed => {
                debug!("SequencerAppender ended because of sealing");
                if let Some(commit_resolver) = self.commit_resolver.take() {
...
impl<T: TransportConnect> LogServerStoreTask<T> {
    ...
    async fn send(&mut self) -> Result<StoreTaskStatus, NetworkError> {
        ...
        // time it took the global or local tail for this node to move.
        // todo: remove me
        debug!(
            peer = %self.node_id,
            "waited for tail to reach our first offset for {:?}",
            wait_start.elapsed()
        );
