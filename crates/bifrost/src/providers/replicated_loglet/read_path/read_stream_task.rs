...
impl ReadStreamTask {
    ...
    async fn run<T: TransportConnect>(
        ...
        let readahead_trigger_watermark = 50;
        debug_assert!(readahead_trigger_watermark >= 1);
        ...
        'main: loop {
            ...
            // - Capacity is 100 (`readahead_trigger_watermark=50`)
            ...
            // - reader reads 40
            // - Capacity is 51
            // - readahead_trigger_watermark exceeded. We try to fetch 49. Capacity => 0.
            ...
            //    is zero, we'll try to acquire capacity of `readahead_trigger_watermark`
            ...
            debug_assert!(self.read_pointer <= self.last_known_tail);
            if !self.can_advance() {
                // HODL.
            ...
            assert!(self.last_known_tail > self.read_pointer);
            ...
            let mut permits = self
                ...
                .reserve_many(
                    self.tx
                        .capacity()
                        .max(readahead_trigger_watermark)
                        .min(readahead_max),
                )
