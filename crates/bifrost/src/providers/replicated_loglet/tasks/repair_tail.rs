...
/// - Limit `from_offset` to repair from to max(min(local_tails), max(known_global_tails), known_archived, trim_point)
...
impl<T: TransportConnect> RepairTail<T> {
    pub fn new(
        ...
        let digests = Digests::new(&my_params, from_offset, target_tail);
        RepairTail {
            my_params,
            task_center,
            networking,
    ...
    pub async fn run(mut self) -> RepairTailResult {
        ...
        for node in effective_nodeset.iter() {
            let msg = GetDigest {
                ...
                from_offset: self.digests.from_offset(),
        ...
        // Keep reading responses from digest requests since it can assist moving the from_offset
        ...
        // For every record between from_offset->target_tail.prev() we need to replicate enough
        // copies to satisfy the write-quorum.
        //
        // Replication goes in the direction of "from_offset" towards the tail, one record at a
        // a time.
