...
pub struct Digests {
    ...
    // all offsets `[from_offset..target_tail)`
    offsets_under_repair: BTreeMap<u32, NodeSet>,
    known_nodes: NodeSet,
...
impl Digests {
    pub fn new(
        ...
        let offsets = if from_offset >= target_tail {
            // already finished
            Default::default()
        } else {
            let offsets =
                BTreeMap::from_iter((*from_offset..*target_tail).map(|o| (o, Default::default())));
            offsets
        };
    ...
    pub fn from_offset(&self) -> LogletOffset {
        self.from_offset
    }
    ...
    pub fn is_finished(&self) -> bool {
        self.from_offset >= self.target_tail
    }
    ...
    pub fn on_digest_message(
        ...
        self.update_from_offset(known_global_tail.latest_offset());
        ...
        for entry in msg.entries {
            for o in *entry.from_offset..=*entry.to_offset {
                if o >= *self.from_offset && entry.status == RecordStatus::Exists {
                    self.offsets_under_repair
                        .get_mut(&o)
                        .expect("offset in original range")
                        .insert(peer_node);
                }
            }
        }
    ...
    pub async fn replicate_record_and_advance<T: TransportConnect>(
        ...
        if entry.is_trim_gap() {
            ...
            self.update_from_offset(entry.next_sequence_number());
            return Ok(());
        }

        // get a spread. See how many copies do we need to do to achieve write quorum.
        let existing_spread = self
            .offsets_under_repair
            .get(&offset)
            .expect("repairing an offset that we have not truncated");

        let nodes_to_amend = self
            .spread_selector
            .amend(
                existing_spread,
                &mut thread_rng(),
        ...
        trace!(
            loglet_id = %self.loglet_id,
            %offset,
            "Repairing record, existing copies on {} and nodes to add are {}",
            existing_spread,
            nodes_to_amend,
        );
        ...
        replication_checker.set_attribute_on_each(existing_spread, || true);

        let msg = Store {
            // As we send store messages, we consider the from_offset a reliable source of
            // global_known_loglet. This allows log-servers to accept those writes if they were still
            // behind.
            header: LogServerRequestHeader::new(self.loglet_id, self.from_offset),
            timeout_at: None,
        ...
        loop {
            if replication_checker.check_write_quorum(|attr| *attr) {
                ...
                self.update_from_offset(offset);
                return Ok(());
            }
    ...
    pub fn advance(&mut self, nodes_config: &NodesConfiguration) -> bool {
        if !self.can_repair(nodes_config) {
            // we don't have write-quorum of nodes yet, we can't advance from_offset.
            return false;
        }
        ...
        while let Some((offset, nodes)) = range.next_back() {
            ...
            if checker.check_write_quorum(|known| *known) {
                self.update_from_offset(LogletOffset::new(offset + 1));
                return true;
            }
        }
        false
    ...
    fn update_from_offset(&mut self, new_from_offset: LogletOffset) {
        let original = self.from_offset;
        self.from_offset = self.from_offset.max(new_from_offset);
        if self.from_offset != original {
            self.truncate_range();
        }
    }
    ...
    fn truncate_range(&mut self) {
        if self.is_finished() {
            ...
            self.offsets_under_repair = self.offsets_under_repair.split_off(&self.from_offset);
        }
    }
}
