...
impl<'a, Attribute> NodeSetChecker<'a, Attribute> {
    ...
    where
        Attribute: Default,
    {
        Self::with_factory(node_set, nodes_config, replication_property, |_| {
            Default::default()
        })
    }
    ...
    pub fn with_factory(
        ...
        let storage_states: HashMap<_, _> = node_set
            .iter()
            .filter_map(|n| {
                match nodes_config.find_node_by_id(*n) {
                    // storage states. Only include nodes that enable reads or above.
                    Ok(config) if config.log_server_config.storage_state.should_read_from() => {
                        Some((*n, config.log_server_config.storage_state))
                    }
                    // node is not readable. Treat as DISABLED
                    Ok(_) => None,
                    // node doesn't exist. Consider it DISABLED
                    Err(_) => None,
                }
            })
            .collect();
...
mod tests {
    ...
    fn test_replication_checker_basics() -> Result<()> {
        ...
        // all_authoritative
        for i in 1..=10 {
            nodes_config.upsert_node(generate_logserver_node(i, StorageState::ReadWrite));
        }

        let nodeset: Vec<_> = (1..=5).map(PlainNodeId::new).collect();
    ...
    fn test_replication_checker_mixed() -> Result<()> {
        ...
        let nodeset: Vec<_> = (1..=6).map(PlainNodeId::new).collect();
        let replication = ReplicationProperty::new(3.try_into().unwrap());
        let mut checker: NodeSetChecker<bool> =
            NodeSetChecker::new(&nodeset, &nodes_config, &replication);
    ...
    fn test_replication_single_copy_single_node() -> Result<()> {
        ...
        let mut checker: NodeSetChecker<bool> =
            NodeSetChecker::new(&[PlainNodeId::new(1)], &nodes_config, &replication);
