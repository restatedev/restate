...
/// which means it has lost data and it cannot be considered a reliable participent in some quorum
...
impl<Attr: Eq + Hash + Clone + std::fmt::Debug> NodeSetChecker<Attr> {
        ...
        Predicate: Fn(&Attr) -> bool,
    {
        ...
        // A shortcut to get the number of authoriative nodes that have this attribute matching
        ...
        Predicate: Fn(&Attr) -> bool,
    {
        ...
        for scope_state in self.scopes.values() {
            ...
            // );
            if total > 0 && auth_domains + non_auth_domains >= f_majority_needed {
                f_majority = true;
    ...
    fn add_node(
        ...
        for (scope, scope_state) in self.scopes.iter_mut() {
            ...
            let fd = scope_state
                ...
                .or_insert_with(|| Arc::new(RwLock::new(FailureDomainState::default())));
            fd.write().increment();
            scope_state.node_to_fd.insert(node_id, HashWeakFd::new(fd));
        }
    }
    ...
    fn rm_attr_internal(&mut self, node_id: PlainNodeId, attr: &Attr, storage_state: StorageState) {
        ...
        for scope_state in self.scopes.values_mut() {
            let Some(weak_fd) = scope_state.node_to_fd.get(&node_id) else {
                // the node's location isn't defined at this scope
                continue;
            };
            let fd = weak_fd
                .upgrade()
                .expect("nodeset checker internal state must be consistent");
            let mut fd_mut = fd.write();
            // this was an authoritative node, if the domain was complete on this attribute, removing
            // this attribute from this node will render it incomplete.
            if storage_state.is_authoritative()
                && fd_mut.is_domain_complete(attr)
                && scope_state
                    .num_complete_domains
                    .get_mut(attr)
                    .is_some_and(|x| {
                        // it's not a complete domain any more, remove from the completed
                        // cache
                        *x -= 1;
                        *x == 0
                    })
            {
                scope_state.num_complete_domains.remove(attr);
            }

            let Some(counters) = fd_mut.per_attribute_counter.get_mut(attr) else {
                continue;
            };
            ...
            if counters.is_empty() {
                ...
                let is_replication_set_empty = if let Some(replication_set) = replication_set {
                    replication_set.remove(weak_fd);
                ...
                fd_mut.per_attribute_counter.remove(attr);
            }
        }
    }
    ...
    fn add_attr_internal(&mut self, node_id: PlainNodeId, attr: Attr, storage_state: StorageState) {
        // Apply this attribute on every scope
        for scope_state in self.scopes.values_mut() {
            let Some(fd) = scope_state.node_to_fd.get(&node_id) else {
                // the node's location isn't defined at this scope
                continue;
            };
            let fd = fd
            ...
            if counters.num_readable_nodes == 1 {
                let replicate_fds = scope_state
                    .replication_fds
                    .entry(attr.clone())
                    .or_insert_with(|| HashSet::with_hasher(Xxh3Builder::new()));
                replicate_fds.insert(HashWeakFd::new(&fd));
            }

            if storage_state.is_authoritative() {
                debug_assert!(counters.num_authoritative_nodes <= counters.num_readable_nodes);
                debug_assert!(counters.num_authoritative_nodes <= fd_mut.num_nodes);
                if fd_mut.is_domain_complete(&attr) {
                    *scope_state
                        .num_complete_domains
                        .entry(attr.clone())
                        .or_default() += 1;
...
struct FailureDomainState<Attr> {
    /// number of nodes in this this domain
    num_nodes: u32,
...
impl<Attr: Eq + Hash> FailureDomainState<Attr> {
    ...
    /// A domain is considered complete when it contains at least one authoritative node,
    /// with all authoritative nodes holding this attribute and the rest of the nodes in
    /// the same domain are known to be empty.
    ///
    /// An all-empty domain (a domain with no authoritative nodes) is always considered incomplete.
    fn is_domain_complete(&self, attr: &Attr) -> bool {
        self.per_attribute_counter
            .get(attr)
            .map(|counters| counters.num_authoritative_nodes > 0)
            .unwrap_or(false)
    }
...
/// A wrapper type that to implement Hash and Eq on Weak<RwLock<..>>, equality is based on the
/// underlying allocation pointer.
#[derive(derive_more::Deref, derive_more::DerefMut, Debug)]
struct HashWeakFd<Attr>(Weak<RwLock<FailureDomainState<Attr>>>);

impl<Attr> Hash for HashWeakFd<Attr> {
    fn hash<H: Hasher>(&self, hasher: &mut H) {
        self.0.as_ptr().hash(hasher)
    }
}

impl<Attr> Eq for HashWeakFd<Attr> {}
impl<Attr> PartialEq for HashWeakFd<Attr> {
    fn eq(&self, other: &Self) -> bool {
        Weak::ptr_eq(&self.0, &other.0)
    }
}

impl<Attr> HashWeakFd<Attr> {
    fn new(value: &Arc<RwLock<FailureDomainState<Attr>>>) -> Self {
...
struct LocationScopeState<Attr> {
    ...
    failure_domains: HashMap<SmartString, Arc<RwLock<FailureDomainState<Attr>>>, Xxh3Builder>,
    /// maps between node-id to the domain it belongs to for fast lookups
    node_to_fd: HashMap<PlainNodeId, HashWeakFd<Attr>, Xxh3Builder>,
    ...
    replication_fds: HashMap<Attr, HashSet<HashWeakFd<Attr>, Xxh3Builder>>,
}
...
impl<Attr: Eq + Hash> LocationScopeState<Attr> {
    fn new(replication: u8) -> Self {
        Self {
            ...
            num_complete_domains: HashMap::default(),
...
mod tests {
    ...
    fn nodeset_checker_fd_basics() -> Result<()> {
        ...
        // all nodes are false by default. Can't establish write quorum.
        assert_that!(checker.check_write_quorum(|attr| *attr), eq(false));
        checker.set_attribute_on_each([1, 2, 10], true);
