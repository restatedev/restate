...
use crate::Result;
use crate::{ErasedInputRecord, Record};
...
impl MemoryLoglet {
    ...
    fn read_from(
        ...
        if from_offset > commit_offset {
            ...
            // todo(asoli): pass key as key extractor
            let erased_record = guard.get(index).expect("reading untrimmed data").clone();
            // todo(asoli). ABSOLUTELY TEMPORARY
            //
            let mut buf = BytesMut::new();
            let value_buf =
                StorageCodec::encode_and_split(&*erased_record.body, &mut buf).expect("infallible");
            Ok(Some(LogEntry::new_data(
                from_offset,
                Record::from_parts(erased_record.header, value_buf),
            )))
        }
    }
}
