...
async fn append_records_multi_log(bifrost: Bifrost, log_id_range: Range<u64>, count_per_log: u64) {
    // for every log there is N streams. total count_per_log / streams_per_log gives the number of
    // writes per stream per log.
...
fn write_throughput_local_loglet(c: &mut Criterion) {
    ...
    for count_per_run in [1, 10, 100, 1000].iter() {
        group
            .sample_size(10)
            .throughput(Throughput::Elements(*count_per_run))
            .bench_with_input(
