...
use restate_types::net::codec::serialize_message;
...
fn serde_one_record(payloads: Arc<[Record]>) -> anyhow::Result<Message> {
...
fn serde_replicated_loglet_records(c: &mut Criterion) {
    let mut group = c.benchmark_group("ReplicatedLoglet");
    ...
    group
        ...
        .bench_function("serialization", |bencher| {
            bencher.iter(|| {
                let mut buf = BytesMut::new();
                let message = black_box(serde_one_record(payloads.clone()).unwrap());
...
criterion_group!(
    ...
    targets = serde_replicated_loglet_records
);
