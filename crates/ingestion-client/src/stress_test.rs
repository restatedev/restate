...
/// Maximum number of un-resolved `RecordCommit`s a producer keeps outstanding before draining.
const MAX_INFLIGHT_COMMITS: usize = 8192;
...
async fn produce(
    ...
    for seq in 1..=count {
        ...
        let commit = client
            ...
            .await
            .expect("ingest accepts the record");
        pending.push(commit);

        if pending.len() >= MAX_INFLIGHT_COMMITS {
            pending
                .next()
                .await
