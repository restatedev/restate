...
impl ShardPlan {
    ...
    pub const fn new(range: KeyRange, max_shards: u16) -> Self {
    ...
    ///
    /// # Panics
    /// Panics in debug mode if `idx` is outside the range of shards.
    pub const fn find_shard_unchecked(&self, idx: ShardIdx) -> Shard {
        ...
        // adding num_shards - 1 to dividend is equivalent to applying ceil function to result
    ...
    pub fn split(&self, new_max_shards: u16) -> Split<'_> {
...
mod tests {
    ...
    fn shards_iterator_is_exact_size() {
        ...
        let plan = ShardPlan::new(KeyRange::from(100..=104), 4);
    ...
    #[test]
    fn small_range() {
        let plan = ShardPlan::new(KeyRange::from(0..=9), 1024);
    ...
    #[test]
    fn single_key() {
        let plan = ShardPlan::new(KeyRange::from(55..=55), 1024);
    ...
    fn larger_equal_splits() {
        let plan = ShardPlan::new(KeyRange::from(100..=199), 4); // span=100 => base=25, rem=0
    ...
    #[test]
    fn clamps_to_1024() {
        let plan = ShardPlan::new(KeyRange::from(0..=1_000_000), 1024);
    ...
    fn full_partition_key_range() {
        let plan = ShardPlan::new(KeyRange::FULL, 1024);
        assert_eq!(plan.shard_count(), 1024);
        // Edge keys map
    ...
    fn example_100_199_split() {
        ...
        let old = ShardPlan::new(KeyRange::from(100..=199), 4);
        assert_eq!(
    ...
    fn split_tight_range() {
        let old = ShardPlan::new(KeyRange::from(100..=104), 4);
    ...
    fn split_single_key() {
        ...
        let old = ShardPlan::new(KeyRange::from(42..=42), 4);
    ...
    fn split_shrinking_to_fewer_shards() {
        ...
        let old = ShardPlan::new(KeyRange::from(0..=99), 8);
        let split = old.split(2);
