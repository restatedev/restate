...
impl EqualSizedPartitionPartitioner {
    /// Creates a partitioner for the full key space.
    ///
    /// `num_partitions == 0` yields an empty iterator.
    pub fn new(num_partitions: u16) -> Self {
        Self {
            plan: NonZeroU16::new(num_partitions)
                .map(|num_partitions| ShardPlan::new(KeyRange::FULL, num_partitions)),
        }
    }
}
...
impl Iterator for EqualSizedPartitions {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.next < plan.shard_count() {
            let idx = self.next;
            self.next += 1;
            let range = *plan.find_shard_unchecked(idx).key_range();
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        ...
        let remaining = (plan.shard_count() - self.next) as usize;
...
mod tests {
    ...
    #[test]
    fn zero_partitions_is_empty() {
        let partitioner = EqualSizedPartitionPartitioner::new(0);
        assert_eq!(partitioner.into_iter().count(), 0);
    }
}
