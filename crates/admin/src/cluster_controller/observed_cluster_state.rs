...
pub struct ObservedClusterState {
    ...
    // lock. However, since ObservedClusterState is hold across await points, this would prevent
...
impl ObservedClusterState {
    ...
    /// Returns the generational node id if the given node_id is alive.
    pub fn is_node_alive(&self, node_id: impl Into<NodeId>) -> Option<GenerationalNodeId> {
