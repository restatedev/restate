...
use restate_types::replication::balanced_spread_selector::{
    BalancedSpreadSelector, SelectorOptions, worker_candidate_filter,
};
...
impl PartitionState {
    ...
    fn max_partition_configuration_version(&self) -> Version {
        self.next
            .as_ref()
        ...
        if let Some(leader) = &self.leader {
                ...
                .is_some_and(|state| state.run_mode == RunMode::Leader)
            {
                commands.entry(*leader).or_default().push(ControlProcessor {
                    ...
                    partition_configuration_version: self.max_partition_configuration_version(),
                });
            }
        }
                        ...
                        let preferred_nodes = entry
                            ...
                            .filter(|replica| {
                                nodes_config
                                    ...
                                    .map(|node_config| {
                                        worker_candidate_filter(**replica, node_config)
                                    })
                                    .unwrap_or_default()
                            })
                            .copied()
