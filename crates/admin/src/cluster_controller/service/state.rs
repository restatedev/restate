...
fn compute_new_safe_trim_points(
    ...
    for (partition_id, processor_status) in partition_statuses.into_iter() {
        ...
        let max_archived_lsn = processor_status
            .values()
...
mod tests {
    ...
    fn test_compute_new_safe_trim_points() {
        ...
        current_trim_points.insert(LogId::from(partition_1), Lsn::INVALID);
        current_trim_points.insert(LogId::from(partition_2), Lsn::INVALID);

        let trim_points =
            compute_new_safe_trim_points(Arc::new(cluster_state), current_trim_points);
    ...
    fn leader_partition() -> PartitionProcessorStatus {
        PartitionProcessorStatus {
            ...
            last_archived_log_lsn: Some(Lsn::new(123)),
