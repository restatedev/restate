...
impl<T: TransportConnect> Scheduler<T> {
    ...
    fn note_observed_membership_update(
        ...
        // If current is not valid then the partition state does not contain any relevant membership
        // information.
        if !partition_state.current.is_valid() {
            return;
        }
    ...
    async fn ensure_valid_partition_configuration(
        ...
        for partition_id in partition_table.iter_ids().copied() {
            ...
            let mut occupied_entry = match entry {
                ...
                Entry::Occupied(entry) if entry.get().placement_policy.is_frozen() => entry,
                entry => {
                    let partition_replication = partition_table.replication_property(nodes_config);
            ...
            .read_modify_write(
                ...
                |epoch_metadata: Option<EpochMetadata>| {
                    if let Some(epoch_metadata) = epoch_metadata {
                        // check whether someone else stored an initial current partition configuration or whether it's frozen
                        if epoch_metadata.placement_policy().is_frozen()
                            || epoch_metadata.current().is_valid()
                        {
                            let (_, _, current, next, leadership_policy, placement_policy) =
                                epoch_metadata.into_inner();
...
mod tests {
    ...
    #[tokio::test]
    async fn persisted_freeze_blocks_initial_configuration_but_not_completion() {
        let metadata_store_client = MetadataStoreClient::new_in_memory();
        let policy = PlacementPolicy {
            freeze: Some(PlacementFreeze {
                reason: "maintenance".to_owned(),
        ...
        Scheduler::<FailingConnector>::note_observed_membership_update(
            partition_id,
            &state,
            &replica_set_states,
        );
        assert!(replica_set_states.partition_versions().is_empty());
        ...
        let stored = metadata_store_client
            ...
            .unwrap()
            .unwrap();
        assert_eq!(stored.version(), frozen.version());
        assert!(!stored.current().is_valid());

        let unfrozen = stored.set_placement_policy(PlacementPolicy::default());
        metadata_store_client
            .put(
                partition_processor_epoch_key(partition_id),
                &unfrozen,
                Precondition::MatchesVersion(frozen.version()),
            )
            .await
            .unwrap();
        let state = Scheduler::<FailingConnector>::store_initial_partition_configuration(
        ...
        assert!(!state.placement_policy.is_frozen());

        let partition_id = PartitionId::new_unchecked(1);
        let frozen = EpochMetadata::new(PartitionConfiguration::default(), Some(configuration(2)))
            .set_placement_policy(policy.clone());
        let (_, _, current, next, leadership_policy, placement_policy) =
        ...
        assert!(
            Scheduler::<FailingConnector>::should_complete_reconfiguration(
                partition_id,
                &NodesConfiguration::new_for_testing(),
                &state,
