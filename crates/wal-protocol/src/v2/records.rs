...
impl HasRecordKeys for TruncateOutboxRequest {
    fn record_keys(&self) -> Keys {
...
pub struct Effect(Box<restate_worker_api::invoker::Effect>);
...
pub struct ProxyThroughRequest {
    pub invocation: ServiceInvocation,

    pub proxy_partition: Keys,
}

flexbuffers_storage_encode_decode!(ProxyThroughRequest);

impl HasRecordKeys for ProxyThroughRequest {
    fn record_keys(&self) -> Keys {
