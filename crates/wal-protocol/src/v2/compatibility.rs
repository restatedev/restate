...
use crate::{
    ...
    v2::{
        self, Record,
        records::{ProxyThroughPayload, TruncateOutboxPayload},
    },
};
    ...
    fn try_from(value: v1::Envelope) -> Result<Self, Self::Error> {
        let source = match value.header.source {
            ...
            v1::Source::Processor {
                ...
                partition_key: _,
                leader_epoch,
            } => v2::Source::Processor { leader_epoch },
        ...
        let envelope = match value.command {
            v1::Command::AnnounceLeader(payload) => {
                records::AnnounceLeader::envelope(source, dedup, *payload).into_raw()
            }
            v1::Command::AttachInvocation(payload) => {
                records::AttachInvocation::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::InvocationResponse(payload) => {
                records::InvocationResponse::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::Invoke(payload) => {
                records::Invoke::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::InvokerEffect(payload) => {
                records::InvokerEffect::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::NotifyGetInvocationOutputResponse(payload) => {
                records::NotifyGetInvocationOutputResponse::envelope(source, dedup, payload)
                    .into_raw()
            }
            v1::Command::NotifySignal(payload) => {
                records::NotifySignal::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::PatchState(payload) => {
                records::PatchState::envelope(source, dedup, payload).into_raw()
            }
            ...
            v1::Command::PurgeInvocation(payload) => {
                records::PurgeInvocation::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::PurgeJournal(payload) => {
                records::PurgeJournal::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::RestartAsNewInvocation(payload) => {
                records::RestartAsNewInvocation::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::ResumeInvocation(payload) => {
                records::ResumeInvocation::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::ScheduleTimer(payload) => {
                records::ScheduleTimer::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::TerminateInvocation(payload) => {
                records::TerminateInvocation::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::Timer(payload) => {
                records::Timer::envelope(source, dedup, payload).into_raw()
            }
            ...
            v1::Command::UpdatePartitionDurability(payload) => {
                records::UpdatePartitionDurability::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::UpsertSchema(payload) => {
                records::UpsertSchema::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::VersionBarrier(payload) => {
                records::VersionBarrier::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::VQSchedulerDecisions(payload) => {
                ...
                records::VQSchedulerDecisions::envelope(source, dedup, payload).into_raw()
            }
            v1::Command::UpsertRuleBook(payload) => {
                records::UpsertRuleBook::envelope(source, dedup, payload).into_raw()
            }
        };
    ...
    fn from(value: v2::Dedup) -> Self {
        match value {
            ...
            v2::Dedup::Arbitrary {
                ...
                #[allow(deprecated)]
                Some(DedupInformation::ingress(prefix, seq))
            }
            v2::Dedup::Arbitrary {
                prefix: None,
                producer_id,
...
impl TryFrom<v2::Envelope<records::VQSchedulerDecisions>> for v1::Envelope {
    type Error = StorageDecodeError;

    fn try_from(value: v2::Envelope<records::VQSchedulerDecisions>) -> Result<Self, Self::Error> {
        let (header, inner) = value.split()?;

        let header = v1::Header::from((inner.record_keys(), header));

        // Hopefully this is short lived transformation (from v2 to v1) then we
        // will stop creating V1 type anymore
        Ok(Self {
            header,
            command: v1::Command::VQSchedulerDecisions(inner.bilrost_encode_to_bytes()),
        })
    }
}
