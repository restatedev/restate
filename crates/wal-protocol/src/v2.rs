...
impl PartialRecord {
    ...
    pub fn build(self, source: Source, dedup: Dedup) -> BodyWithKeys<Envelope<Raw>> {
        let inner = Envelope {
            header: Header {
                source,
                dedup,
...
pub trait Record: sealed::Sealed + Send + Sync + Clone + Copy + Sized {
    ...
    fn envelope(source: Source, dedup: Dedup, payload: impl Into<Self::Payload>) -> Envelope<Self> {
    ...
    fn test_envelope(payload: impl Into<Self::Payload>) -> Envelope<Raw> {
        let record = Self::envelope(Source::Ingress, Dedup::None, payload);
        record.into_raw()
    }
}
...
mod test {
    ...
    fn partial_envelope_with_keys() {
        ...
        assert_eq!(
            keyed.keys(),
            &Keys::RangeInclusive(payload.partition_key_range.clone().into())
        );
