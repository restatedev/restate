...
impl IncomingEnvelope<Raw> {
    ...
    /// Build an incoming envelope from parts. This is useful in case of converting a V1 envelope
    /// into V2.
    #[allow(dead_code)]
    pub(crate) fn from_parts<M: Record>(source: Source, dedup: Dedup, payload: M::Payload) -> Self {
        Self {
            header: Header {
                source,
                dedup,
...
mod test {
    ...
    #[test]
    fn incoming_envelope_from_parts() {
        let payload = AnnounceLeader {
            leader_epoch: 11.into(),
            node_id: GenerationalNodeId::new(1, 3),
            partition_key_range: 0..=u64::MAX,
        ...
        let envelope = IncomingEnvelope::from_parts::<records::AnnounceLeader>(
            Source::Ingress,
            Dedup::None,
            payload.clone(),
        );
