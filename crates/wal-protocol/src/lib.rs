...
impl HasRecordKeys for Envelope {
    fn record_keys(&self) -> logs::Keys {
        ...
        match &self.command {
            ...
            // todo: Remove or pass the partition key range
            Command::TruncateOutbox(_) => Keys::Single(self.partition_key()),
            Command::ProxyThrough(invocation) => Keys::Single(invocation.partition_key()),
            Command::AttachInvocation(attach) => Keys::Single(attach.partition_key()),
