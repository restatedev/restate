    ...
    Schemas: InvocationTargetResolver + Clone + Send + Sync + 'static,
{
                        ...
                        Ok(result) => {
                            ...
                            let (increment_bytes, state) = match result {
                                ProcessorResult::Continue(increment_bytes) => {
                                    // current window size mut not go above
                                    // max window size. This is possible because we
                                    // permit over size invocations (and negative window).
                                    // so we make sure to cap the current_window_size here

                                    let new_current = state
                                        .current_window_size
                                        .saturating_add(increment_bytes as i64)
                                        .min(self.max_window_size.get() as i64);

                                    // we are sure new_current is >= current, so this operation
                                    // is safe.
                                    let increment_bytes =
                                        (new_current - state.current_window_size) as u64;

                                    state.current_window_size = new_current;
    ...
    async fn wait_start(&mut self) -> Result<ProcessorState, Status> {
        let first = self
            ...
            .await
            .map_err(|_| Status::deadline_exceeded("Timeout waiting for Start message"))?
        ...
        Schemas: InvocationTargetResolver,
    {
        ...
        let limit_key = record
            .limit_key
            .as_deref()
            .or(settings.limit_key.as_deref())
            .map(parse_limit_key::<ReString>)
            .transpose()
            .map_err(|err| {
        ...
        let invocation_target = match target_meta.target_ty {
            InvocationTargetType::Service => {
                if record.key.as_deref().or(settings.key.as_deref()).is_some() {
                    return Err(Error::BadRequestWithOffset(
                        record.offset,
