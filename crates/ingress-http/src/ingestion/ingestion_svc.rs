...
//! The `IngestionSvc` gRPC service and its streaming ingestion logic.
//!
//! ## The `Ingest` stream
//!
...
pub mod proto {
    tonic::include_proto!("dev.restate.ingress.ingestion.v1");
}
...
{
    IngestionSvcServer::new(IngestionService::new(
        ingestion_client,
        schemas,
        max_window_size,
    ))
    ...
    Schemas: InvocationTargetResolver + Clone + Send + Sync + 'static,
{
        ...
        Schemas: InvocationTargetResolver,
    {
        ...
        if scope.is_none() && !limit_key.is_empty() {
            return Err(Error::BadRequestWithOffset(
                record.offset,
        ...
        {
            invocation.source = Source::Integration(state.integration.clone());
        }
