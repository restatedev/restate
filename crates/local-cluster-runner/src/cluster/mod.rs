...
impl<C, N> ClusterBuilder<(C, N, ())> {
    ...
    /// it, and use LOCAL_CLUSTER_RUNNER_DIR to set the base dir.
    pub fn temp_base_dir(self, dirname: &str) -> ClusterBuilder<(C, N, (MaybeTempDir,))> {
        let maybe_temp_dir = MaybeTempDir::new(&dirname);
...
impl MaybeTempDir {
    pub fn new<P: AsRef<Path>>(path: &P) -> MaybeTempDir {
        if let Ok("true" | "1") = std::env::var("LOCAL_CLUSTER_RUNNER_RETAIN_TEMPDIR").as_deref() {
            let dir = match std::env::var("LOCAL_CLUSTER_RUNNER_DIR") {
