...
/// Default combined seed size reserved from the memory pool per invocation
/// (split equally between inbound and outbound).
pub const DEFAULT_SEED_SIZE: usize = 64 * 1024;
...
pub struct InvokerCapacity {
    ...
    /// Combined inbound + outbound initial memory in bytes, split equally.
