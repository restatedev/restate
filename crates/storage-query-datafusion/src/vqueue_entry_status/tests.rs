...
async fn get_vqueue_entry_status_header_fields() {
    ...
    let records = engine
        .execute(format!(
            "SELECT entry_id, vqueue_id, stage, status, has_lock, next_run_at, sequence_number, \
    ...
    assert_that!(
        ...
        all!(row!(
            0,
            {
                ...
                "next_run_at" => TimestampMillisecondArray: eq(key.run_at().as_unix_millis().as_u64() as i64),
