...
fn duration(value: Duration) -> String {
    assert_eq!(value.subsec_nanos(), 0, "fixture duration must use seconds");
...
async fn query_sys_invocation_by_id_list() {
    ...
    let expected_row = |invocation: &InvocationFixture| {
    ...
    fixture
        .assert_query_unordered(
            &sql,
            vec![expected_row(invocation_1), expected_row(invocation_3)],
        )
        .await;
}
