...
use crate::filter::{FirstMatchingPartitionKeyExtractor, IdSelection, VQueueMetaFilter};
...
impl From<VQueueMetaFilter> for ScanMetaFilter {
    fn from(value: VQueueMetaFilter) -> Self {
        match value.ids {
            Some(IdSelection::Set(ids)) => ScanMetaFilter::MetaIdSet(ids),
            Some(IdSelection::Range { start, last }) => {
                ScanMetaFilter::MetaIdRange(std::range::RangeInclusive { start, last })
            }
