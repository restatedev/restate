...
pub(crate) fn append_lock_row(
    ...
    if row.is_invocation_id_defined()
        && let AcquiredBy::InvocationId(invocation_id) = state.acquired_by
    {
        row.fmt_invocation_id(invocation_id);
    }

    if row.is_acquired_by_other_defined()
        && let AcquiredBy::Other(other) = state.acquired_by
    {
        row.acquired_by_other(other);
    }
}
