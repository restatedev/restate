...
use crate::invocation::schema::{SysInvocationBuilder, SysInvocationRowBuilder};

#[inline]
pub(crate) fn append_invocation_row<'a>(
    builder: &mut SysInvocationBuilder,
    invocation_id: InvocationId,
    invocation_status: &'a InvocationStatusV2Lazy<'a>,
    status_report: Option<&InvocationStatusReport>,
    ...
    // Fill status_report fields
    if let Some(report) = status_report {
        fill_status_report(&mut row, report);
    }
    ...
    match invocation_status.inner.status() {
        Status::Scheduled => {
            fill_computed_status(&mut row, Status::Scheduled, status_report);
        ...
        Status::Inboxed => {
            fill_computed_status(&mut row, Status::Inboxed, status_report);
            if row.is_created_using_restate_version_defined() {
                row.created_using_restate_version(
                    invocation_status.created_using_restate_version()?,
                );
        ...
        Status::Invoked => {
            fill_computed_status(&mut row, Status::Invoked, status_report);
            fill_journal_metadata(&mut row, invocation_status)?;
            fill_in_flight_invocation_metadata(&mut row, invocation_status)?;
        }
        Status::Paused => {
            fill_computed_status(&mut row, Status::Paused, status_report);
            fill_journal_metadata(&mut row, invocation_status)?;
            fill_in_flight_invocation_metadata(&mut row, invocation_status)?;
        }
        Status::Suspended => {
            fill_computed_status(&mut row, Status::Suspended, status_report);
        ...
        Status::Completed => {
            fill_computed_status(&mut row, Status::Completed, status_report);
            fill_journal_metadata(&mut row, invocation_status)?;
...
fn fill_computed_status(
    row: &mut SysInvocationRowBuilder,
    status: restate_storage_api::protobuf_types::v1::invocation_status_v2::Status,
    status_report: Option<&InvocationStatusReport>,
) {
    use restate_storage_api::protobuf_types::v1::invocation_status_v2::Status;
    ...
    row.status(match status {
        ...
        Status::Invoked => match status_report {
            Some(report) if report.in_flight() => "running",
            Some(report) if report.retry_count() > 0 => "backing-off",
            _ => "ready",
        },
...
fn fill_status_report(row: &mut SysInvocationRowBuilder, report: &InvocationStatusReport) {
    if row.is_retry_count_defined() {
        row.retry_count(report.retry_count() as u64);
    }
    if row.is_last_start_at_defined() {
        row.last_start_at(MillisSinceEpoch::as_u64(&report.last_start_at().into()) as i64);
    }
    if row.is_last_attempt_deployment_id_defined()
        && let Some(last_attempt_deployment_id) = report.last_attempt_deployment_id()
    {
        row.last_attempt_deployment_id(last_attempt_deployment_id.to_string());
    }
    if row.is_last_attempt_server_defined()
        && let Some(last_attempt_server) = report.last_attempt_server()
    {
        row.last_attempt_server(last_attempt_server);
    }
    if row.is_next_retry_at_defined()
        && let Some(next_retry_at) = report.next_retry_at()
    {
        row.next_retry_at(MillisSinceEpoch::as_u64(&next_retry_at.into()) as i64);
    }
    if let Some(last_retry_attempt_failure) = report.last_retry_attempt_failure() {
        if row.is_last_failure_defined() {
            row.fmt_last_failure(&last_retry_attempt_failure.err);
        }
        if row.is_last_failure_error_code_defined()
            && let Some(doc_error_code) = last_retry_attempt_failure.doc_error_code
        {
            row.last_failure_error_code(doc_error_code.code())
        }

        match report.last_attempt_service_protocol_version() {
            None => {}
...
fn needs_invoked_by(row: &SysInvocationRowBuilder) -> bool {
...
#[inline]
fn fill_timestamps(row: &mut SysInvocationRowBuilder, status: &InvocationStatusV2Lazy) {
