        ...
        let physical_partitions: Vec<(PartitionId, Partition)> = self
            ...
            .flat_map(|(partition_id, partition)| {
                match &partition_key_selection {
                    ...
                    // filter re-derives every requested key in that span, so a batching scanner
                    // sees them together instead of one key at a time. Only extractors matching
                    // predicates the scanner can batch select this fanout.
