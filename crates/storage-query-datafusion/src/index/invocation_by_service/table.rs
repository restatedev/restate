...
pub(super) const NAME: &str = "_idx_invocation_by_service";
...
pub(crate) fn register_self(
    ...
    let table = PartitionedTableProvider::new(
        ...
        FirstMatchingPartitionKeyExtractor::partition_key(PointReadFanout::PerPartition)
            .with_grouped_invocation_id("invocation_id"),
    )
...
#[derive(Debug, Clone)]
pub(super) struct InvocationIndexScanner;

pub(super) struct InvocationIndexFilter {
    range: KeyRange,
    predicate: Filter<InvocationByService>,
}

impl ScanLocalPartitionFilter for InvocationIndexFilter {
...
impl ScanLocalPartition for InvocationIndexScanner {
    type Builder = IdxInvocationByServiceBuilder;
    type Item<'a> = (PartitionId, InvocationByServiceStageKey);
    type ConversionError = ConversionError;
    type Filter = InvocationIndexFilter;
            ...
            + 'static,
    {
        ...
        partition_store.scan_invocation_by_service(filter.range, &filter.predicate, move |key| {
