...
define_table!(
    /// Experimental inspection of persisted entry secondary-index records.
    /// Contains VQueue invocations and state mutations, including Finished entries until deletion.
    /// Entries predating index activation are not backfilled,
    /// so this table is not an authoritative inventory of all entries.
    _idx_entry_by_service(
        /// Physical partition containing the index entry.
        partition_id: DataType::UInt32,
        /// Name of the entry's service.
