...
async fn sql_index_scan_filters_projects_orders_and_tracks_lifecycle() {
    ...
    assert_ids(
        &engine,
        &format!("entry_id = '{}'", entries[6].as_base_entry_id()),
        &entries[6..],
    )
    .await;
    assert_ids(
    ...
    assert_eq!(
        projected[0]
            ...
            .unwrap()
            .value(0),
        entries[6].as_base_entry_id().to_string()
    );
    ...
    let mut tx = engine.partition_store().transaction();
    let before = tx
        .get_vqueue_entry_status(entries[0].as_base_entry_id())
        .await
        .unwrap()
        .unwrap();
...
async fn native_constraints_survive_remote_predicates_and_skip_unselected_keys() {
    ...
    EntryByServiceStageKey::prefix(partition, &mut poison).service_name("between");
    let mut tx = engine.partition_store().transaction();
    ...
    let mut poison = Vec::new();
    EntryByServiceStageKey::prefix(partition, &mut poison)
        .service_name("alpha")
        .stage(Stage::Inbox)
    ...
    for offset in 0..5 {
        ...
        for at in [first, last] {
            ...
            tx.update_secondary_index(
                None,
                Some(&EntryByServiceStageKey::borrowed(
                    "svc",
                    Stage::Inbox,
                    Reverse(at),
                    id,
