...
use restate_partition_store::index::EntryNextAtByStageKey;
...
impl ScanLocalPartition for EntryNextAtByStageScanner {
    ...
    type Item<'a> = (PartitionId, EntryNextAtByStageKey);
            ...
            + 'static,
    {
        ...
        partition_store.scan_entry_next_at_by_stage(
            filter.range,
            &filter.predicate,
            metrics,
            move |key| {
    ...
    fn append_row<'a>(
        ...
        row.next_at(key.next_at.as_unix_millis().as_u64() as i64);
