...
use super::schema::IdxEntryNextAtByVirtualObjectBuilder;
...
pub(crate) fn register_self(
    ...
    register::<EntryNextAtByVirtualObjectScanner>(
        ctx,
        selector,
        manager,
        remote,
...
impl ScanLocalPartition for EntryNextAtByVirtualObjectScanner {
    ...
    where
        F: for<'a> FnMut(Self::Item<'a>) -> ControlFlow<Result<(), StorageError>>
            ...
            + 'static,
    {
        store.scan_entry_next_at_by_virtual_object(filter.range, &filter.predicate, metrics, f)
    }
