...
async fn assert_ids(engine: &MockQueryEngine, predicate: &str, expected: &[CanonicalEntryId]) {
    let mut actual = ids(
        engine,
        &format!("SELECT canonical_id FROM _idx_entry_by_service WHERE {predicate}"),
    )
    .await;
    actual.sort();
...
async fn populate(engine: &mut MockQueryEngine) -> Vec<CanonicalEntryId> {
    ...
    ids.push(key.to_canonical_entry_id(qid.partition_key()));
...
async fn sql_index_scan_filters_projects_orders_and_tracks_lifecycle() {
    ...
    assert!(
        ids(&engine, "SELECT canonical_id FROM _idx_entry_by_service")
            .await
            .is_empty()
    );
    ...
    for (predicate, visits) in [
        ...
        ("stage = 'running' AND service_name = 'missing'", 0),
    ] {
        let batches = select(
            &engine,
            &format!(
    ...
    let count = select(&engine, "SELECT COUNT(*) AS n FROM _idx_entry_by_service").await;
    assert_eq!(
        count[0]
            .column(0)
            .as_any()
    ...
    assert_ids(
        ...
        &[
            entries[0], entries[1], entries[2], entries[3], entries[4], entries[6],
        ],
    )
    .await;
    assert_ids(
    ...
    assert_ids(
        ...
        &[entries[0], entries[1], entries[2], entries[4], entries[6]],
    )
    .await;
    assert_ids(
        &engine,
    ...
    assert_ids(
        &engine,
        &format!("canonical_id NOT IN ('{}', '{}')", entries[0], entries[1]),
        &entries[2..],
    )
    .await;
    // Millisecond equality must retain all distinct HLC values, including state mutations.
    assert_ids(
        &engine,
    ...
    .await;
    assert_ids(&engine, "canonical_id IS NULL OR entry_id IS NULL", &[]).await;
    assert_ids(
        &engine,
        &format!("entry_id = '{}'", entries[6].to_base_entry_id()),
        &entries[6..],
    )
    .await;
    assert_ids(
        &engine,
        &format!("canonical_id = '{}'", entries[6]),
        &entries[6..],
    )
    .await;
    assert_ids(
        &engine,
        &format!("canonical_id = '{}'", entries[6].with_seq(Seq::MAX)),
        &[],
    )
    .await;
    assert_ids(
    ...
    let above = entries
        ...
        .collect::<Vec<_>>();
    assert_ids(&engine, &format!("canonical_id > '{}'", entries[0]), &above).await;
    assert_eq!(ids(&engine, &format!("SELECT canonical_id FROM _idx_entry_by_service WHERE transitioned_at > to_timestamp_millis({}) LIMIT 1", MS + 3)).await, [entries[5].to_string()]);

    let ordered = ids(
        &engine,
        "SELECT canonical_id FROM _idx_entry_by_service ORDER BY transitioned_at DESC LIMIT 3",
    )
    .await;
    assert_eq!(
        ordered,
    ...
    let projected = select(&engine, "SELECT canonical_id, transitioned_at, service_name, entry_id FROM _idx_entry_by_service WHERE service_name = 'alpha' AND stage = 'inbox' ORDER BY transitioned_at DESC, canonical_id DESC LIMIT 1").await;
    assert_eq!(
        projected[0]
            .column(0)
            .as_any()
    ...
    assert_eq!(
        projected[0]
            ...
            .unwrap()
            .value(0),
        entries[6].to_base_entry_id().to_string()
    );
    assert_eq!(ids(&engine, "SELECT canonical_id FROM _idx_entry_by_service WHERE ends_with(service_name, 'a') AND stage = 'running' LIMIT 1").await, [entries[2].to_string()]);
    ...
    let joined = ids(&engine, "SELECT i.canonical_id FROM _idx_entry_by_service i JOIN sys_vqueue_entry_status s ON i.canonical_id = s.canonical_id WHERE i.stage = 'finished'").await;
    assert_eq!(joined, [entries[4].to_string()]);

    let mut tx = engine.partition_store().transaction();
    let before = tx
        .get_vqueue_entry_status(&entries[0].to_base_entry_id())
        .await
        .unwrap()
        .unwrap();
    ...
    assert_ids(
        &engine,
        "service_name = 'alpha'",
        &[entries[1], entries[2], entries[6]],
    )
    .await;
}

#[restate_core::test(flavor = "multi_thread", worker_threads = 2)]
async fn lazy_index_views_decode_only_projected_values_and_validate_key_structure() {
    let mut engine = MockQueryEngine::create().await;
    let partition = engine.partition_store().partition_id();
    let id = BaseEntryId::from(InvocationId::from_parts(100, InvocationUuid::from_u128(1)))
        .canonicalize(Seq::new(1));
    let at = timestamp(MS, 0);

    for field in ["service_name", "stage", "transitioned_at"] {
        let mut key = Vec::new();
        EntryByStageServiceKey::prefix(partition, &mut key);
        (if field == "stage" {
            "future-stage"
        } else {
            "inbox"
        })
        .encode_field(&mut key);
        (if field == "service_name" { "" } else { "svc" }).encode_field(&mut key);
        // A reversed all-zero timestamp decodes to the invalid u64::MAX HLC.
        (if field == "transitioned_at" {
            0
        } else {
            !at.as_u64()
        })
        .encode_field(&mut key);
        id.encode_field(&mut key);
        let mut tx = engine.partition_store().transaction();
        tx.raw_put_cf(KeyKind::SecondaryIndex, &key, []);
        tx.commit().await.unwrap();
        drop(tx);

        assert_eq!(
            ids(&engine, "SELECT canonical_id FROM _idx_entry_by_service").await,
            [id.to_string()]
        );
        let batches = select(&engine, "SELECT COUNT(*) AS n FROM _idx_entry_by_service").await;
        assert_eq!(
            batches[0]
                .column(0)
                .as_any()
        ...
        let result = engine
            .execute(&format!("SELECT {field} FROM _idx_entry_by_service"))
            .await;
        let failed = match result {
            Err(_) => true,
            Ok(result) => result.stream.try_collect::<Vec<_>>().await.is_err(),
        };
        assert!(failed, "projecting invalid {field} must fail");

        let mut tx = engine.partition_store().transaction();
        tx.raw_delete_cf(KeyKind::SecondaryIndex, &key);
        tx.commit().await.unwrap();
    }

    // Even a projection with no columns must reject truncated and trailing fields.
    let mut valid = Vec::new();
    EntryByStageServiceKey::prefix(partition, &mut valid)
        .stage(Stage::Inbox)
        .service_name("svc")
    ...
    trailing.push(0);
    for key in [&valid[..valid.len() - 1], trailing.as_slice()] {
        let mut tx = engine.partition_store().transaction();
        tx.raw_put_cf(KeyKind::SecondaryIndex, key, []);
        tx.commit().await.unwrap();
        drop(tx);
        let result = engine
            .execute("SELECT COUNT(*) FROM _idx_entry_by_service")
            .await;
        let failed = match result {
            Err(_) => true,
            Ok(result) => result.stream.try_collect::<Vec<_>>().await.is_err(),
        ...
        assert!(
            failed,
            "malformed key structure must fail without projected columns"
        );
        let mut tx = engine.partition_store().transaction();
        tx.raw_delete_cf(KeyKind::SecondaryIndex, key);
        tx.commit().await.unwrap();
...
async fn native_constraints_survive_remote_predicates_and_skip_unselected_keys() {
    ...
    EntryByStageServiceKey::prefix(partition, &mut poison)
        .stage(Stage::Inbox)
    ...
    assert_ids(
        ...
        &[entries[0], entries[1], entries[2], entries[5], entries[6]],
    )
    .await;
    ...
    let predicate = logical2physical(
        &col("service_name")
            ...
            .and(col("canonical_id").eq(lit(entries[1].to_string()))),
        &schema,
    );
    ...
    for predicate in [predicate, remote] {
        ...
        EntryIndexScanner::for_each_row(engine.partition_store(), filter, None, move |key| {
...
async fn millisecond_timestamp_ranges_and_in_lists_cover_whole_hlc_buckets() {
    ...
    for offset in 0..5 {
        ...
        for at in [first, last] {
            ...
            let id = BaseEntryId::from(id).canonicalize(Seq::new(entries.len() as u64));
...
async fn partition_tables_register_the_scanner_and_sort_across_stores() {
    ...
    for (index, (_, partition)) in partitions.0.iter().enumerate() {
        ...
        let id = BaseEntryId::from(id).canonicalize(Seq::new(index as u64));
    ...
    let query = format!(
        "SELECT COUNT(*) FROM _idx_entry_by_service WHERE canonical_id IN ('{}', '{}', '{}')",
