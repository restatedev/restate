...
async fn stage_tables_project_filter_and_follow_lifecycle_updates() {
    ...
    for (table, time) in TABLES {
        ...
        assert_ids(
            &engine,
            table,
            &format!("entry_id = '{}'", ids[1].as_base_entry_id()),
            &ids[1..2],
        )
        .await;
        ...
        assert_eq!(
            ...
            [ids[1].as_base_entry_id().to_string()]
        );
    ...
    let mut tx = engine.partition_store().transaction();
    let header = tx
        .get_vqueue_entry_status(ids[0].as_base_entry_id())
        .await
        .unwrap()
        .unwrap();
...
async fn stage_tables_register_for_partition_queries_and_sort_across_stores() {
    ...
    for (table, time) in TABLES {
        ...
        for (column, expected) in [
            ...
            ("entry_id", ids[1].as_base_entry_id().to_string()),
        ] {
