...
/// Bound on in-flight stage scans per partition scan. Keeps RocksDB iterator
/// pressure and the shared builder's `Mutex` contention in check while still
/// overlapping I/O across stages.
const MAX_CONCURRENT_STAGE_SCANS: usize = 2;
    ...
    >(
        ...
        Ok(async move {
            let scan_result = async {
                for stage_batch in stages.chunks(MAX_CONCURRENT_STAGE_SCANS) {
                    if should_stop.load(Ordering::Relaxed) {
                        break;
                    }

                    let mut scans = Vec::with_capacity(stage_batch.len());
                    for stage in stage_batch {
                        let callback = callback.clone();
                        let should_stop = should_stop.clone();

                        scans.push(partition_store.for_each_vqueue_inbox_entry(
                            range,
                            *stage,
                            move |item| {
                                if should_stop.load(Ordering::Relaxed) {
