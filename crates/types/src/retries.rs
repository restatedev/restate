...
impl WaitDuration {
    ...
    pub fn sub(mut self, duration: Duration) -> Self {
        match self {
...
impl RetryIter<'_> {
    ...
    ///
    /// This will return `None` if this retry policy does not have a maximum number of attempts
    /// (infinite), or `Duration::ZERO` if no remaining retries are available.
