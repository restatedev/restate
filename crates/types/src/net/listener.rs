...
pub enum ListenError {
    ...
    #[error("[{service_name}] failed binding on unix-socket path '{uds_path}': {source}")]
    #[code(restate_errors::RT0004)]
    UdsBinding {
        service_name: String,
        uds_path: PathBuf,
...
impl AddressBook {
    ...
    async fn bind<P: ListenerPort + 'static>(
        &mut self,
        listener_options: &ListenerOptions<P>,
        listenfd: &mut ListenFd,
    ) -> Result<(), ListenError> {
        // Note: always attempt to listen on tcp first.
    ...
    async fn bind_uds<P: ListenerPort + 'static>(
        ...
        let listener = UnixListener::bind(uds_path).map_err(|err| ListenError::UdsBinding {
            service_name: P::NAME.to_owned(),
...
#[allow(dead_code)]
pub struct Addresses<P: ListenerPort + 'static> {
    tcp_bind_address: Option<SocketAddr>,
    uds_path: Option<PathBuf>,
...
#[derive(Default)]
pub struct Listeners<P: ListenerPort + 'static> {
    tcp_listener: Option<TcpListener>,
    unix_listener: Option<UnixListener>,
    _phantom: std::marker::PhantomData<P>,
}

impl<P: ListenerPort + 'static> Listeners<P> {
...
impl<P: ListenerPort + 'static> Drop for Listeners<P> {
    fn drop(&mut self) {
        if let Some(listener) = self.unix_listener.take()
            && let Ok(addr) = listener.local_addr()
            && let Some(path) = addr.as_pathname()
