...
impl ListenerPort for HttpIngressPort {
    ...
    const UDS_NAME: &'static str = "http_ingress.sock";
...
/// gRPC port for node-level control and introspection
#[derive(Debug, Clone, Default, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[cfg_attr(feature = "schemars", derive(schemars::JsonSchema))]
pub struct NodeCtlPort;
impl ListenerPort for NodeCtlPort {
    const NAME: &'static str = "node-rpc-server";
    // todo change to 5123 once we open the network server on this port.
    // const DEFAULT_PORT: u16 = 5123;
    const DEFAULT_PORT: u16 = 5122;
    const UDS_NAME: &'static str = "node.sock";
    const IS_ANONYMOUS_UDS_ALLOWED: bool = true;
}
impl GrpcPort for NodeCtlPort {}
...
impl ListenerPort for TokioConsolePort {
    ...
    const UDS_NAME: &'static str = "tokio_console.sock";
    const IS_ANONYMOUS_UDS_ALLOWED: bool = false;
}
...
mod tests {
    ...
    fn test_parse_bind_address_with_default_port() {
        let input = "127.0.0.1";
        let result = match input.parse::<BindAddress<NodeCtlPort>>() {
            Ok(addr) => addr.inner,
    ...
    fn test_parse_bind_address_invalid() {
        ...
        let result = input.parse::<BindAddress<NodeCtlPort>>();
        assert!(
            result.is_err(),
    ...
    fn test_parse_advertised_address_host_port() {
        ...
        let input = "localhost";
        let result = input.parse::<AdvertisedAddress<NodeCtlPort>>().unwrap();
