...
pub struct GetDigest {
    ...
    pub from_offset: LogletOffset,
    // exclusive
    pub until_offset: LogletOffset,
}

#[derive(Debug, Clone, PartialEq, Eq, derive_more::Display, Serialize, Deserialize)]
#[display("[{from_offset}..{until_offset}) -> {status} ({})",  self.len())]
...
impl DigestEntry {
    ...
    pub fn len(&self) -> usize {
        debug_assert!(self.until_offset >= self.from_offset);
        (*self.until_offset - *self.from_offset)
            .try_into()
            .expect("usize >= u32")
    }
}
