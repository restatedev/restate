...
const MAX_NODESET_SIZE: u32 = 128;

#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
pub enum NodeSelectorError {
    #[error("no enough candidate nodes to satisfy the replication factor; available={nodeset_len} nodes but replication={replication_factor}")]
    InsufficientNodes {
        nodeset_len: usize,
        replication_factor: u8,
    },
    #[error("node location for node {node_id} is missing. Node locations are required if topology-aware replication is enabled. Topology-aware replication is enabled due to replication-property set to {replication}")]
    NodeLocationMissing {
        node_id: PlainNodeId,
...
pub struct NodeSetSelectorOptions<'a> {
    ...
    /// is used to generate consistent rank of nodes that's different for every id. For instance,
    ...
    pub fn select(
        ...
        for (node_id, node_config) in nodes_config.iter() {
            ...
            let domain_path = if let NodeLocationScope::Root = replicate_across_scope {
                ...
                // nodes should have a location strings and they need to be defined at the widest
        ...
        let mut choose_domains = |writeable_only: bool| {
            ...
            while let Some(domain) = queue.pop() {
                ...
                if !writeable_only && output_nodeset.len() >= MAX_NODESET_SIZE as usize / 2 {
                    // We are stil selecting from all nodes but we are getting dangerously close to
...
struct Domain {
    /// the list of nodes sorted by hash, we pick nodes by poping the last item for O(1) removals
    candidates: Vec<CandidateNode>,
    /// a hash value that is used to order domains
    priority: u64,
    ...
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        match (self.inner.is_top_priority, other.inner.is_top_priority) {
            ...
            _ => {
                ...
                // at the top of the heap. if there is a tie, we order by the normal hash priority.
...
pub mod tests {
    ...
    fn selection_with_preferred_previous_nodeset() {
        ...
        // Now if we regenerate but assuming that enough nodes became unwriteable in nodeset1, we
        // even that we have nodeset1 as preferred, we should see enough writeable nodes in
        // nodeset2.
        // 4 nodes. 0 became Disabled (not candidates), and the rest became readonly
        for i in 0..5 {
            // three regions, 2 zones per region
            let region = i % 3;
            let zone = i % 2;
    ...
    fn selection_shuffling() {
        ...
        // since this is determinstic, we know that this run generates 11 unique leaders. A single
        // node in the cluster won't be a leader for any partition and that's a reasonable outcome.
        // albeit, balance of leaders is not guaranteed.
