...
use crate::net::partition_processor::{
    PartitionProcessorRpcError, PartitionProcessorRpcOkOf, PartitionProcessorRpcRequestHeader,
    PartitionProcessorRpcRequestInner, PartitionProcessorRpcResponse,
    PartitionProcessorWireResponse, PartitionProcessorWireRpc, UnexpectedResponse,
};
...
pub trait PartitionProcessorRpc: WithPartitionKey + Sized + Send + 'static {
    ...
    type Response: TryFrom<PartitionProcessorRpcOkOf<Self::Wire>, Error: Into<WireResponseError>>
        + Send;
    ...
    fn from_wire(
        ...
        response.into_result()?.try_into().map_err(Into::into)
    }
}
