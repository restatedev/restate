...
pub trait PartitionedResourceId {
        ...
        paste::paste! {
            impl $crate::identifiers::ResourceId for [< $res_name Id >] {
                ...
                fn push_to_encoder(&self, encoder: &mut $crate::id_util::IdEncoder<Self>) {
                    ...
                    encoder.push_u128(u128::from_be_bytes(self.rest));
                }
            }
            ...
            impl ::std::str::FromStr for [< $res_name Id >] {
                ...
                fn from_str(input: &str) -> ::std::result::Result<Self, Self::Err> {
                    ...
                    Ok(Self { partition_key, rest: raw_ulid.to_be_bytes() })
                }
            }
        ...
        paste::paste! {
            ...
            impl [< $res_name Id >] {
                ...
                pub fn generate(partition_key: $crate::PartitionKey) -> Self {
                    Self { partition_key, rest: ::ulid::Ulid::new().to_bytes() }
                }

                pub const fn from_parts(partition_key: $crate::PartitionKey, timestamp_ms: u64, random: u128) -> Self {
                    Self {
                        partition_key,
                        rest: ::ulid::Ulid::from_parts(timestamp_ms, random).to_bytes(),
                    }
                }

                pub const fn from_partition_and_bytes(partition_key: $crate::PartitionKey, rest: [u8; 16]) -> Self {
                    Self {
                        partition_key,
                        rest,
                    }
                ...
                pub fn from_slice(b: &[u8]) -> Result<Self, $crate::errors::IdDecodeError> {
                    ...
                    let rest = b[size_of::<$crate::PartitionKey>()..].try_into().map_err(|_| $crate::errors::IdDecodeError::Length)?;
                    Ok(Self{ partition_key, rest })
                }

                pub fn from_bytes(b: [u8; 24]) -> Self {
                    let partition_key = u64::from_be_bytes(b[..size_of::<$crate::PartitionKey>()].try_into().unwrap());
                    let rest = b[size_of::<$crate::PartitionKey>()..].try_into().unwrap();
                    Self{ partition_key, rest }
                }

                pub fn to_bytes(&self) -> [u8; 24] {
                    let mut buf = [0u8; 24];
                    buf[..size_of::<$crate::PartitionKey>()].copy_from_slice(&self.partition_key.to_be_bytes());
                    buf[size_of::<$crate::PartitionKey>()..].copy_from_slice(&self.rest);
                    buf
                }
            }
            ...
            impl $crate::identifiers::TimestampAwareId for [< $res_name Id >] {
                fn timestamp(&self) -> ::restate_clock::time::MillisSinceEpoch {
                    let ulid = ::ulid::Ulid::from_bytes(self.rest);
