    ...
    service_key
        ...
        .or_else(|| {
            idempotency_key.map(|idempotency_key| {
                if !is_unscoped_idempotent_service_bucketing() {
...
/// Number of partition keys each unscoped idempotent service can spread over.
///
/// Unlike [`UNSCOPED_SERVICE_PARTITION_KEY_FANOUT`], this value is effectively
...
fn is_unscoped_idempotent_service_bucketing() -> bool {
...
fn unscoped_idempotent_service_partition_key(
    ...
    let bucket = intermittent_key % UNSCOPED_IDEMPOTENT_SERVICE_PARTITION_KEY_FANOUT as u64;

    debug_assert!(bucket < UNSCOPED_IDEMPOTENT_SERVICE_PARTITION_KEY_FANOUT as u64);
