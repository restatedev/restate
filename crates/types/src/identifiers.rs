...
pub trait ResourceId {
    ...
    fn push_to_encoder(&self, encoder: &mut IdEncoder<Self>) -> std::io::Result<()>;
}
...
impl Display for InvocationUuid {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        let raw: u128 = self.0;
        let written = base62_encode_fixed_width_u128(raw, &mut buf).map_err(|_| fmt::Error)?;
...
impl Display for InvocationId {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        self.push_to_encoder(&mut encoder).map_err(|_| fmt::Error)?;
...
impl ResourceId for AwakeableIdentifier {
    ...
    fn push_to_encoder(&self, encoder: &mut IdEncoder<Self>) -> std::io::Result<()> {
        ...
        let written = restate_base64_util::URL_SAFE
            ...
            .map_err(|_| std::io::Error::other("base64 encoding error"))?;
