...
pub fn decode_serde<T: DeserializeOwned, B: Buf>(
    ...
    match codec {
        StorageCodecKind::FlexbuffersSerde => decode_serde_from_flexbuffers(buf, true)
            .map_err(|err| StorageDecodeError::DecodeValue(err.into())),
        StorageCodecKind::FlexbuffersNoPrefix => decode_serde_from_flexbuffers(buf, false)
            .map_err(|err| StorageDecodeError::DecodeValue(err.into())),
...
fn decode_serde_from_flexbuffers<T: DeserializeOwned, B: Buf>(
    ...
    if !length_prefix {
        // without length-prefix, we rely on the buffer being contiguous.
...
fn decode_serde_from_bincode<T: DeserializeOwned, B: Buf>(
    ...
    if buf.remaining() < length {
        return Err(bincode::error::DecodeError::custom(format!(
            "insufficient data: expecting {length} bytes for bincode"
        )));
    }
    ...
    if slice.len() >= length {
        ...
        let bytes = buf.copy_to_bytes(length);
        let (result, _length) =
            bincode::serde::decode_from_slice(bytes.chunk(), bincode::config::standard())?;
        Ok(result)
    }
}
