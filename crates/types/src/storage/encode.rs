...
pub fn encode_serde<T: Serialize>(
    ...
    match codec {
        StorageCodecKind::FlexbuffersSerde => encode_serde_as_flexbuffers(value, buf, true)
            .map_err(|err| StorageEncodeError::EncodeValue(err.into())),
        StorageCodecKind::FlexbuffersNoPrefix => encode_serde_as_flexbuffers(value, buf, false)
            .map_err(|err| StorageEncodeError::EncodeValue(err.into())),
...
fn encode_serde_as_flexbuffers<T: Serialize>(
    ...
    if length_prefix {
        let size_tag = u32::try_from(vec.len()).map_err(|_| {
            serde::ser::Error::custom("only support serializing types of size <= 4GB")
        })?;
