...
mod serde_impl {
    ...
    impl<S: StringLike> Serialize for LimitKey<S> {
        fn serialize<Ser: Serializer>(&self, serializer: Ser) -> Result<Ser::Ok, Ser::Error> {
            serializer.collect_str(self)
        }
    }

    impl<'de, S: OwnedStringLike> Deserialize<'de> for LimitKey<S> {
...
mod tests {
    ...
    #[test]
    fn serde_limit_key_roundtrip() {
        for input in ["", "hello", "hello/value"] {
            let key: LimitKey<String> = input.parse().unwrap();
            let json = serde_json::to_string(&key).unwrap();
            let decoded: LimitKey<String> = serde_json::from_str(&json).unwrap();
