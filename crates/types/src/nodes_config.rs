...
impl NodesConfiguration {
    ...
    /// is loaded yet, this is will be only the case before attaching to the controller
    /// node on startup.
    ///
    ...
    pub fn set_as_current_if_newer(self) -> ConfigVersion {
        let current = CURRENT_CONFIG.load_full();
        if current.is_none() {
            let after_update = CURRENT_CONFIG.compare_and_swap(&current, Some(Arc::new(self)));
            return after_update
                .as_ref()
                .map(|c| c.version)
                .unwrap_or(ConfigVersion::INVALID);
    ...
    pub fn set_as_current_unconditional(self) {
        CURRENT_CONFIG.store(Some(Arc::new(self)));
    }
...
mod tests {
    ...
    fn test_current_config() {
        NodesConfiguration::unset_current();
        ...
        config.clone().set_as_current_unconditional();
        ...
        let version = config.clone().set_as_current_if_newer();
        assert_eq!(ConfigVersion(2), version);
        assert_eq!(ConfigVersion(2), NodesConfiguration::current_version());

        // force
        config.set_as_current_unconditional();
