...
impl NodesConfiguration {
    ...
    pub fn set_as_current_if_newer(self) {
        let current = CURRENT_CONFIG.load_full();
        match current {
            None => {
                CURRENT_CONFIG.compare_and_swap(&current, Some(Arc::new(self)));
            }
            Some(current) if self.version > current.version => {
                CURRENT_CONFIG.compare_and_swap(&current, Some(Arc::new(self)));
            }
...
mod tests {
    ...
    fn test_current_config() {
        ...
        config.clone().set_as_current_if_newer();
        assert_eq!(ConfigVersion(2), NodesConfiguration::current_version());
