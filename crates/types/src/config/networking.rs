...
pub struct NetworkingOptions {
    ...
    /// back pressure. Data streams are used for sending medium-to-large messages between nodes.
    ///
    ...
    pub data_stream_window_size: NonZeroByteCount,

    /// # Control Stream Window Size
    ///
    /// Controls the number of bytes the can be sent on every control stream before inducing
    /// back pressure. Control streams are used for sending low-latency control messages between
    /// nodes. These messages are usually small and may benefit from a smaller window size.
    ///
    /// If network latency is high, it's recommended to keep this value small.
    /// Maximum theoretical value is 2^31-1 (2 GiB - 1), but we will sanitize this value to 500 MiB
    pub control_stream_window_size: NonZeroByteCount,
}
...
impl Default for NetworkingOptions {
    fn default() -> Self {
        Self {
            ...
            // 256KiB
            control_stream_window_size: NonZeroByteCount::new(
                NonZeroUsize::new(256 * 1024 * 1024).expect("Non zero number"),
            ),
        }
    }
}
