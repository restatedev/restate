...
pub fn compute_background_work_budgets(
    ...
    // `rocksdb-high-priority-bg-threads` to give the partition-store more flush parallelism.
    let high_pri_pool = rocksdb_high_priority_bg_threads.get();
    let metadata_flush_reserve = if has_metadata_server { 1 } else { 0 };
    // virtually a single CF + atomic_flush => at most 1 flush job in flight (change this to 2
    // if we stopped using atomic_flush for log-server)
    let log_server_flush_reserve = if has_log_server { 1 } else { 0 };
    ...
    // Non-starvation invariant: concurrent flush demand never exceeds the pool, so every
    // database always finds a free high-priority thread (unless the pool is configured below
    // the active reserves on a tiny node).
    debug_assert!(
        worker_flushes + log_server_flush_reserve + metadata_flush_reserve <= high_pri_pool,
        "flush budget over-commits the high-priority pool"
    );
