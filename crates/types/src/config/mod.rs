            ...
            match dirs.len().cmp(&1) {
                ...
                std::cmp::Ordering::Greater => {
                    return Err(InvalidConfigurationError::RequiredNodeName(
                        "Directory has multiple subdirectories".to_owned(),
                    ));
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_read_dir_subdirs_did_not_exist() {
        let temp_dir = tempfile::tempdir().unwrap();
        let temp_dir_path = temp_dir.path().to_path_buf();
        assert!(fs::remove_dir(temp_dir).is_ok());
        assert!(read_dir_subdirs(&temp_dir_path).is_empty());
    }

    #[test]
    fn test_read_dir_subdirs_empty() {
        let temp_dir = tempfile::tempdir().unwrap();
        let temp_dir_path = temp_dir.path().to_path_buf();
        assert!(read_dir_subdirs(&temp_dir_path).is_empty());
    }
    ...
    #[test]
    fn test_read_dir_subdirs_with_subdirs_and_files() {
        let temp_dir = tempfile::tempdir().unwrap();
        let temp_dir_path = temp_dir.path().to_path_buf();
        ...
        let mut sub_dirs = read_dir_subdirs(&temp_dir_path);
        sub_dirs.sort();
