...
use crate::config::{AwsLambdaOptions, HttpOptions, IngestionOptions};
...
impl InvokerOptions {
    ...
    pub(crate) fn merge(&mut self, common: &CommonOptions, opts: &NetworkingOptions) {
        #[allow(deprecated)]
        self.service_client
            .apply_deprecated("worker.invoker.service-client", &common.service_client);
...
impl ServiceClientOptions {
    ...
    pub fn apply_deprecated(&mut self, new_base: &str, other: &ServiceClientOptions) {
        self.http.apply_deprecated(new_base, &other.http);
        self.lambda.apply_deprecated(new_base, &other.lambda);
        ...
        apply!(additional_request_headers, "additional-request-headers");
    }
}
...
mod tests {
    ...
    fn apply_deprecated_precedence() {
        let base = "worker.invoker.service-client";
        ...
        let mut deprecated = ServiceClientOptions::default();
        deprecated.lambda.aws_profile = Some("old".to_owned());
        new.apply_deprecated(base, &deprecated);
        assert_eq!(new.lambda.aws_profile.as_deref(), Some("old"));

        // both set: the explicitly-set new value wins, deprecated is ignored
        let mut new = ServiceClientOptions::default();
        new.lambda.aws_profile = Some("new".to_owned());
        let mut deprecated = ServiceClientOptions::default();
        deprecated.lambda.aws_profile = Some("old".to_owned());
        new.apply_deprecated(base, &deprecated);
        assert_eq!(new.lambda.aws_profile.as_deref(), Some("new"));

        // regression guard: a new-only value must not be clobbered by the deprecated default
        let mut new = ServiceClientOptions::default();
        new.lambda.aws_profile = Some("new".to_owned());
        let deprecated = ServiceClientOptions::default();
        new.apply_deprecated(base, &deprecated);
        assert_eq!(new.lambda.aws_profile.as_deref(), Some("new"));

        // an own field migrates through the same logic
        let mut new = ServiceClientOptions::default();
        let deprecated = ServiceClientOptions {
