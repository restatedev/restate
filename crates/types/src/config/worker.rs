...
impl InvokerOptions {
    ...
    pub(crate) fn merge(&mut self, common: &CommonOptions, opts: &NetworkingOptions) {
        #[allow(deprecated)]
        self.service_client.apply_deprecated(
            "worker.invoker.service-client",
            common.service_client.clone(),
        );
...
impl ServiceClientOptions {
    ...
    pub fn apply_deprecated(&mut self, new_base: &str, deprecated: DeprecatedServiceClientOptions) {
...
pub struct DeprecatedServiceClientOptions {
    #[serde(flatten)]
...
mod tests {
    ...
    fn apply_deprecated_precedence() {
        ...
        // regression guard for the `request_compression_threshold` bug: its canonical default is
        // `Some(4 MiB)`, so the previous compare-to-default approach would silently swallow a
        // user that wrote that exact value at the deprecated location. The shadow tracks
        // user-set-ness directly, so this value now migrates.
        let four_mib = ByteCount::from(4usize * 1024 * 1024);
        let mut new = ServiceClientOptions::default();
        let deprecated = DeprecatedServiceClientOptions {
            lambda: DeprecatedAwsLambdaOptions {
                request_compression_threshold: Some(four_mib),
                ..Default::default()
            },
            ..Default::default()
        };
        new.apply_deprecated(base, deprecated);
        assert_eq!(
            new.lambda.request_compression_threshold,
            Some(four_mib),
            "value equal to canonical default must still migrate"
        );
