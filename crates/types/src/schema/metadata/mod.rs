...
impl ActiveServiceRevision {
    fn as_service_metadata(&self) -> service::ServiceMetadata {
        self.service_revision
            .as_service_metadata(self.deployment_id)
    }
}
...
impl Deployment {
    fn as_deployment(&self) -> deployment::Deployment {
...
impl ServiceRevision {
    fn as_service_metadata(&self, deployment_id: DeploymentId) -> service::ServiceMetadata {
    ...
    fn get_deployment_and_services(
        ...
        self.deployments.get(deployment_id).map(|dp| {
            (
                ...
                dp.services
                    .values()
                    .map(|s| s.as_service_metadata(*deployment_id))
                    .collect(),
            )
        })
    }
