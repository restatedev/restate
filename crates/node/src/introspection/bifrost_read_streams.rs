...
impl Scan for BifrostReadStreamsScanner {
    fn scan(
        ...
        let fut = async move {
            ...
            for (count, (stream_id, state)) in snapshot.iter().enumerate() {
                if limit.is_some_and(|l| count >= l) {
                    break;
                }
                append_row(&mut builder, my_node_id, stream_id.to_string(), state);
            }
...
fn append_row(
    ...
    if let Some(tail) = state.safe_known_tail {
        ...
        // Compute how far behind: tail - read_pointer (saturating)
        let tail_val = u64::from(tail);
        let pointer_val = u64::from(state.read_pointer);
        row.behind_count(tail_val.saturating_sub(pointer_val));
    }
    ...
    row.is_tailing(state.is_tailing);
    row.is_terminated(state.is_terminated);
}
