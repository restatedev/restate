...
use restate_core::network::ConnectThrottle;
...
use super::node_state::Node;

const GOSSIP_ATTEMPT_LIMIT: usize = 10;
...
impl FdState {
    ...
    pub fn gossip_tick(&mut self, opts: &GossipOptions) -> bool {
        ...
        let full_intervals = self
            ...
            .floor() as u32;
        // if self.last_gossip_tick.elapsed() >= *opts.gossip_tick_interval {
        if full_intervals > 0 {
    ...
    pub fn update_from_gossip_message(
        ...
        for (_idx, incoming_node) in msg.nodes.iter().enumerate() {
            ...
            if node.maybe_reset(incoming_node.node_id, sender_nc_version, node.instance_ts) {
                // yes, we have reset
    ...
    pub fn select_targets_for_gossip(
        ...
        let mut chosen = self
            ...
            .filter_map(|(_, node)| {
                ...
                let is_gone = node.is_gone();
                let is_me = node.gen_node_id.as_plain() == self.my_node_id.as_plain();
                (is_known && !is_gone && !is_me).then_some(node)
            })
