...
impl TaskCenter {
    ...
    /// Launch a new task
    /// Note: if shutdown_node_on_failure is set to true, failure of the task
    /// will request a shutdown of the task center.
    #[track_caller]
    pub fn spawn<F>(
        &self,
        kind: TaskKind,
        ...
        F: Future<Output = anyhow::Result<()>> + Send + 'static,
    {
        ...
        Ok(self.spawn_unchecked(kind, name, partition_id, shutdown_node_on_failure, future))
    }
    ...
    ///
    ///   cancel_tasks(Some(TaskKind::RpcServer), None)
    ///
            ...
            if let Some(mut join_handle) = join_handle {
                if log_all {
                    if partition_id.is_none() {
                        ...
                        info!(kind = ?task_kind, name = ?task.name, "stopping task");
                    } else {
    ...
    F: Future<Output = anyhow::Result<()>> + Send + 'static,
{
    ...
    let result = CURRENT_TASK_CENTER
        ...
        .await;
    task_center
        .on_finish(result, kind, task_id, shutdown_process_on_error)
        .await;
}
...
pub async fn cancellation_watcher() {
    let token = CANCEL_TOKEN
        .try_with(|t| t.clone())
        .expect("cancellation_watcher() called in a task-center task");
...
mod tests {
    ...
    async fn test_basic_lifecycle() -> Result<()> {
        ...
        let tid = tc
            .spawn(TaskKind::RoleRunner, "worker-role", None, false, async {
                info!("Hello async");
