...
use restate_types::sharding::{KeyRange, WithPartitionKey};

use crate::keys::IndexKeyPrefix;
use crate::keys::filter::KeyMatch;
use crate::{PartitionStore, Result, break_on_err};

use super::{InvocationByServiceStage, InvocationByServiceStageKey};
...
impl PartitionStore {
    ...
    pub fn scan_invocation_by_service<F>(
        &self,
        range: KeyRange,
        filter: &Filter<InvocationByService>,
        mut f: F,
    ) -> Result<impl Future<Output = Result<()>> + Send + use<'_, F>>
    where
        F: FnMut(InvocationByServiceStageKey) -> ControlFlow<Result<()>> + Send + 'static,
    {
        let prefix = IndexKeyPrefix::of::<InvocationByServiceStage>(self.partition_id());
        let cursor =
            InvocationByServiceStageKey::prepare_filter(filter)?.into_cursor(prefix.as_bytes())?;
            ...
            .map(|(range, mut cursor)| {
                ...
                self.iterator_controlled_physical(
                    ...
                    move |(key, value)| {
                        ...
                        if !range.contains(&key.invocation_id.partition_key()) {
...
mod tests {
    ...
    use restate_storage_api::index::InvocationByServiceClause;
    ...
    async fn scan(
        ...
        store
            .scan_invocation_by_service(range, &filter, move |key| {
    ...
    async fn scans_enforce_ownership_predicates_and_callback_control() {
        ...
        let mut store = manager
            ...
            .await
            .unwrap();
        let id = |pk| InvocationId::from_parts(pk, InvocationUuid::from_u128(pk as u128));
        ...
        InvocationByServiceStageKey::borrowed("svc", Stage::Inbox, Reverse(at), id(180))
        ...
        let filter = Filter::All
            .and(InvocationByServiceClause::ServiceName(
                ValuePredicate::Equal("svc".into()),
            ))
            .and(InvocationByServiceClause::InvocationId(ValuePredicate::In(
                vec![id(110), id(150), id(110)],
            )));
        assert_eq!(
        ...
        let (sender, mut receiver) = tokio::sync::mpsc::unbounded_channel();
        store
            .scan_invocation_by_service(KeyRange::FULL, &Filter::All, move |_| {
                sender.send(()).unwrap();
        ...
        let error = store
            .scan_invocation_by_service(KeyRange::FULL, &Filter::All, |_| {
