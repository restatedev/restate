...
use self::entry::{EntryStateKey, Owned, State};
...
impl ScanVQueueTable for PartitionDb {
    fn scan_active_vqueues(
        ...
        // key. If we want to use it, we need to set the length to (
        ...
        let success = convert_to_upper_bound(&mut key_buf);
...
impl ReadVQueueTable for PartitionStoreTransaction<'_> {
    ...
    async fn get_entry_state_header(
        ...
        let slice = raw_value;
        let decoded = State::<()>::decode(&mut slice.as_ref())?;
        Ok(Some(Owned {
            partition_key,
            kind,
            id: *id,
