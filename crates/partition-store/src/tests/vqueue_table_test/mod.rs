...
use restate_storage_api::vqueue_table::{
    EntryKey, EntryMetadata, EntryValue, Stage, VQueueCursor, VQueueStore, WriteVQueueTable,
};
...
fn entry_value(id: u8, original_run_at: u64, num_attempts: u32) -> EntryValue {
    ...
    let first_started_at = if num_attempts == 0 {
        None
    } else {
        Some(created_at)
    };
    let original_run_at = test_run_at(original_run_at);
    let first_runnable_at = if original_run_at < created_at_ms {
        created_at_ms
    } else {
        original_run_at
    };
    ...
    EntryValue {
        first_started_at,
        first_runnable_at: first_runnable_at.into(),
...
async fn key_ordering_by_has_lock_run_at_seq<W: WriteVQueueTable>(txn: &mut W) {
    ...
    for (key, value) in &entries {
        txn.put_vqueue_entry(&qid, Stage::Inbox, key, value);
    }
}
...
async fn running_and_inbox_are_separate<W: WriteVQueueTable>(txn: &mut W) {
    ...
    txn.put_vqueue_entry(&qid, Stage::Running, &run_entry.0, &run_entry.1);

    // Put two entries in Inbox stage
    let inbox_entry1 = default_entry(20);
    let inbox_entry2 = default_entry(21);
    txn.put_vqueue_entry(&qid, Stage::Inbox, &inbox_entry1.0, &inbox_entry1.1);
    txn.put_vqueue_entry(&qid, Stage::Inbox, &inbox_entry2.0, &inbox_entry2.1);
}
...
async fn vqueue_isolation<W: WriteVQueueTable>(txn: &mut W) {
    ...
    txn.put_vqueue_entry(&qid1, Stage::Inbox, &entry1.0, &entry1.1);
    txn.put_vqueue_entry(&qid2, Stage::Inbox, &entry2.0, &entry2.1);
    txn.put_vqueue_entry(&qid3, Stage::Inbox, &entry3.0, &entry3.1);
}
...
async fn waiting_cursor_boundary_is_respected<W: WriteVQueueTable>(txn: &mut W) {
    ...
    txn.put_vqueue_entry(&qid_a, Stage::Inbox, &a1.0, &a1.1);
    txn.put_vqueue_entry(&qid_a, Stage::Inbox, &a2.0, &a2.1);
    txn.put_vqueue_entry(&qid_b, Stage::Inbox, &b1.0, &b1.1);
}
...
async fn tailing_iterator_sees_new_items_on_reseek(rocksdb: &mut PartitionStore) {
    ...
    let entry2 = default_entry(2);
    {
        ...
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry1.0, &entry1.1);
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry2.0, &entry2.1);
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry3.0, &entry3.1);
        txn.commit().await.expect("commit should succeed");
    }
...
async fn reseek_shows_new_higher_order_items(rocksdb: &mut PartitionStore) {
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_vqueue_entry(&qid, Stage::Inbox, &base1.0, &base1.1);
        txn.put_vqueue_entry(&qid, Stage::Inbox, &base2.0, &base2.1);
        txn.commit().await.expect("commit should succeed");
    }
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_vqueue_entry(&qid, Stage::Inbox, &older_run_at.0, &older_run_at.1);
        txn.commit().await.expect("commit should succeed");
    }
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_vqueue_entry(&qid, Stage::Inbox, &locked.0, &locked.1);
        txn.commit().await.expect("commit should succeed");
    }
...
async fn tailing_iterator_sees_new_items_on_seek_after(rocksdb: &mut PartitionStore) {
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry_first.0, &entry_first.1);
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry_second.0, &entry_second.1);
        txn.commit().await.expect("commit should succeed");
    }
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry_third.0, &entry_third.1);
        txn.commit().await.expect("commit should succeed");
    }
...
async fn tailing_iterator_sees_inserted_ahead_without_reseek(rocksdb: &mut PartitionStore) {
    ...
    let entry5 = entry(5, false, 50, 5);
    {
        ...
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry1.0, &entry1.1);
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry3.0, &entry3.1);
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry5.0, &entry5.1);
        txn.commit().await.expect("commit should succeed");
    }
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry4.0, &entry4.1);
        txn.commit().await.expect("commit should succeed");
    }
...
async fn deleted_items_not_visible_after_reseek(rocksdb: &mut PartitionStore) {
    ...
    // Delete the middle entry while the reader is still open
    {
        let mut txn = rocksdb.transaction();
        assert!(
            txn.get_vqueue_entry(&qid, Stage::Inbox, &entry2.0)
                .unwrap()
                .is_some()
        );
        txn.delete_vqueue_entry(&qid, Stage::Inbox, &entry2.0);
        txn.commit().await.expect("commit should succeed");
    }
...
async fn deleted_items_not_visible_after_seek_after(rocksdb: &mut PartitionStore) {
    ...
    // Delete entries 2 and 3 while reader is open
    {
        ...
        txn.delete_vqueue_entry(&qid, Stage::Inbox, &entry2.0);
        assert!(
            txn.get_vqueue_entry(&qid, Stage::Inbox, &entry3.0)
                .unwrap()
                .is_some()
        );
        txn.delete_vqueue_entry(&qid, Stage::Inbox, &entry3.0);
        txn.commit().await.expect("commit should succeed");
    }
...
async fn concurrent_enqueue_and_delete(rocksdb: &mut PartitionStore) {
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry_high.0, &entry_high.1);
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry_mid.0, &entry_mid.1);
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry_low.0, &entry_low.1);
        txn.commit().await.expect("commit should succeed");
    }
    ...
    let entry_new_first = entry(5, true, 5, 5);
    {
        let mut txn = rocksdb.transaction();
        assert!(
            txn.get_vqueue_entry(&qid, Stage::Inbox, &entry_mid.0)
                .unwrap()
                .is_some()
        );
        txn.delete_vqueue_entry(&qid, Stage::Inbox, &entry_mid.0);
        txn.put_vqueue_entry(&qid, Stage::Inbox, &entry_new_first.0, &entry_new_first.1);
        txn.commit().await.expect("commit should succeed");
    }
