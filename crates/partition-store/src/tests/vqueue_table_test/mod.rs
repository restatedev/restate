...
fn entry_card_with_visible_at(id: u8, priority: EffectivePriority, visible_at: u64) -> EntryCard {
    EntryCard {
        priority,
...
async fn ordering_within_same_priority<W: WriteVQueueTable>(txn: &mut W) {
    ...
    let entries = [
        entry_card_with_visible_at(3, EffectivePriority::UserDefault, 3000), // visible later
        entry_card_with_visible_at(1, EffectivePriority::UserDefault, 0),    // Now (visible_at=0)
        entry_card_with_visible_at(2, EffectivePriority::UserDefault, 2000), // visible sooner
    ];
