...
use restate_storage_api::vqueue_table::{
    EntryId, EntryKey, EntryMetadata, EntryStatistics, EntryValue, RunAt, Stage, VQueueCursor,
    VQueueStore, WriteVQueueTable,
};
...
fn entry_id(id: u8) -> EntryId {
    EntryId::Invocation([id; 16])
}

fn entry_key(has_lock: bool, run_at: u64, seq: u64) -> EntryKey {
    assert!(run_at < (1u64 << 63), "run_at must fit in 63 bits");

    let lock_bit = if has_lock { 0 } else { 1 };
    let hi = (lock_bit << 63) | run_at;

    let mut encoded = [0u8; EntryKey::serialized_length_fixed()];
    encoded[..8].copy_from_slice(&hi.to_be_bytes());
    encoded[8..].copy_from_slice(&seq.to_be_bytes());
    EntryKey::from_bytes(encoded)
}
...
fn entry_value(id: u8, original_run_at: u64, num_attempts: u32) -> EntryValue {
    ...
    EntryValue {
        id: entry_id(id),
        original_run_at: RunAt::new(MillisSinceEpoch::new(original_run_at)),
        stats,
...
fn collect_ids(items: &[(EntryKey, EntryValue)]) -> Vec<EntryId> {
    items.iter().map(|(_, value)| value.id.clone()).collect()
}
...
async fn key_ordering_by_has_lock_run_at_seq<W: WriteVQueueTable>(txn: &mut W) {
    ...
    for (key, value) in &entries {
        txn.put_inbox_entry(&qid, Stage::Inbox, key, value);
    }
}
...
fn verify_key_ordering_by_has_lock_run_at_seq(db: &crate::PartitionDb) {
    ...
    assert_eq!(items[0].0, entry_key(true, 100, 1));
    assert_eq!(items[1].0, entry_key(true, 150, 2));
    assert_eq!(items[2].0, entry_key(false, 5, 1));
    assert_eq!(items[3].0, entry_key(false, 5, 2));
    assert_eq!(items[4].0, entry_key(false, 15, 10));
    assert_eq!(
        collect_ids(&items),
        vec![
            entry_id(2),
...
fn verify_ordering_within_same_lock_domain(db: &crate::PartitionDb) {
    ...
    assert_eq!(items[0].0, entry_key(false, 0, 9));
    assert_eq!(items[1].0, entry_key(false, 2_000, 2));
    assert_eq!(items[2].0, entry_key(false, 2_000, 10));
    assert_eq!(items[3].0, entry_key(false, 3_000, 5));
    assert_eq!(
        collect_ids(&items),
...
async fn running_and_inbox_are_separate<W: WriteVQueueTable>(txn: &mut W) {
    ...
    txn.put_inbox_entry(&qid, Stage::Run, &run_entry.0, &run_entry.1);

    // Put two entries in Inbox stage
    let inbox_entry1 = default_entry(20);
    let inbox_entry2 = default_entry(21);
    txn.put_inbox_entry(&qid, Stage::Inbox, &inbox_entry1.0, &inbox_entry1.1);
    txn.put_inbox_entry(&qid, Stage::Inbox, &inbox_entry2.0, &inbox_entry2.1);
}
...
fn verify_running_and_inbox_are_separate(db: &crate::PartitionDb) {
    ...
    assert_eq!(run_items[0].1.id, entry_id(10));
...
async fn vqueue_isolation<W: WriteVQueueTable>(txn: &mut W) {
    ...
    txn.put_inbox_entry(&qid1, Stage::Inbox, &entry1.0, &entry1.1);
    txn.put_inbox_entry(&qid2, Stage::Inbox, &entry2.0, &entry2.1);
    txn.put_inbox_entry(&qid3, Stage::Inbox, &entry3.0, &entry3.1);
}
...
async fn waiting_cursor_boundary_is_respected<W: WriteVQueueTable>(txn: &mut W) {
    ...
    txn.put_inbox_entry(&qid_a, Stage::Inbox, &a1.0, &a1.1);
    txn.put_inbox_entry(&qid_a, Stage::Inbox, &a2.0, &a2.1);
    txn.put_inbox_entry(&qid_b, Stage::Inbox, &b1.0, &b1.1);
}
...
fn verify_waiting_cursor_boundary_is_respected(db: &crate::PartitionDb) {
    ...
    assert_eq!(
        reader_a.peek().unwrap().as_ref().map(|e| e.1.id.clone()),
        Some(entry_id(11))
    );
    reader_a.advance();
    assert_eq!(
...
async fn tailing_iterator_sees_new_items_on_reseek(rocksdb: &mut PartitionStore) {
    ...
    let entry2 = default_entry(2);
    {
        ...
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry1.0, &entry1.1);
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry2.0, &entry2.1);
    ...
    let item = reader.peek().unwrap();
    assert_eq!(item.as_ref().map(|e| e.1.id.clone()), Some(entry_id(1)));
    reader.advance();

    let item = reader.peek().unwrap();
    assert_eq!(item.as_ref().map(|e| e.1.id.clone()), Some(entry_id(2)));
    reader.advance();
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry3.0, &entry3.1);
        txn.commit().await.expect("commit should succeed");
    }
...
async fn reseek_shows_new_higher_order_items(rocksdb: &mut PartitionStore) {
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_inbox_entry(&qid, Stage::Inbox, &base1.0, &base1.1);
        txn.put_inbox_entry(&qid, Stage::Inbox, &base2.0, &base2.1);
        txn.commit().await.expect("commit should succeed");
    }
    ...
    let mut reader = db.new_inbox_reader(&qid);
    reader.seek_to_first();
    assert_eq!(reader.peek().unwrap().unwrap().1.id, entry_id(1));

    // Add an item with older run_at (higher order among unlocked entries)
    let older_run_at = entry(3, false, 50, 3);
    {
        let mut txn = rocksdb.transaction();
        txn.put_inbox_entry(&qid, Stage::Inbox, &older_run_at.0, &older_run_at.1);
        txn.commit().await.expect("commit should succeed");
    }

    reader.seek_to_first();
    assert_eq!(reader.peek().unwrap().unwrap().1.id, entry_id(3));

    // Add an item that has a lock (always higher order than unlocked entries)
    let locked = entry(4, true, 300, 4);
    {
        let mut txn = rocksdb.transaction();
        txn.put_inbox_entry(&qid, Stage::Inbox, &locked.0, &locked.1);
        txn.commit().await.expect("commit should succeed");
    }
...
async fn tailing_iterator_sees_new_items_on_seek_after(rocksdb: &mut PartitionStore) {
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry_first.0, &entry_first.1);
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry_second.0, &entry_second.1);
        txn.commit().await.expect("commit should succeed");
    }
    ...
    assert_eq!(first.1.id, entry_id(1));

    // Now add a new entry that sorts after the seek position
    let entry_third = entry(3, false, 10, 3);
    {
        let mut txn = rocksdb.transaction();
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry_third.0, &entry_third.1);
        txn.commit().await.expect("commit should succeed");
    }
...
async fn tailing_iterator_sees_inserted_ahead_without_reseek(rocksdb: &mut PartitionStore) {
    ...
    let entry5 = entry(5, false, 50, 5);
    {
        ...
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry1.0, &entry1.1);
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry3.0, &entry3.1);
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry5.0, &entry5.1);
        txn.commit().await.expect("commit should succeed");
    }
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry4.0, &entry4.1);
        txn.commit().await.expect("commit should succeed");
    }

    // Continue from current position without seek.
    reader.advance();
    let maybe_inserted = reader.peek().unwrap().as_ref().map(|e| e.1.id.clone());
    assert_eq!(
        maybe_inserted,
        Some(entry_id(4)),
    ...
    reader.advance();
    assert_eq!(reader.peek().unwrap().unwrap().1.id, entry_id(5));
}
...
async fn tailing_iterator_sees_item_added_after_end_without_reseek(rocksdb: &mut PartitionStore) {
    ...
    assert_eq!(reader.peek().unwrap().unwrap().1.id, entry_id(2));
}
...
async fn deleted_items_not_visible_after_reseek(rocksdb: &mut PartitionStore) {
    ...
    // Delete the middle entry while the reader is still open
    {
        let mut txn = rocksdb.transaction();
        assert!(
            txn.pop_inbox_entry(&qid, Stage::Inbox, &entry2.0)
                .unwrap()
                .is_some()
        );
        txn.commit().await.expect("commit should succeed");
...
async fn deleted_items_not_visible_after_seek_after(rocksdb: &mut PartitionStore) {
    ...
    // Delete entries 2 and 3 while reader is open
    {
        ...
        assert!(
            txn.pop_inbox_entry(&qid, Stage::Inbox, &entry3.0)
                .unwrap()
                .is_some()
        );
        txn.commit().await.expect("commit should succeed");
...
async fn concurrent_enqueue_and_delete(rocksdb: &mut PartitionStore) {
    ...
    {
        let mut txn = rocksdb.transaction();
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry_high.0, &entry_high.1);
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry_mid.0, &entry_mid.1);
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry_low.0, &entry_low.1);
        txn.commit().await.expect("commit should succeed");
    }
    ...
    let first = reader.peek().unwrap().unwrap();
    assert_eq!(first.1.id, entry_id(10));
    ...
    let entry_new_first = entry(5, true, 5, 5);
    {
        let mut txn = rocksdb.transaction();
        assert!(
            txn.pop_inbox_entry(&qid, Stage::Inbox, &entry_mid.0)
                .unwrap()
                .is_some()
        );
        txn.put_inbox_entry(&qid, Stage::Inbox, &entry_new_first.0, &entry_new_first.1);
        txn.commit().await.expect("commit should succeed");
    }
