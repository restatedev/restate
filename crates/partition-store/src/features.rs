...
use crate::fsm_table::{append_storage_features_to_wb, append_storage_version_to_wb};
...
impl LoadedStorageFeatures {
    pub(crate) fn load(
        ...
        if storage_version.is_scope_migrated() {
            ...
            loaded.set_enabled::<MigratedToScopedStateTableFeature>(None);
            loaded.set_enabled::<MigratedToScopedPromiseTableFeature>(None);
        }
...
async fn enable_helper<F: StorageFeature>(
    ...
    if features.enabled().is_migrated_to_scoped_state_table
        && features.enabled().is_migrated_to_scoped_promise_table
    {
    ...
    (
        ...
        ::paste::paste! {
            ...
            pub(crate) struct StorageFeatures {
                $(
                    $(#[$attrs])*
                    pub(crate) [<is_ $feature:snake>]: bool,
                )*
            }
