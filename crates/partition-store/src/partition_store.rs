...
impl PartitionStore {
    ...
    fn prefix_iterator<K: Into<Vec<u8>>>(
        &self,
        table: TableKind,
        _key_kind: KeyKind,
        ...
        self.raw_db.raw_iterator_cf_opt(&table, opts)
    }
    ...
    fn range_iterator(
        ...
        table: TableKind,
        _key: KeyKind,
        maybe_partition_id: Option<PartitionId>,
        range: impl rocksdb::IterateBounds,
    ) -> DBIterator {
        let table = self.table_handle(table);
        let mut opts = ReadOptions::default();
        if maybe_partition_id.is_none() {
    ...
    fn iterator_from<K: TableKey>(
        ...
        match scan {
            PhysicalScan::Prefix(table, key_kind, partition_id, prefix) => {
                assert!(table.has_key_kind(&prefix));
                let mut it = self.prefix_iterator(table, key_kind, partition_id, prefix.clone());
                it.seek(prefix);
                it
            }
            PhysicalScan::RangeExclusive(table, key_kind, maybe_partition_id, start, end) => {
                assert!(table.has_key_kind(&start));
                let mut it = self.range_iterator(
                    table,
                    key_kind,
                    maybe_partition_id,
            ...
            PhysicalScan::RangeOpen(table, key_kind, start) => {
                ...
                let mut it = self.range_iterator(table, key_kind, None, start.clone()..end);
                it.seek(start);
                it
            }
        }
...
impl<'a> RocksDBTransaction<'a> {
    pub(crate) fn prefix_iterator<K: Into<Vec<u8>>>(
        &self,
        table: TableKind,
        _key_kind: KeyKind,
        _partition_id: PartitionId,
...
impl<'a> StorageAccess for RocksDBTransaction<'a> {
    ...
    fn iterator_from<K: TableKey>(
        ...
        match scan {
            ...
            PhysicalScan::RangeExclusive(table, key_kind, maybe_partition_id, start, end) => {
                let mut it =
                    self.range_iterator(table, key_kind, maybe_partition_id, start.clone()..end);
                it.seek(start);
                it
            }
            PhysicalScan::RangeOpen(table, key_kind, start) => {
