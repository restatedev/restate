...
use crate::keys::{KeyCodec, KeyKind, TableKey, TableKeyPrefix, define_table_key};
...
impl KeyCodec for OptionalScope {
        ...
        let scope = match source.get_u8() {
            ...
            _ => {
                return Err(StorageError::Generic(anyhow::anyhow!(
                    "unknown scope prefix: {:x?}",
                    source.get_u8()
                )));
            }
        };
        Ok(OptionalScope { scope })
    }

    fn serialized_length(&self) -> usize {
        1 + std::mem::size_of::<u32>() + self.scope.as_ref().map(|s| s.len()).unwrap_or_default()
    }
}

// Note that LockName *must* be used as the suffix portion of a key since it's not length-prefixed.
impl KeyCodec for LockName {
    fn encode<B: BufMut>(&self, target: &mut B) {
...
impl LoadLocks for PartitionDb {
    fn scan_all_locked(&self, mut on_item: impl FnMut(Option<Scope>, LockName)) -> Result<()> {
        ...
        while it.valid() {
            ...
            tracing::info!("Loading active lock: {lock_key:?}");
            let (_partition_key, opt_scope, lock_name) = lock_key.split();
            on_item(opt_scope.scope, lock_name);
            it.next();
        }
...
impl WriteLockTable for PartitionStoreTransaction<'_> {
    fn acquire_lock(&mut self, scope: Option<Scope>, lock_name: LockName, state: LockState) {
        ...
        let key = LockKey::builder()
            .partition_key(partition_key)
            .optional_scope(OptionalScope { scope })
            .lock_name(lock_name);
        let key_buf = {
    ...
    >(
        ...
        self.iterator_for_each(
            ...
            move |(mut key, mut value)| {
                ...
                f((partition_key, opt_scope.scope, lock_name, lock_state)).map_break(Ok)
            },
        )
