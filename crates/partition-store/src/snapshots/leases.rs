...
mod inner {
    ...
    impl<C: Clock + Clone + Send + Sync + 'static> MetadataBackedLeaseGuard<C> {
        ...
        /// Test-only: mark the guard as released without actually releasing
        #[cfg(test)]
...
/// Manages snapshot lease acquisition and release via metadata store
#[derive(Clone)]
...
impl SnapshotLeaseManager<WallClock> {
    /// Create a new lease manager with WallClock
    pub fn new(metadata_store_client: MetadataStoreClient, node_id: GenerationalNodeId) -> Self {
        Self {
            metadata_store_client,
            node_id,
            clock: WallClock,
...
impl<C: Clock + Clone + Send + Sync + 'static> SnapshotLeaseManager<C> {
    /// Create a new lease manager with a custom clock (for testing)
    pub fn with_clock(
        metadata_store_client: MetadataStoreClient,
        node_id: GenerationalNodeId,
        clock: C,
...
mod tests {
    ...
    /// Helper to create a lease manager with a mock clock for testing
    fn create_test_manager(
        metadata_store_client: MetadataStoreClient,
        node_id: GenerationalNodeId,
        clock: MockClock,
    ) -> SnapshotLeaseManager<MockClock> {
        SnapshotLeaseManager::with_clock(metadata_store_client, node_id, clock)
    }
    ...
    async fn test_lease_acquire_no_existing() {
        ...
        let manager =
            create_test_manager(env.metadata_store_client.clone(), node_id, clock.clone());
    ...
    async fn test_lease_acquire_expired_takeover() {
        ...
        let other_manager = create_test_manager(
    ...
    async fn test_lease_acquire_concurrent_race() {
        ...
        let manager1 = create_test_manager(env.metadata_store_client.clone(), node1, clock.clone());
        let manager2 = create_test_manager(env.metadata_store_client.clone(), node2, clock.clone());
