...
impl PartitionDb {
    pub(crate) fn new(
        ...
        Self {
            ...
            archived_lsn,
            cf: ManuallyDrop::new(PartitionBoundCfHandle::new(cf)),
            rocksdb: ManuallyDrop::new(rocksdb),
        }
    }
    ...
    pub fn get_archived_lsn(&self) -> Option<Lsn> {
        self.archived_lsn.borrow().clone()
    }
...
impl Drop for PartitionDb {
    fn drop(&mut self) {
        // SAFETY: this ensures we have a well-known order of drops. Rust will drop the fields in the
        // order they are declated in the struct but we to spell out our intention to avoid any
        // issues caused by unintentonal reordering of fields.
        unsafe { ManuallyDrop::drop(&mut self.cf) };
        unsafe { ManuallyDrop::drop(&mut self.rocksdb) };
    }
}
...
impl PartitionCell {
    ...
    /// Returns a clone of the underlying [`PartitionDb`], if it is open.
    ...
    /// Updates the durable LSN to the given value.
...
pub(crate) enum State {
    ...
    #[default]
    Unknown,
    /// The column famil(ies) for this partition do not exist locally
    CfMissing,
    Open {
        db: PartitionDb,
    },
