...
use crate::{
    wrap_service_invocation_in_envelope, AckSender, ExpiringIngressResponse, IdempotencyMode,
    IngressDeduplicationId, IngressRequest, IngressRequestMode, IngressResponseSender,
};
...
struct IngressDispatcherState {
    ...
    waiting_for_acks: DashMap<MessageIndex, AckSender>,
    waiting_for_acks_with_custom_id: DashMap<IngressDeduplicationId, AckSender>,
}
    ...
    async fn dispatch_ingress_request(
        ...
        let (dedup_source, msg_index) = match request_mode {
            ...
            IngressRequestMode::FireAndForget(ack_sender) => {
                ...
                self.state.waiting_for_acks.insert(msg_index, ack_sender);
                (None, msg_index)
            }
...
impl MessageHandler for IngressDispatcher {
    ...
    async fn on_message(&self, msg: restate_node_protocol::MessageEnvelope<Self::MessageType>) {
        ...
        match msg {
            ...
            IngressMessage::AckIngress(ack) => {
                info!("Received message ack: {}", ack.message_index);

                if let Some(dedup_name) = ack.dedup_source {
                    if let Some((_, ack_sender)) = self
                        .state
                        .waiting_for_acks_with_custom_id
                        .remove(&(dedup_name, ack.message_index))
                    {
                        // Receivers might be gone if they are not longer interested in the ack notification
                        let _ = ack_sender.send(());
                    }
                } else if let Some((_, ack_sender)) =
                    self.state.waiting_for_acks.remove(&ack.message_index)
                {
                    // Receivers might be gone if they are not longer interested in the ack notification
                    let _ = ack_sender.send(());
                }
