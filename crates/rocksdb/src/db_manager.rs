...
impl RocksDbManager {
    ...
    pub async fn open_db<T: RocksAccess + Send + Sync + 'static>(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            // use options to open db
            // register db to manager. Keep the latest RocksDbOptions used for this db.
            // watch for config update
            // on config update, iterate over all databases, and use a function to
            // translate the config update to [(key, val)]
            //self.subscriptions.append(&mut db.subscriptions);
            Ok(db)
        })
        .await
...
impl DbWatchdog {
    ...
    fn shutdown(&mut self) {
        ...
        for v in &self.manager.dbs {
            ...
            if let Err(e) = v.db.flush_memtables_atomically(v.cfs().as_slice(), true) {
                warn!(
                    db = %v.name,
                    owner = %v.owner,
