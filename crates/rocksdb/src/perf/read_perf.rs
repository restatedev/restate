...
impl RealRocksGuard {
    ...
    fn report(&self) {
        ...
        // Avoid record_many: the Prometheus recorder calls Instant::now() per repeated sample.
        ...
        if v != 0 {
            histogram!(FIND_NEXT_USER_ENTRY, &labels).record(n_to_s(v));
        }
        ...
        if v != 0 {
            histogram!(INDEX_BLOCK_READ_DURATION, &labels).record(n_to_s(v));
        }

        let v = self.context.metric(PerfMetric::ReadFilterBlockNanos);
        if v != 0 {
            histogram!(FILTER_BLOCK_READ_DURATION, &labels).record(n_to_s(v));
        }
        ...
        if v != 0 {
            histogram!(BLOCK_SEEK_DURATION, &labels).record(n_to_s(v));
        }
        ...
        if v != 0 {
            histogram!(MERGE_OPERATOR_DURATION, &labels).record(n_to_s(v));
        }
