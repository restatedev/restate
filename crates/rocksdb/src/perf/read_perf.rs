...
use crate::metric_definitions::{
    BLOCK_CACHE_HIT, BLOCK_SEEK_DURATION, BLOOM_MEMTABLE_HIT, BLOOM_MEMTABLE_MISS, BLOOM_SST_HIT,
    BLOOM_SST_MISS, DATA_BLOCK_READ_BYTES, FILTER_BLOCK_READ_BYTES, FILTER_BLOCK_READ_DURATION,
    FIND_NEXT_USER_ENTRY, INDEX_BLOCK_READ_BYTES, INDEX_BLOCK_READ_DURATION,
    MERGE_OPERATOR_DURATION,
};
use crate::{
    BLOCK_DECOMPRESS_DURATION, BLOCK_READ_BYTES, BLOCK_READ_DURATION, GET_FROM_MEMTABLE_DURATION,
    NEXT_ON_MEMTABLE, OP_NAME, SEEK_ON_MEMTABLE, TOTAL_DURATION,
};

// Sample 33% of rocksdb write operations.
// Weight W = 1/P where p is the sampling rate.
const WEIGHT: usize = 10;
const SAMPLE_RATE: u64 = const { u64::MAX / WEIGHT as u64 };

/// This guard is !Send. It must be created and dropped on the same thread since
/// it's backed by a thread-local in rocksdb.
pub struct RocksDbReadPerfGuard(Inner);

static_assertions::assert_not_impl_any!(RocksDbReadPerfGuard: Send);

impl RocksDbReadPerfGuard {
...
impl Drop for RocksDbReadPerfGuard {
    fn drop(&mut self) {
        if let Inner::Real(guard) = &mut self.0 {
...
impl RealRocksGuard {
    ...
    fn report(&self) {
        ...
        if v != 0 {
            counter!(NEXT_ON_MEMTABLE, &labels).increment(v * WEIGHT as u64)
        };
        let v = self.context.metric(PerfMetric::SeekOnMemtableTime);
        if v != 0 {
            histogram!(SEEK_ON_MEMTABLE, &labels).record_many(n_to_s(v), WEIGHT);
        }
        ...
        if v != 0 {
            counter!(BLOCK_READ_BYTES, &labels).increment(v * WEIGHT as u64)
        };

        let v = self.context.metric(PerfMetric::ReadIndexBlockNanos);
        if v != 0 {
            histogram!(INDEX_BLOCK_READ_DURATION, &labels).record_many(n_to_s(v), WEIGHT);
        }

        let v = self.context.metric(PerfMetric::ReadFilterBlockNanos);
        if v != 0 {
            histogram!(FILTER_BLOCK_READ_DURATION, &labels).record_many(n_to_s(v), WEIGHT);
        }

        let v = self.context.metric(PerfMetric::DataBlockReadByte);
        if v != 0 {
            counter!(DATA_BLOCK_READ_BYTES, &labels).increment(v * WEIGHT as u64);
        }

        let v = self.context.metric(PerfMetric::IndexBlockReadByte);
        if v != 0 {
            counter!(INDEX_BLOCK_READ_BYTES, &labels).increment(v * WEIGHT as u64);
        }

        let v = self.context.metric(PerfMetric::FilterBlockReadByte);
        if v != 0 {
            counter!(FILTER_BLOCK_READ_BYTES, &labels).increment(v * WEIGHT as u64);
        }

        let v = self.context.metric(PerfMetric::BlockSeekNanos);
        if v != 0 {
            histogram!(BLOCK_SEEK_DURATION, &labels).record_many(n_to_s(v), WEIGHT);
        }

        let v = self.context.metric(PerfMetric::BlockReadTime);
        if v != 0 {
            histogram!(BLOCK_READ_DURATION, &labels).record_many(n_to_s(v), WEIGHT);
        }

        let v = self.context.metric(PerfMetric::GetFromMemtableTime);
        if v != 0 {
            histogram!(GET_FROM_MEMTABLE_DURATION, &labels).record_many(n_to_s(v), WEIGHT);
        }

        let v = self.context.metric(PerfMetric::BlockDecompressTime);
        if v != 0 {
            histogram!(BLOCK_DECOMPRESS_DURATION, &labels).record_many(n_to_s(v), WEIGHT);
        }

        let v = self.context.metric(PerfMetric::BloomSstHitCount);
        if v != 0 {
            counter!(BLOOM_SST_HIT, &labels).increment(v * WEIGHT as u64);
        }
        let v = self.context.metric(PerfMetric::BloomSstMissCount);
        if v != 0 {
            counter!(BLOOM_SST_MISS, &labels).increment(v * WEIGHT as u64);
        }
        let v = self.context.metric(PerfMetric::BloomMemtableHitCount);
        if v != 0 {
            counter!(BLOOM_MEMTABLE_HIT, &labels).increment(v * WEIGHT as u64);
        }
        let v = self.context.metric(PerfMetric::BloomMemtableMissCount);
        if v != 0 {
            counter!(BLOOM_MEMTABLE_MISS, &labels).increment(v * WEIGHT as u64);
        }
        let v = self.context.metric(PerfMetric::TotalMetricCount);
        if v != 0 {
            counter!(BLOOM_MEMTABLE_HIT, &labels).increment(v * WEIGHT as u64);
        }
        let v = self.context.metric(PerfMetric::MergeOperatorTimeNanos);
        if v != 0 {
            histogram!(MERGE_OPERATOR_DURATION, &labels).record_many(n_to_s(v), WEIGHT);
        }
        let v = self.context.metric(PerfMetric::BlockCacheHitCount);
        if v != 0 {
            counter!(BLOCK_CACHE_HIT, &labels).increment(v * WEIGHT as u64);
        }
        let v = self.context.metric(PerfMetric::FindNextUserEntryTime);
        if v != 0 {
            histogram!(FIND_NEXT_USER_ENTRY, &labels).record_many(n_to_s(v), WEIGHT);
        }
    }
}
