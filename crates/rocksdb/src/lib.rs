    ...
    pub async fn write_batch(
        ...
        match io_mode {
            IoMode::AllowBlockingIO => {
                ...
                counter!(STORAGE_OP_FG, DISPOSITION => DISPOSITION_MAYBE_BLOCKING).increment(1);
                return Ok(());
            }
            IoMode::OnlyIfNonBlocking => {
                write_options.set_no_slowdown(true);
                self.db.write_batch(&write_batch, &write_options)?;
                counter!(STORAGE_OP_FG, DISPOSITION => DISPOSITION_NON_BLOCKING).increment(1);
                return Ok(());
            }
            _ => {}
        }
        ...
        match result {
            ...
            Err(e) if is_retryable_error(e.kind()) => {
                counter!(STORAGE_OP_FG, DISPOSITION => DISPOSITION_MOVED_TO_BG).increment(1);
            ...
            Err(e) => {
                counter!(STORAGE_OP_FG, DISPOSITION => DISPOSITION_FAILED).increment(1);
                Err(e.into())
            }
        }
    }
...
fn is_retryable_error(error_kind: rocksdb::ErrorKind) -> bool {
    match error_kind {
        rocksdb::ErrorKind::Incomplete
        | rocksdb::ErrorKind::TryAgain
        | rocksdb::ErrorKind::Busy => true,
        _ => false,
    }
}
