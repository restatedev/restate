    ...
    pub async fn write_batch(
        ...
        match io_mode {
            IoMode::AllowBlockingIO => {
                ...
                counter!(STORAGE_OP_FG, DISPOSITION => DISPOSITION_MAYBE_BLOCKING, OP_TYPE =>
                    OP_WRITE)
                .increment(1);
                return Ok(());
            }
            IoMode::OnlyIfNonBlocking => {
                ...
                self.db.write_batch(&write_batch, &write_options)?;
                counter!(STORAGE_OP_FG, DISPOSITION => DISPOSITION_NON_BLOCKING, OP_TYPE =>
                    OP_WRITE)
                .increment(1);
                return Ok(());
            }
        ...
        match result {
            Ok(_) => {
                counter!(STORAGE_OP_FG, DISPOSITION => DISPOSITION_NON_BLOCKING, OP_TYPE => OP_WRITE).increment(1);
                Ok(())
            }
            Err(e) if is_retryable_error(e.kind()) => {
                counter!(STORAGE_OP_FG, DISPOSITION => DISPOSITION_MOVED_TO_BG, OP_TYPE =>
                    OP_WRITE)
                .increment(1);
            ...
            Err(e) => {
                counter!(STORAGE_OP_FG, DISPOSITION => DISPOSITION_FAILED, OP_TYPE => OP_WRITE)
                    .increment(1);
                Err(e.into())
            }
        }
