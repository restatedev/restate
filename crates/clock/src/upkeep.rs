...
impl ClockUpkeep {
    ...
    /// any code attempts to read from [`WallClock::now()`](crate::WallClock::now). When the
    /// `cached-time` feature is enabled, `WallClock::now()` will return `MillisSinceEpoch(0)`
    /// until this function is called and performs its initial update.
    ///
    /// The server's `main()` function is responsible for starting the upkeep thread before
    /// any other initialization occurs. Other users who don't use the `cached-time` feature
    /// are not required to start this thread.
    ///
    /// # Errors
    ///
    ...
    pub fn start() -> Result<Self, std::io::Error> {
        ...
        if !cfg!(feature = "cached-time") {
            eprintln!(
                "WARN: Restate clock is running in uncached mode, feature `cached-time` is not enabled."
            );
        }
