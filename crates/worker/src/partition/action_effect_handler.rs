...
impl ActionEffectHandler {
    ...
    pub(super) async fn handle(
        ...
        for actuator_output in effects {
            ...
            let sender = match self.bifrost_appenders.entry(log_id) {
                btree_map::Entry::Vacant(entry) => {
                    let appender = self
                        .bifrost
                        .create_background_appender(log_id, 1000, 100)?
        ...
        // Check all all appenders are quiescent.
        let mut notifications = FuturesUnordered::new();
        for appender in self.bifrost_appenders.values() {
            notifications.push(appender.sender().notify_committed());
        }

        while let Some(quiescent) = notifications.next().await {
            // We fail if any of the appenders crashed or stopped since we didn't ask any of them
            // to drain, we shouldn't expect this to happen except for abnormal scenarios.
            //
            // In this case, we cannot consider this batch of effects to be committed!
            quiescent?;
        }
