...
pub enum Error {
    ...
    #[error("Bifrost envelope has unknown record kind")]
    UnknownRecordKind,
}
...
impl<S> StateMachineApplyContext<'_, S> {
    ...
    {
        match envelope.kind() {
            RecordKind::Unknown => Err(Error::UnknownRecordKind),
            RecordKind::AnnounceLeader | RecordKind::UpdatePartitionDurability => {
                // no-op
                Ok(())
            }
            RecordKind::VQSchedulerDecisions => {
            ...
            RecordKind::UpsertRuleBook => {
                let upsert = envelope
                    .into_typed::<records::UpsertRuleBook>()
                    .into_inner()?;
