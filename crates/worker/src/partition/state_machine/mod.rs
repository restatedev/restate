...
impl<S> StateMachineApplyContext<'_, S> {
            ...
            + WriteJournalTable,
    {
        ...
        .enqueue_new(
            record_unique_ts,
            visible_at,
            // or from the input invocation metadata
        ...
        // 1. Check if we need to schedule it
        let invocation_status = if let Some(execution_time) = metadata.execution_time
            // only schedule the invocation if it's actually in the future
            && visible_at > record_unique_ts
        {
            let span_context = metadata.span_context().clone();

            // this should be removed when scheduler handles future runs. This will avoid
            // the extra roundtrip to bifrost when the timer is due.
            self.register_timer(
    ...
    {
        info!(
            "[{}] entries in qid_parent={}, instance={} should be started",
            cards.entry_cards.len(),
            cards.parent,
            cards.instance
        );
        let qid = VQueueId::new(
    ...
    {
        // actually run.
        tracing::info!("We are RSM transitioning the entry inbox -> running");
