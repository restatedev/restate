...
mod tests {
    ...
    async fn cancel_invoked_invocation_without_pinned_deployment_with_journal_table_v2_default() {
        let mut test_env = TestEnv::create_with_min_restate_version(RESTATE_VERSION_1_6_0).await;
        let invocation_id = fixtures::mock_start_invocation(&mut test_env).await;
    ...
    -> anyhow::Result<()> {
        run_cancel_scheduled_invocation_through_notify_signal(RESTATE_VERSION_1_6_0).await
    }
    ...
    -> anyhow::Result<()> {
        run_cancel_inboxed_invocation_through_notify_signal(RESTATE_VERSION_1_6_0).await
    }
