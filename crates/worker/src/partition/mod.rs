...
use crate::partition::leadership::{AppendCallback, LeadershipState};
    ...
    T: TransportConnect,
{
        ...
        loop {
            ...
            tokio::select! {
                ...
                Some(msg) = self.network_leader_svc_rx.next(), if network_processing_permit.is_some() => {
                    ...
                    self.on_rpc(msg, live_schemas.live_load(), &last_applied_lsn_watch, network_processing_permit.unwrap()).await;
                }
    ...
    fn on_pp_ingest_request(
        ...
        let callback: AppendCallback = Box::new(move |result| match result {
            Ok(()) => reciprocal.send(ResponseStatus::Ack.into()),
            Err(
        ...
        permit.buffer_forwarded_records(request.records, callback);
    }
