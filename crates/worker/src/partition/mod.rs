    ...
    T: TransportConnect,
{
        ...
        loop {
            ...
            tokio::select! {
                ...
                maybe_first = record_stream.next() => {
                    ...
                    let bytes_limit = config.worker.write_batch_commit_bytes.as_usize();

                    // The first record is always applied; this guarantees forward progress even
                    // for a single record that is larger than `bytes_limit`.
                    let mut accumulated_bytes = first.estimated_memory_size();
                    ...
                    // Eagerly drain the records that are immediately available, applying them one
                    // by one, until we reach the record-count or the byte limit, whichever first.
                    while count < max_batching_size {
                        let next_size = match record_stream.as_mut().peek().now_or_never() {
                            Some(Some(Ok(entry))) => entry.estimated_memory_size(),
                        ...
                        if accumulated_bytes + next_size > bytes_limit {
                            // Leave the peeked record in the slot; it becomes the first record of
                            // the next batch.
                            break;
                        }
                        accumulated_bytes += next_size;

                        // The slot was primed by the peek above, so this is immediately ready and
                        // holds the `Ok` record we just inspected.
                        let entry = record_stream
                            .next()
                            .now_or_never()
    ...
    async fn apply_entry(
        ...
        let Some((lsn, record)) = self.maybe_advance(entry, transaction, started_at)? else {
        ...
        let maybe_announce_leader = self
            .apply_record(record, transaction, action_collector, vqueues)
            .await?;
