    ...
    T: TransportConnect,
{
    ...
    fn decode_record(record: Record) -> Result<Arc<v2::Envelope<v2::Raw>>, StorageDecodeError> {
        // todo(azmy): Implement a faster path that tries to decode v1 first, since
        // we don't write v2 yet it might be slightly faster to decode directly to v1, then
        // convert.

        let envelope: Arc<v2::Envelope<v2::Raw>> = match record.decode_arc() {
