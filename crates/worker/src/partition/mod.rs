...
use crate::metric_definitions::{
    PARTITION_ACTUATOR_HANDLED, PARTITION_TIMER_DUE_HANDLED, PP_APPLY_EFFECTS_DURATION,
    PP_APPLY_RECORD_DURATION, PP_APPLY_TIMERS_DURATION, PP_LOG_READ_NEXT_DURATION,
    PP_WAIT_OR_IDLE_DURATION, PARTITION_LABEL,
};
...
impl LogReader {
    ...
    async fn read_next(&mut self) -> anyhow::Result<(Lsn, Envelope)> {
        ...
        histogram!(PP_LOG_READ_NEXT_DURATION, "log_id" => self.log_id.to_string()).record(start.elapsed());
        res
    }
