...
impl KeyRangeSlicer {
    fn new(key_range: KeyRange, num_slices: NonZeroU16, start_slice: ShardIdx) -> Self {
        Self {
            shard_plan: ShardPlan::new(key_range, num_slices),
            next_slice: start_slice % num_slices.get(),
        }
    }
    ...
    fn aligned_to(
        ...
        let slice_interval_ms = u64::try_from(slice_interval.as_millis())
            .unwrap_or(u64::MAX)
            .max(1);
        let start_slice = (at.as_u64() / slice_interval_ms) % u64::from(num_slices.get());
        Self::new(key_range, num_slices, start_slice as ShardIdx)
    }
