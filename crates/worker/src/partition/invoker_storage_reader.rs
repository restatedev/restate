...
where
    for<'a> Storage: journal_table_v1::ReadOnlyJournalTable + journal_table_v2::ReadOnlyJournalTable + ReadOnlyInvocationStatusTable + Send + 'a,
{
    ...
    async fn read_journal<'a>(
        ...
        if let InvocationStatus::Invoked(invoked_status) = invocation_status {
            ...
            let journal_stream = if invoked_status.pinned_deployment.is_some_and(|p| p.service_protocol_version >= ServiceProtocolVersion::V4) {
                // If pinned service protocol version exists and >= V4, we need to read from Journal Table V2!
                journal_table_v2::ReadOnlyJournalTable::get_journal(&mut self.0, *invocation_id, journal_metadata.length)
                    .map(|entry| {
                        entry
                ...
                journal_table_v1::ReadOnlyJournalTable::get_journal(&mut self.0, invocation_id, journal_metadata.length)
                    .map(|entry| {
                        entry
                            .map_err(InvokerStorageReaderError::Storage)
                            .map(|(_, journal_entry)| match journal_entry {
                                journal_table_v1::JournalEntry::Entry(entry) => restate_invoker_api::journal_reader::JournalEntry::JournalV1(entry.erase_enrichment()),
