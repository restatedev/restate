    ...
    TStorage: ReadInvocationStatusTable,
{
    ...
    async fn handle(
        ...
        let status = match self.storage.get_invocation_status(&invocation_id).await {
            Ok(status) => status,
            Err(storage_error) => {
                return Ok(NextStep::Reply(Err(PartitionProcessorRpcError::Internal(
        ...
        if on_vqueues {
            ...
            return Ok(NextStep::Propose {
                ...
                reply_policy: ReplyPolicy::OnApplyAndFence {
                    request_id,
                    invocation_id,
                },
            });
        ...
        Ok(match status {
            InvocationStatus::Invoked(_) => NextStep::NotifyInvokerAndReply {
            ...
            | InvocationStatus::Inboxed(_) => {
                NextStep::Reply(Ok(PauseInvocationRpcResponse::NotRunning.into()))
            }
            InvocationStatus::Paused(_) | InvocationStatus::Suspended { .. } => {
                NextStep::Reply(Ok(PauseInvocationRpcResponse::AlreadyPaused.into()))
            }
            InvocationStatus::Free => {
                NextStep::Reply(Ok(PauseInvocationRpcResponse::NotFound.into()))
            }
        })
    }
}
...
mod tests {
    ...
    async fn reply_not_leader_when_not_leader() {
        ...
        let next_step = RpcHandler::handle(
