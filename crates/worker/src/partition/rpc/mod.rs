...
pub(crate) enum ReplyPolicy {
    /// Reponds to the request; the state machine's Action replies later.
    OnApply {
    ...
    /// Like OnApply, but clear the invocation's fencing token strictly AFTER the
    /// append succeeds.
    OnApplyAndFence {
        request_id: PartitionProcessorRpcRequestId,
...
pub(super) trait RpcHandler<Input> {
    ...
    fn handle(self, input: Input) -> impl Future<Output = Result<NextStep, Self::Error>>;
}
