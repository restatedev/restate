...
impl LogTrimmer {
    ...
    async fn run(self) -> anyhow::Result<()> {
        ...
        tracing::info!(log_id = %self.log_id, "Log trimmer task started");
        loop {
        ...
        tracing::info!(log_id = %self.log_id, "Log trimmer task terminated");
        Ok(())
    }
...
impl State {
    /// `cutoff` is the timestamp at which trim requests created before are considered
    /// due.
