...
use restate_types::net::partition_processor::{
    ...
    KillInvocationRpcResponse, PartitionProcessorRpcError, PartitionProcessorRpcOk,
...
/// The wire type that carries a handler's response.
type WireResponseOf<T> = <T as PartitionProcessorRpcOk>::WireEnvelope;
...
macro_rules! define_rpc_reciprocals {
    ($(
        ...
        pub enum RpcReciprocal {
            $( $tag(Reciprocal<Oneshot<WireResponseOf<$response>>>), )*
        }

        $(
            impl RpcReciprocalVariant for $response {
                fn wrap(reciprocal: Reciprocal<Oneshot<Self::WireEnvelope>>) -> RpcReciprocal {
                    RpcReciprocal::$tag(reciprocal)
                }
            }
        )*
...
/// Links a handler's response type to its [`RpcReciprocal`] variant.
trait RpcReciprocalVariant: PartitionProcessorRpcOk {
    fn wrap(reciprocal: Reciprocal<Oneshot<Self::WireEnvelope>>) -> RpcReciprocal;
}

impl<W> From<Reciprocal<Oneshot<W>>> for RpcReciprocal
where
    W: PartitionProcessorWireEnvelope,
    W::Ok: RpcReciprocalVariant,
{
    fn from(reciprocal: Reciprocal<Oneshot<W>>) -> Self {
        <W::Ok as RpcReciprocalVariant>::wrap(reciprocal)
    }
}
