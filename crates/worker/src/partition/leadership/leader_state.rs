...
use crate::partition::leadership::{
    AppendCallback, Error, InvokerStream, LeaderEvent, RpcReciprocal, TimerService,
};
...
pub struct LeaderState {
    ...
    network_events_tx: tokio::sync::mpsc::Sender<LeaderEvent>,
    network_events_stream: ReceiverStream<LeaderEvent>,
...
impl LeaderState {
    ...
    pub(super) fn try_reserve_network_event_permit(
        ...
        tokio::sync::mpsc::OwnedPermit<LeaderEvent>,
        tokio::sync::mpsc::error::TrySendError<tokio::sync::mpsc::Sender<LeaderEvent>>,
    > {
    ...
    pub async fn run(
        ...
        let network_event_stream = &mut self.network_events_stream;
    ...
    pub async fn stop(mut self) {
        ...
        // No network processing permit can outlive the partition processor's select iteration,
        // so after closing the receiver all buffered events are immediately available.
        self.network_events_stream.close();
        while let Some(Some(event)) = self.network_events_stream.next().now_or_never() {
            match event {
                LeaderEvent::RpcProposal { reciprocal, .. } => reciprocal.send(Err(
                    PartitionProcessorRpcError::LostLeadership(self.partition_id),
                )),
                LeaderEvent::IngestRecords { callback, .. } => callback(Err(
                    PartitionProcessorRpcError::LostLeadership(self.partition_id),
                )),
                _ => unreachable!("only network events are buffered"),
            }
        }
    }
    ...
    pub async fn handle_events(
        ...
        for event in events {
            match event {
                ...
                LeaderEvent::RpcProposal {
                    proposal,
                    reciprocal,
                } => {
                    self.handle_rpc_proposal(proposal, reciprocal).await;
                }
                LeaderEvent::IngestRecords { records, callback } => {
                    self.forward_many_with_callback(records.into_iter(), callback)
                        .await;
                }
            }
        }
            ...
            .await
        {
            Ok(commit_token) => {
                self.awaiting_rpc_self_propose.push(SelfAppendFuture::new(
                    commit_token,
                    Box::new(|result: Result<(), PartitionProcessorRpcError>| {
    ...
    async fn forward_many_with_callback(
        &mut self,
...
impl SelfAppendFuture {
    fn new(commit_token: CommitToken, callback: AppendCallback) -> Self {
        Self {
            commit_token,
    ...
    fn fail_with_internal(&mut self) {
        if let Some(callback) = self.callback.take() {
            callback(Err(PartitionProcessorRpcError::Internal(
    ...
    fn fail_with_lost_leadership(&mut self, this_partition_id: PartitionId) {
        if let Some(callback) = self.callback.take() {
            callback(Err(PartitionProcessorRpcError::LostLeadership(
                this_partition_id,
            )));
        }
    }
