...
pub struct DurabilityTracker {
    ...
    /// the last archived lsn we popped from the archived_lsn_watch
    last_archived: Lsn,
    terminated: bool,
}
...
impl DurabilityTracker {
    ...
    fn detect_durability_mode(
        ...
        // Special cases:
        // - A single node cluster with no snapshot repository configured. We still want to trim
        // after local durability. Which mode works? `ReplicaSetOnly`
        //
        // - A single node cluster **with** a snapshot repository configured. We want to trim after
        // the snapshot is taken + local durability. = `Balanced` || `SnapshotAndReplicaSet`
        //
        //   Simply put, if snapshot is not configured, what are our options?
        //   - None
        //   - ReplicaSetOnly
        //
        ...
        //   - SnapshotAndReplicaSet (not supported
        //   - SnapshotOnly
        let durability_mode = input_durability_mode.unwrap_or_else(|| {
...
impl Stream for DurabilityTracker {
    ...
    fn poll_next(
        ...
        let durability_mode = self.detect_durability_mode(None);
        ...
        if suggested > Lsn::INVALID && suggested > self.last_reported_durable_lsn {
            ...
            // We don't want to keep reporting the same durable Lsn over and over.z
