...
use restate_wal_protocol::v2::{Dedup, Envelope, PartialRecord, Raw};
...
impl SelfProposer {
    ...
    pub async fn self_propose_many(
        ...
        let envelopes = records.enumerate().map(|(idx, record)| {
            record.build(Dedup::SelfProposal {
                leader_epoch,
    ...
    pub async fn self_propose(&mut self, record: PartialRecord) -> Result<(), Error> {
        ...
        let record = record.build(Dedup::SelfProposal {
    ...
    pub async fn append_with_notification(
        ...
        let record = envelope.build(Dedup::None);
