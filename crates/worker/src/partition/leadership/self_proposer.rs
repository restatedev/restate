...
use restate_wal_protocol::v2::{Command, Dedup, Envelope, PartialEnvelope, Raw};
...
impl SelfProposer {
    ...
    pub async fn self_propose_many<P: Into<PartialEnvelope>>(
        &mut self,
        records: impl ExactSizeIterator<Item = P>,
    ) -> Result<(), Error> {
        ...
        let envelopes = records.enumerate().map(|(idx, record)| {
            let partial = record.into();
            partial.build(Dedup::SelfProposal {
                leader_epoch,
