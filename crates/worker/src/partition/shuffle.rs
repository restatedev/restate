...
use crate::metric_definitions::{PARTITION_LABEL, PARTITION_SHUFFLED_MESSAGE_COUNT};
    ...
    OR: OutboxReader + Send + Sync + 'static,
{
    pub(super) fn create_hint_sender(&self) -> HintSender {
        self.inner.create_hint_sender()
    }

    pub(super) fn run(self) -> impl Future<Output = anyhow::Result<()>> {
        self.inner.run()
    }
}
...
#[cfg(not(feature = "legacy-ingestion"))]
pub(super) struct ShuffleInner<T, OR> {
    metadata: ShuffleMetadata,
    outbox_reader: OR,
    ingestion_client: IngestionClient<T, Envelope>,
    ...
    OR: OutboxReader + Send + Sync + 'static,
{
    ...
    async fn shuffle(self) -> anyhow::Result<()> {
        ...
        let ingested_counter = counter!(
            PARTITION_SHUFFLED_MESSAGE_COUNT,
        ...
        loop {
            ...
            tokio::select! {
                commit_token = stream.shuffle_next_message() => {
    ...
    OR: OutboxReader + Send + Sync + 'static,
{
    pub(super) fn new(
        ...
        channel_size: usize,
        bifrost: &Bifrost,
        _ingestion_client: IngestionClient<T, Envelope>,
    ) -> Self {
        Self {
            inner: ShuffleInner::new(
                metadata,
    ...
    OR: OutboxReader + Send + Sync + 'static,
{
    ...
    pub(super) async fn shuffle(self) -> anyhow::Result<()> {
        use std::sync::Arc;
        ...
        let state_machine = state_machine::StateMachine::new(
            metadata,
            outbox_reader,
            move |msg| {
...
#[cfg(all(test, not(feature = "legacy-ingestion")))]
mod tests {
    use std::iter;
    ...
    use crate::partition::shuffle::{
        OutboxReader, OutboxReaderError, ShuffleInner, ShuffleMetadata,
    };
    ...
    struct ShuffleEnv<OR> {
        ...
        shuffle: ShuffleInner<FailingConnector, OR>,
    }
    ...
    async fn create_shuffle_env<OR: OutboxReader + Send + Sync + 'static>(
        ...
        let shuffle = ShuffleInner::new(metadata, outbox_reader, truncation_tx, 1, ingress.clone());
...
#[cfg(all(test, feature = "legacy-ingestion"))]
mod tests {
    use std::iter;
    use std::sync::Arc;
    use std::sync::atomic::{AtomicUsize, Ordering};
    ...
    async fn create_shuffle_env<OR: OutboxReader + Send + Sync + 'static>(
        ...
        let shuffle = ShuffleInner::new(metadata, outbox_reader, truncation_tx, 1, bifrost.clone());
