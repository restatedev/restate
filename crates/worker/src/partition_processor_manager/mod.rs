        ...
        match inner {
                        ...
                        if let Some(processor_state) = self.processor_states.get_mut(&partition_id)
                        {
                            match processor_state {
                                ProcessorState::Starting { target_run_mode } => {
                                    debug!(%partition_id, "Partition processor started successfully.");
                            ...
                            debug!(%partition_id, "Started partition processor is no longer needed. Stopping it.");
            ...
            EventKind::Stopped(result) => {
                match self.processor_states.remove(&partition_id) {
                    None => {
                        debug!(%partition_id, "Stopped partition processor which is no longer running.");
                    }
                    Some(processor_state) => match processor_state {
                        ...
                        ProcessorState::Stopping {
                            ...
                            debug!(%partition_id, "Partition processor stopped: {result:?}");
                    ...
                    match result {
                        ...
                        Err(err) => {
                            if processor_state.is_valid_leader_epoch_token(leader_epoch_token) {
                                info!(%partition_id, "Failed obtaining new leader epoch: {err}. Stopping partition processor now.");
                                processor_state.stop();
                            } else {
                                debug!(%partition_id, "Received outdated new leader epoch. Ignoring it.");
                            }
                        }
                    }
                } else {
                    debug!(%partition_id, "Partition processor is no longer running. Ignoring new leader epoch result.");
                }
            }
        }
    }
