...
impl PartitionProcessorManager {
    ...
    fn trigger_periodic_partition_snapshots(&mut self) {
        ...
        let (mut known_archived_lsn, unknown_archived_lsn): (Vec<_>, Vec<_>) =
            potential_snapshot_partitions.partition_map(|(partition_id, status)| {
                match self.latest_snapshots.get(&partition_id) {
                    Some(latest_snapshot) => Either::Left((partition_id, latest_snapshot.clone())),
                    None => Either::Right((partition_id, status)),
                }
            });

        for (partition_id, status) in unknown_archived_lsn {
            match self.archived_lsns.get(&partition_id) {
                Some(&archived_lsn) => {
                    known_archived_lsn.push((
                        partition_id,
                        LatestSnapshot {
                            snapshot_id: SnapshotId::new(),
                            log_id: LogId::default_for_partition(partition_id),
                            snapshot_lsn: status.last_applied_log_lsn.unwrap_or(Lsn::INVALID),
        ...
        let snapshot_partitions = known_archived_lsn
            .into_iter()
            .filter_map(|(partition_id, latest_snapshot)| {
                ...
                if records_per_snapshot.is_none_or(|num_records| {
                    ...
                    latest_snapshot
                        ...
                        .elapsed()
                        .ok()
                        .is_some_and(|age| age > with_jitter(interval.into(), 0.1))
                }) {
                    Some(partition_id)
                } else {
                    None
