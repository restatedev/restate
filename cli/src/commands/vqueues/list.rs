...
pub struct List {
    ...
    /// Order the results by oldest virtual queues first
    #[clap(long)]
    oldest_first: bool,
...
async fn list(env: &CliEnv, opts: &List) -> Result<()> {
    ...
    let direction = if opts.oldest_first { "ASC" } else { "DESC" };
    let rows: Vec<VQueueRow> = client
        .run_json_query(format!(
            "SELECT {VQUEUE_COLUMNS} FROM sys_vqueue_meta \
             ORDER BY created_at {direction}, id LIMIT {}",
            opts.limit
        ))
        .await?;
