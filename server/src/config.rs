...
/// Except for the `main` section, for those, they can be addressed directly like
...
impl Configuration {
    ...
    pub fn load_with_default(
        ...
        let figment = figment
            ...
            .merge(
                // Allows RESTATE_... to also work with the `main` section without having to
                // specify `RESTATE_MAIN__`
                Env::prefixed("RESTATE_")
                    .split("__")
                    .map(|k| format!("main.{}", k).into()),
            )
            // Override tracing.log with RUST_LOG, if present
            .merge(
                Env::raw()
                    .only(&["RUST_LOG"])
                    .map(|_| "main.log_filter".into()),
            )
            .merge(
                Env::raw()
                    .only(&["HTTP_PROXY"])
