...
//! Integration tests for incremental snapshot support with SST deduplication.
...
/// Test that incremental snapshots correctly rotate and clean up old snapshots,
/// preserving SSTs still referenced by retained snapshots.
...
async fn incremental_snapshot_cleanup() -> googletest::Result<()> {
    ...
    base_config.common.log_filter = "restate_partition_store::snapshots=debug,restate_worker::partition_processor_manager=debug,restate_partition_store::memory=warn,info".to_owned();
    ...
    // Create snapshots by sending bursts and waiting for automatic snapshots
    info!(
    ...
    for snapshot_num in 0..TOTAL_SNAPSHOTS {
        ...
        // Send a burst of invocations to exceed the record threshold
...
/// Get the latest snapshot ID from the partition directory.
/// Returns None if no snapshot exists, otherwise returns the snapshot_id.
...
/// Count the number of retained snapshots in the partition directory.
...
/// Verify snapshot repository state and return retained snapshot IDs and referenced SSTs.
...
/// Test that rapid back-to-back manual snapshots correctly coordinate via leases,
/// ensuring cleanup doesn't race with snapshot updates.
#[test_log::test(tokio::test(flavor = "multi_thread", worker_threads = 4))]
async fn back_to_back_manual_snapshots() -> googletest::Result<()> {
    ...
    // Wait for cleanup to complete - use longer timeout than the automatic snapshot test
    // since rapid manual snapshots may queue up more cleanup work
...
/// Test that concurrent snapshot requests for the same partition result in only
/// one snapshot being created, with others receiving SnapshotInProgress errors.
#[test_log::test(tokio::test(flavor = "multi_thread", worker_threads = 4))]
