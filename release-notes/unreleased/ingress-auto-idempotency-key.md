...
in `NodesConfiguration` at provisioning time and cannot be toggled later. We
plan to support a manual post-provisioning toggle in a future release, with
the caveat that flipping the flag re-shards in-flight idempotent invocations
onto different partitions and therefore breaks deduplication for a short
