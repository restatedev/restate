...
2. **Unified eager state controls**: The `enable_lazy_state` (per-service/per-handler) and
   `eager_state_size_limit` settings are now unified internally. Setting `eager-state-size-limit`
   to `0` is equivalent to enabling lazy state.
