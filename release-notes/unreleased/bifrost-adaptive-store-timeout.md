...
The `store_timeout` configuration field under
...
store_timeout = "250ms..15s"   # default
```
...
- **Customised retry policy**: If you previously tuned `sequencer_retry_policy`,
  migrate to `store_timeout` (see Migration Guidance below). The new field
  covers both the per-wave timeout and the inter-wave backoff delay.
- **New deployments**: The default `"250ms..15s"` range applies.

### Migration Guidance

Remove `sequencer_retry_policy` from your configuration and, if needed, set
`store_timeout` to reflect your desired min/max bounds:
