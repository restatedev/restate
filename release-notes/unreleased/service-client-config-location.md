# Release Notes: Move `service-client` options under `worker.invoker.service-client`
...
section under `worker.invoker.service-client`. The previous root-level location continues
...
`worker.invoker.service-client` makes the ownership explicit and aligns them with the
...
```toml
# After — under worker.invoker.service-client
...
[worker.invoker.service-client.http-keep-alive-options]
interval = "1m"
timeout = "30s"
```
