...
- During a rolling upgrade, a `V1.7` partition leader **rejects** ingestion requests coming from
  pre-`v1.7` ingestion clients (which do not send a leader epoch). These requests are **rejected,
  not lost**: the affected clients keep retrying (with back-pressure) and resume successfully once
  they are upgraded. This is intentional, to avoid data loss from clients that cannot participate
