...
used for in-flight data flowing between the server and service deployments. This includes journal
entries being replayed, service state being sent, and response bodies being received from
deployments. Previously, this memory usage was unbounded and could grow without limit under
high load or when processing large payloads.
...
- **Per-invocation budgets**: Each invocation receives a per-direction budget (inbound and
  outbound) capped at `worker.invoker.per-invocation-memory-limit` (defaults to
  `message-size-limit`, typically **32 MiB**). Inbound covers data flowing from deployments to
  the server (response bodies, commands). Outbound covers data flowing from the server to
  deployments (journal entries, state).
...
- **Scheduler-level gating**: The scheduler only dispatches new invocations when the global
  ...
  (`worker.invoker.per-invocation-initial-memory`, default **64 KiB**). This prevents
...
large state, or large response bodies) could cause the server to consume unbounded memory,
potentially leading to OOM kills or degraded performance for all invocations on the node.
...
# Maximum memory a single invocation may use per direction (inbound and outbound).
...
# Memory reserved from the global pool before an invocation starts, split
# equally between inbound and outbound directions.
...
# maximum concurrency.
# Default: "64KB"
per-invocation-initial-memory = "64KB"
```
...
- Each invocation's per-direction budget can grow up to `per-invocation-memory-limit` by
  drawing from the global pool as needed.
...
- **Users processing very large payloads or state**: Invocations that read or write data
  exceeding the per-invocation memory limit (default 32 MiB per direction) may now be paused
  with an error message like:

  > memory budget exhausted (upper bound exceeded) while reading journal entries: needed
  > 45000000 inbound and 0 outbound bytes
