...
The `version` field continues to work for backward compatibility but will be removed in a future release. If both `image.tag` and `version` are set, `image.tag` takes precedence.
