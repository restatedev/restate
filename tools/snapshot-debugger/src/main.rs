...
//! An offline tool that opens Restate partition snapshots from a snapshot repository (S3 or
//! compatible) and runs SQL queries against them — without a running cluster. It auto-discovers
//! the partitions in the repository, downloads each partition's latest snapshot (caching the
//! imported store locally so re-runs skip the download), wires up the partition store and the
//! DataFusion query layer, and drops you into an interactive SQL terminal.
...
use restate_cli_util::{CliContext, CommonOpts, c_println, c_warn};
...
#[command(
    author,
    version,
    about = "Run SQL queries against Restate partition snapshots"
)]
struct Args {
...
fn main() -> anyhow::Result<()> {
    ...
    config.common.set_base_dir(&args.data_dir);
    ...
    runtime.block_on(
        async move {
            let tracing_guard = init_tracing_and_logging(&config.common, "snapshot-debugger")
...
async fn run(
    ...
    match &args.query {
        Some(query) => repl::run_query(&query_context, query).await,
        None => repl::run_repl(&query_context, &args.data_dir).await,
    }
}
