...
pub enum DisableNodeError {
    #[error("log server is part of a node set of loglet {0}")]
    NodeSetMember(LogletId),
    #[error("log server cannot be disabled because it is in read-write state")]
    ReadWrite,
    #[error("The current default loglet provider '{0}', does not support disabling nodes")]
