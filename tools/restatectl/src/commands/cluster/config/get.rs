...
async fn config_get(connection: &ConnectionInfo, _get_opts: &ConfigGetOpts) -> anyhow::Result<()> {
    ...
    let cluster_configuration: ClusterConfigurationSeed = cluster_configuration
        .try_into()
        .context("Configuration type mismatch")?;

    let output = toml::to_string_pretty(&cluster_configuration).unwrap();

    c_title!("⚙️", format!("Version: {}", configuration.version));
    c_println!("{}", output);
