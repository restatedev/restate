...
pub async fn run_metadata(global_opts: &GlobalOpts, cmd: &Metadata) -> Result<()> {
    ...
    for loglet_id in &loglet_ids {
        ...
        let local_tail = find_local_tail(&db_info.db, &data_cf, *loglet_id)?;
...
fn format_trim_point(trim_point: LogletOffset) -> String {
    if trim_point == LogletOffset::INVALID {
        "-".to_string()
    } else {
        trim_point.to_string()
    }
}
