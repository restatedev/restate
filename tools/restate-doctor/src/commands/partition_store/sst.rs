            ...
            if let Some(file_num) = extract_file_number(&filename) {
                ...
                file_map.insert(
                    ...
                    FileInfo {
                        ...
                        smallest_seqno: live_info.map(|i| i.smallest_seqno),
                        largest_seqno: live_info.map(|i| i.largest_seqno),
                    },
                );
            }
        }
            ...
            let file_info = get_file_info_from_cf(db_info, &live_info.column_family_name, filename)
                .unwrap_or_else(|| {
                    ...
                    FileInfo {
                        ...
                        smallest_seqno: Some(live_info.smallest_seqno),
                        largest_seqno: Some(live_info.largest_seqno),
                    }
                });
...
fn print_file_details(file_num: u64, file: &FileInfo) {
    ...
    // Sequence number range (indicates write order/age)
    if let (Some(smallest), Some(largest)) = (file.smallest_seqno, file.largest_seqno) {
        let spread = largest.saturating_sub(smallest);
        info.add_kv_row(
            "Seqno Range:",
            format!("{} .. {} (span: {})", smallest, largest, spread),
        );
    }
