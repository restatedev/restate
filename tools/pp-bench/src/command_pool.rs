...
impl CommandPool {
    ...
    pub fn new(envelopes: Vec<Envelope<Raw>>) -> Self {
        assert!(
            !envelopes.is_empty(),
    ...
    pub fn with_lsns(envelopes: Vec<Envelope<Raw>>, lsns: Vec<Lsn>) -> Self {
        assert!(
            !envelopes.is_empty(),
            "CommandPool must have at least one command"
        );
        assert_eq!(envelopes.len(), lsns.len(), "commands and LSNs must match");
        Self {
            envelopes,
            lsns: Some(lsns),
            index: 0,
    ...
    pub fn next_envelope(&mut self) -> (Envelope<Raw>, Option<Lsn>) {
        let envelope = self.envelopes[self.index].clone();
        let lsn = self.lsns.as_ref().map(|l| l[self.index]);
        self.index = (self.index + 1) % self.envelopes.len();
        (envelope, lsn)
    }
