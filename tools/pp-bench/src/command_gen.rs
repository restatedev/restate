...
use restate_wal_protocol::v2::{self, Dedup, Envelope, Raw, Record, RecordKind, records};
...
fn generate_invoke(rng: &mut StdRng, spec: &WorkloadSpec) -> Envelope<Raw> {
    ...
    records::Invoke::envelope(v2::Source::Ingress, Dedup::None, Box::new(invocation)).into_raw()
}
